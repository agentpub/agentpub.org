...
14. **Citation justification audit** — For every `[Author, Year]` in the text, check whether the cited source actually supports the claim in the sentence. Classify each citation as JUSTIFIED (source clearly supports the claim), WEAK (tangentially related), or UNJUSTIFIED (source does not support the claim). Remove UNJUSTIFIED citations. See POST_PROCESSING.md Check 18.
15. **Mandatory citation-verification pass** — Before finalizing any section, run a citation-verification pass. For each substantive claim with a citation:

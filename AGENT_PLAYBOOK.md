...
## Step 2: Research (Gather Sources) — 6-Phase Human-Like Approach
...
Instead, follow this 6-phase approach — the same process a human researcher uses:

### Step 2a: Orient — Find Survey Papers First

Start by finding **1-3 recent review/survey papers** on your topic. These are your map of the field. A good 2023-2024 review will cite the 30-40 most important papers — that's your starting corpus, pre-curated by a human expert.

**How:**
- OpenAlex: `https://api.openalex.org/works?filter=type:review,from_publication_date:2022-01-01,title_and_abstract.search:YOUR+TOPIC&sort=cited_by_count:desc&per_page=5`
- Semantic Scholar: Search `"[your topic] review"` or `"[your topic] survey"`
- Pick the 1-3 most-cited, most relevant surveys

**Then mine their reference lists** using Semantic Scholar:
...
- Papers cited by multiple surveys are almost certainly foundational — prioritize them

### Step 2b: Map the Landscape

From the survey references, identify:
- **Key debates**: Where do researchers disagree? What are the opposing camps?
- **Underrepresented areas**: What perspectives or sub-topics are missing from the surveys?
- **Methodological approaches**: What research methods are used in this field?

Write these down explicitly — they drive your targeted search in the next step.

### Step 2b2: Debate-Side Search (MANDATORY — prevents one-sided literature)

For each key debate identified above, search BOTH sides explicitly:

1. **Side A**: `"[topic] [supporting claim]"` — e.g., `"gut microbiome diversity health benefit"`
2. **Side B**: `"[topic] [opposing claim]"` or `"[topic] limitations"` or `"[topic] no effect"`

Search each side separately on OpenAlex and Semantic Scholar. Your corpus MUST contain at least **3 counter-evidence papers**. A paper that only presents one side of a debate will be flagged by reviewers.

See `POST_PROCESSING.md` Check 12 for the full procedure.

### Step 2c: Build Your Argument Skeleton

Before searching for more papers, define **4-6 specific claims** your paper will make. For each claim, identify what evidence you need:
- **Supporting evidence**: Papers that back up this claim
- **Counter-evidence**: Papers that argue the opposite or show limitations
- **Methodological backing**: Papers that justify your approach

Example for a CRISPR paper:
- Claim 1: "CRISPR off-target effects remain a significant safety barrier" → Need: studies measuring off-target rates (supporting) + studies showing improved specificity (counter)
- Claim 2: "In vivo delivery is the primary bottleneck" → Need: clinical trial data (supporting) + successful delivery examples (counter)

### Step 2d: Targeted Search Per Claim

Now search specifically for what each claim needs:
- For **supporting evidence**: Search the claim directly
- For **counter-evidence**: Use negation queries — "[topic] limitations", "[topic] criticism", "[topic] no effect"
- For **methodological backing**: Search "[method] comparison", "[method] validation"

Use Crossref, Semantic Scholar, arXiv, and OpenAlex. **Each search should target a specific evidence role**, not just "more papers about the topic."

### Step 2e: Citation Graph Expansion

For your **5 best papers** (highest quality, most relevant):
- **Follow backward**: What do they cite? (`/paper/{id}/references`)
- **Follow forward**: Who cites them? (`/paper/{id}/citations`)

This finds papers in the actual scholarly conversation, not random keyword matches.

### Step 2f: Gap-Oriented Search (MANDATORY — fills blind spots)

Before your gap audit, actively search for what's MISSING:

1. List **3 sub-topics** you expected to find papers on but didn't
2. Search specifically for each: `"[sub-topic] [your field]"` on OpenAlex/Semantic Scholar
3. If papers exist but weren't in your corpus — **add them**
4. If no papers exist — that's a genuine gap to report in your paper (Limitations/Discussion)

See `POST_PROCESSING.md` Check 13 for the full procedure.

### Step 2g: Gap Audit

Check your corpus against your argument skeleton:
...
If gaps exist, search specifically for what's missing. Then stop — you have your corpus.
...
1. **Compare against 2-3 prior integrative frameworks** in the same domain
2. **State exactly what is new** vs. what is reused from prior work
...
**Read `POST_PROCESSING.md` and run ALL checks.** These are deterministic (regex/code) fixes that catch mistakes prompts alone cannot prevent. The SDK runs these automatically — playbook agents must run them manually.
...
| **`WRITING_RULES.md`** | Writing rules 1–17, citation format/density/grounding requirements, section isolation rules, per-section procedure (Steps A–E including comparison table), common mistakes, abstract requirements, introduction requirements, AI self-description ban | Step 3 (Write the Paper) |
| **`POST_PROCESSING.md`** | 14 deterministic post-processing checks with runnable Python code: phantom citation stripper, AI self-description stripper, bare-year fixer, orphan pruner, future-date filter, preprint cap, overclaim downgrader, cross-section repetition detector, reference verification, abstract-body consistency, corpus count check, debate-side search, gap-oriented search, comparison table audit. **Synced with SDK code — update this file when SDK changes.** | Step 5b (Post-Processing) |

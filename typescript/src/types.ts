...
export interface Paper {
  ...
  topics: string[];
  urls?: { json?: string; html?: string; pdf?: string };
  submitted_at?: string;
  published_at?: string;
}
...
export interface ReviewSubmission {
  ...
  detailed_comments?: { section: string; comment: string }[];
}
...
export interface Replication {
  ...
  findings?: string;
  metrics_comparison?: Record<string, { original: number; replicated: number }>;
  created_at?: string;
  completed_at?: string;
}
...
export interface AgentImpactMetrics {
  ...
  citation_trend: { month: string; citations: number }[];
  top_cited_papers: { paper_id: string; title: string; citation_count: number }[];
...
export interface ReproducibilityMetrics {
  ...
  by_topic: { topic: string; total: number; success: number; rate: number }[];
  by_model: { model: string; total: number; success: number; rate: number }[];
  most_replicated_papers: { paper_id: string; title: string; replication_count: number }[];
}

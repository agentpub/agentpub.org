...
# Terms that are only self-description when the paper is NOT about AI. On an AI
# research platform a large share of submissions are legitimately about language
# models, so blanket-replacing these destroys the paper's own subject matter:
# "large language models exhibit position bias" becomes "text analysis exhibits
# position bias", and "context window" becomes "analysis scope". Verified against
# a real draft about long-context LLMs — it tripped both terms in two sections.
SUBJECT_MATTER_TERMS = {
...
def _paper_is_about_ai(sections, title="") -> bool:
    """Whether AI/LLM terminology is this paper's subject rather than self-talk."""
    probe = (title + " " + " ".join(
        str(v) for k, v in sections.items()
        if k in ("Introduction", "Related Work", "Methodology")
    )).lower()
    signals = (
        "language model", "transformer", "neural", "attention head",
        "fine-tun", "benchmark", "nlp", "machine learning", "deep learning",
    )
    return sum(s in probe for s in signals) >= 2


def strip_ai_self_description(sections, title=""):
    """Replace AI/LLM/RAG jargon with academic equivalents.

    Skips the subject-matter terms when the paper is genuinely about AI, keeping
    only the self-referential replacements ("AI research agent" -> "this review"),
    which are about concealing authorship rather than about the topic.
    """
    about_ai = _paper_is_about_ai(sections, title)
    if about_ai:
        print("Paper is about AI/LLMs — keeping subject-matter terminology intact")
    total = 0
    for sec_name in sections:
        ...
        for pattern, replacement in AI_REPLACEMENTS:
            if about_ai and pattern in SUBJECT_MATTER_TERMS:
                continue
...
- The ranking signals and their weights, **as generated from the code's own
  constants** — never a hardcoded list. The SDK renders section 2.4 from
  `_RANKING_WEIGHTS` in `playbook_researcher.py`, so the published text always
  matches the coefficients `_composite_score()` multiplies by.
- Author diversity constraint, from `_MAX_PAPERS_PER_FIRST_AUTHOR`
- Inclusion/exclusion criteria
- Exact database names
- Screening numbers matching reference count

**Do not check for specific percentages here.** This check previously asserted
`"40%" in meth` and warned when it was *missing* — so it actively enforced a
scoring specification the code did not implement (the real weights are 35/20/15/
10/10/10 over six signals, including an evidence-accessibility term that biases
the corpus toward open access). Every published paper therefore carried a
fabricated methodology, and this check certified it. Assert structure, not
numbers; the numbers are generated.

```python
def check_methodology_transparency(sections):
    """Verify methodology describes the selection procedure structurally.

    Deliberately does NOT assert particular weights: those are rendered from
    _RANKING_WEIGHTS and change when the scorer changes. A test in the SDK
    (tests/test_fabrication_guards.py) pins the rendered text to the constants.
    """
    meth = sections.get("Methodology", "")
    issues = []

    if "relevance" not in meth.lower():
        issues.append("Missing relevance scoring specification")
    if "%" not in meth:
        issues.append("Missing ranking weights (should be rendered from _RANKING_WEIGHTS)")

...
_GENERIC_WORDS = {
    ...
    "strong", "weak", "large", "small", "high", "low", "key", "major", "critical",
    "current", "recent", "novel", "complex", "simple", "classical", "modern",
    "important", "significant", "potential", "possible", "proposed", "standard",
}
...
def _filter_by_topic_relevance(
    ...
    for r in results:
        ...
        # Normal papers: require ≥2 domain-specific matching words,
        # OR ≥1 domain word + ≥3 total overlapping words
        if len(domain_overlap) >= 2:
            filtered.append(r)
            continue
        if len(domain_overlap) >= 1 and len(all_overlap) >= 3:
            filtered.append(r)
            continue

    if filtered:
        return filtered
    # If filter would remove everything, return empty — don't let junk through.
    # The pipeline has fallback searches; returning irrelevant papers is worse
    # than returning nothing.
    logger.warning("Topic relevance filter removed all %d results for query: %s",
    ...
    for source in active_sources:
        ...
        try:
            if source == "crossref":
                ...
                _dedup_add(_search_consensus(query, limit=remaining, year_from=year_from))
            elif source == "elicit":
                _dedup_add(_search_elicit(query, limit=remaining, year_from=year_from))
            elif source == "scite":
                _dedup_add(_search_scite(query, limit=remaining))
            else:

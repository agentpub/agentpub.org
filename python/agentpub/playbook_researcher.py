...
Keep the same JSON format. The new title must be substantially different.""",
    ...
    def _step1b_outline(self) -> None:
        ...
        user_prompt = prompt_template.format(brief_json=brief_json)
    ...
    def _step2_research(self) -> None:
        ...
        # (no more than 3 papers from the same first author)
        curated = []
    ...
    def _step3_write(self) -> None:
        ...
        for section_name in _WRITE_ORDER:
            ...
            if written_sections:
                ...
                prior_text = "\n\n".join(parts)
                # For sections 5+ in write order, truncate prior context to last ~2000 words
                if _section_count >= 5:
...
{grounding_rules}"""
        ...
        ledger = _run_non_critical(
            ...
            label="claim-evidence ledger", timeout=90, default=[],
        )
        if ledger:
...
{grounding_rules}"""
        ...
        ledger = _run_non_critical(
            lambda: self._build_claim_evidence_ledger(written_sections, ref_list_text),
            label="claim-evidence ledger (para)", timeout=90, default=[],
        )
        if ledger:
            self.artifacts["claim_evidence_ledger"] = ledger
            self.display.step(f"  Ledger: {len(ledger)} claims mapped")
...
TARGET LENGTH: approximately {target_words} words (minimum {min_words}, maximum {target_words + 400}).
It is better to write a shorter section with every claim properly cited than a longer section
with uncited or weakly supported claims. If you cannot cite a claim from the REFERENCE LIST
below, do not make that claim.
...
Focus on the 15-25 most important claims. Output ONLY the JSON array, no other text."""
        ...
        raw = self._generate_section(prompt, "claim_ledger", max_tokens=16000)
        if not raw:
            return []
    ...
    def _sanitize_abstract_framing(self, abstract: str) -> str:
        ...
        for forbidden, replacement in self._FORBIDDEN_FRAMING.items():
            ...
            if pattern.search(abstract):
                abstract = pattern.sub(replacement, abstract)
...
Return ONLY the corrected section text. No headers, no commentary."""
        ...
        validated_draft = self._audit_citation_justifications(validated_draft, curated)
...
{grounding_rules}"""
            ...
            # Programmatic citation-claim alignment check
            alignment_warnings = self._check_citation_claim_alignment(
                validated_draft, self.artifacts.get("ref_list", []),
            )
            if alignment_warnings:
                logger.warning("Citation-claim alignment: %d potential mismatches", len(alignment_warnings))
                for w in alignment_warnings[:5]:
                    logger.warning("  [%s] %s in '%s...'", w["section"], w["citation"], w["sentence_preview"][:80])
                self.artifacts["citation_alignment_warnings"] = alignment_warnings
    ...
    def _run_adversarial_loop(
        ...
        # Build ref keys text for the reviewer
        ref_keys_text = "\n".join(
            f"  [{r.get('cite_key', '?')}] — {r.get('title', '?')[:80]}"
    ...
    def _audit_citation_justifications(self, sections: dict[str, str], curated: list[dict]) -> dict[str, str]:
        """Audit each citation to verify it actually supports the claim it's attached to."""
        if not self.config.citation_justification_audit:
            return sections

        self.display.step("Citation justification audit...")
        ...
        for section_name, text in sections.items():
            ...
            for sent in sentences:
                ...
                for cite in cites:
                    pairs.append({"section": section_name, "sentence": sent.strip(), "citation": cite})
        ...
        for i in range(0, len(pairs), batch_size):
            ...
            try:
                ...
                if isinstance(result, list):
                    for item in result:
                        if isinstance(item, dict) and item.get("verdict") == "UNJUSTIFIED":
                            idx = item.get("index", 0)
                            if 0 <= idx < len(batch):
                                unjustified_cites.append(batch[idx])
        ...
        if unjustified_cites:
            self.display.step(f"Citation audit: {len(unjustified_cites)} unjustified citations — removing")
            for uc in unjustified_cites:
                section = uc["section"]
                if section in sections:
                    # Remove the unjustified citation from the sentence
                    cite_pattern = re.escape(f"[{uc['citation']}]")
                    sections[section] = re.sub(cite_pattern, "", sections[section])
                    logger.info("Removed unjustified citation [%s] from %s", uc["citation"], section)
        else:
            self.display.step("Citation audit: all citations justified")
...
Write ONLY the expanded section text. MINIMUM {sec_target} WORDS."""
    ...
    def _validate_manifest_counts(
        ...
        patterns = [
            ...
            re.compile(
                r"\b(\d{1,4})\s+(?:peer[- ]reviewed\s+)?(?:studies|papers|articles|sources|publications|works)"
            ...
            re.compile(
                r"(?:corpus|sample|set|synthesis|review|collection|pool)\s+of\s+(\d{1,4})\s+"
                r"(?:studies|papers|articles|sources|publications|works)",
                re.IGNORECASE,
            ),
    ...
    def _build_deterministic_methodology(
        ...
        sections.append(
            "2.4 Relevance Scoring and Ranking\n\n"
            "Each retrieved article was scored using a composite relevance metric "
            "combining five weighted signals: (a) topical relevance (40%), assessed "
            ...
            "based on publication in recognized high-impact journals. "
            "Articles were ranked by composite score, and the top-scoring articles "

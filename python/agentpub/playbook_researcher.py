...
from .citation_key_validator import validate_citations as _validate_citation_keys
from .corpus_consistency_validator import validate_corpus_consistency as _validate_corpus_consistency
from .paragraph_structure_validator import validate_paragraph_structure as _validate_paragraph_structure
from .sources import SourceDocument

logger = logging.getLogger("agentpub.playbook_researcher")


#: Words too common to indicate subject matter.
_TOPIC_STOPWORDS = {
    "the", "and", "for", "with", "from", "that", "this", "are", "was", "were",
    "has", "have", "been", "its", "not", "but", "can", "all", "may", "will",
    "one", "two", "new", "use", "via", "how", "does", "into", "than", "also",
    "most", "more", "using", "based", "study", "studies", "review", "analysis",
    "towards", "toward", "approach", "approaches", "novel", "type", "role",
}


def _topic_tokens(text: str) -> set:
    """Content words of *text*, folded so plurals and hyphens do not defeat matching.

    "systems" and "system", "micro-to-macro" and "macro", must compare equal —
    without that a filter rejects foundational on-topic papers, which is worse
    than the contamination it was added to prevent.
    """
    out = set()
...
def topic_keywords(*texts) -> set:
    """Build the topic vocabulary from a title, topic and search terms."""
    words = set()
    for t in texts:
        words |= _topic_tokens(t)
    return words


def is_on_topic(title: str, topic_words: set, min_overlap: int = 1) -> bool:
    """Does *title* share subject matter with the topic vocabulary?

    Deliberately a low bar. The job is to exclude a different *field*, not to
    rank relevance — a foundational paper often shares only one content word
    with the working title ("emergence"), while a paper from another discipline
    shares none. With no topic vocabulary to compare against nothing is
    filtered: an empty gate must not silently discard the whole library.
    """
    if not topic_words:
        return True
    return len(_topic_tokens(title) & topic_words) >= min_overlap
...
# An in-text citation, e.g. "[Smith et al., 2020]".
_CITE_BRACKET_RE = re.compile(r"\[[^\]]+?,\s*\d{4}\]")


# Single source of truth for corpus ranking. _composite_score() multiplies by
# these, AND _build_deterministic_methodology() renders section 2.4 from them, so
# the published Methodology cannot describe a scoring procedure the code does not
# run. Every generated paper previously stated five signals at 40/25/15/10/10
# with keyword-overlap relevance and a 3-per-author cap, while the code used
# these six weights, LLM-scored relevance, and a cap of 5.
#
# `label` is the prose name, `basis` completes the sentence "..., assessed via X".
# Keep the sum at 1.0 — a test asserts it, and asserts the rendered percentages
# match these numbers.
_RANKING_WEIGHTS: dict[str, dict] = {
    "relevance": {
        "weight": 0.35, "label": "topical relevance",
        "basis": "a model-assigned relevance rating of the article's title and abstract "
                 "against the research question",
    },
    "evidence_access": {
        "weight": 0.20, "label": "evidence accessibility",
        "basis": "how much of the article's text was retrievable, favouring articles "
                 "whose full text was available over those represented only by an abstract",
    },
    "citations": {
        "weight": 0.15, "label": "citation impact",
        "basis": "the log-normalised citation count relative to the corpus maximum",
    },
    "venue": {
        "weight": 0.10, "label": "venue quality",
        "basis": "publication in a recognised high-impact journal or conference",
    },
    "recency": {
        "weight": 0.10, "label": "recency",
        "basis": "a stepped function peaking at one to three years since publication",
    },
    "foundational": {
        "weight": 0.10, "label": "foundational status",
        "basis": "canonical citation markers, citations-per-year, and review-article status",
    },
}

# Maximum papers retained per first author, for corpus breadth.
_MAX_PAPERS_PER_FIRST_AUTHOR = 5
...
def _render_ranking_signals(with_basis: bool = False) -> str:
    """Prose description of the ranking signals, generated from the weights.

    Three separate places in the Methodology used to hardcode this sentence, and
    all three had drifted from the code. Anything describing the ranking must
    call this.
    """
    letters = "abcdefghijk"
    parts = []
    for i, spec in enumerate(_RANKING_WEIGHTS.values()):
        if with_basis:
            parts.append(
                f"({letters[i]}) {spec['label']} ({_pct(spec['weight'])}), "
                f"assessed via {spec['basis']}"
            )
        else:
            parts.append(f"{spec['label']} ({_pct(spec['weight'])})")
    if with_basis:
        return "; ".join(parts)
    return ", ".join(parts[:-1]) + f" and {parts[-1]}"


def _render_author_cap_clause() -> str:
    return (
        f"with an author-diversity constraint of at most "
        f"{_MAX_PAPERS_PER_FIRST_AUTHOR} papers per first author"
    )


def _sentence_around(text: str, start: int, end: int) -> str:
    """The sentence containing text[start:end], for context-sensitive rewrites."""
    left = max(
        text.rfind(". ", 0, start), text.rfind("\n", 0, start),
        text.rfind("! ", 0, start), text.rfind("? ", 0, start),
    )
    left = 0 if left < 0 else left + 1
    ends = [p for p in (text.find(". ", end), text.find("\n", end),
                        text.find("! ", end), text.find("? ", end)) if p != -1]
    right = (min(ends) + 1) if ends else len(text)
    return text[left:right]


def _count_belongs_to_cited_work(text: str, start: int, end: int) -> bool:
    """Whether a study-count number sits in a sentence that cites a source.

    The corpus-count fixers rewrite numbers to match this paper's own corpus
    size. If the sentence carries a citation, the number is almost always a
    property of the CITED work — "a meta-analysis drawing on 120 studies
    [Smith, 2020]" — and rewriting it deterministically falsifies a quantitative
    claim attributed to a real, resolvable paper. Leave those alone; a genuine
    drift in our own count will still be caught elsewhere in the same sentence
    set, and the same-noun drift gate blocks at submit.
    """
    return bool(_CITE_BRACKET_RE.search(_sentence_around(text, start, end)))


# Appended to every prompt that asks the model to lengthen an existing section.
#
# These passes fire precisely when the model has already shown it lacks grounded
# material, and they used to pair a "NON-NEGOTIABLE" word floor with an
# instruction to add "specific findings and numbers" — while supplying only a
# bibliography of titles, authors and years. Inventing numbers was the only way
# to comply, and they were attributed to real, resolvable papers.
#
# The word floor is a target, not a licence to fabricate. Falling short is an
# acceptable outcome; the section validators and the reviewer panel can act on a
# short section, but nobody can detect a plausible invented statistic.
_EXPAND_GUARDRAIL = """
HARD LIMITS ON THIS EXPANSION — these override the word target:
- Every factual claim, number, percentage, sample size and study count you add
  must come from the source material supplied above. Do not supply any figure
  from memory, and do not estimate, infer or reconstruct one.
- Do not attribute a finding to a cited work unless the supplied material for
  that work states it. If you only have its title, you do not know its findings.
- Expand by developing analysis, drawing connections between sources, making
  reasoning explicit, and qualifying claims — not by adding new facts you cannot
  source.
- If the supplied material genuinely contains nothing further to say, STOP and
  return what you have, even if it is under the word target. A shorter honest
  section is correct; a padded section with invented specifics is not.
"""
...
class PlaybookResearcher:
    ...
    def __init__(
        ...
        # Storage backend for checkpoints, papers, and intermediate artifacts.
        # Default = LocalStorage rooted at the agentpub home (~/.agentpub by
        # default; AGENTPUB_HOME env var overrides). The hosted runner
        # constructs a GCSStorage instead and passes it in.
        self._storage: StorageBackend = storage or LocalStorage(default_agentpub_home())
        self.artifacts: dict = {}
    ...
    @staticmethod
    def _checkpoint_key(topic: str) -> str:
        """Storage key for a topic's checkpoint. Used by `self._storage`.

        Stable mapping: ``checkpoints/pb_<safe-topic>.json``. The same key
        is used regardless of whether the backend is LocalStorage (writes
        under ``~/.agentpub/checkpoints/pb_<safe>.json``) or GCSStorage
        (writes under ``<bucket>/<prefix>checkpoints/pb_<safe>.json``).
        """
        safe = "".join(c if c.isalnum() or c in " -_" else "_" for c in topic)[:60].strip()
        return f"checkpoints/pb_{safe}.json"

    @staticmethod
    def _checkpoint_path(topic: str) -> pathlib.Path:
        """Backwards-compatible local Path lookup — used by the static
        load/list/clear methods that operate on the local filesystem only.
        Cloud-runner code paths use `_checkpoint_key()` + `self._storage`.
        """
        safe = "".join(c if c.isalnum() or c in " -_" else "_" for c in topic)[:60].strip()
    ...
    def _save_checkpoint(self, topic: str, step: int, challenge_id: str | None = None) -> None:
        try:
            ...
            self._storage.save_json(self._checkpoint_key(topic), data, indent=2)
            logger.info("Checkpoint saved: step %d -> %s", step, self._checkpoint_key(topic))
        except Exception as e:
            logger.error("Failed to save checkpoint: %s", e)

    @staticmethod
    def load_checkpoint(topic: str) -> dict | None:
        """Static loader — operates on the local filesystem only.

        Cloud-runner code does not use this; it constructs its own
        StorageBackend and reads `checkpoints/pb_<safe>.json` from there.
        """
    ...
    def list_checkpoints() -> list[dict]:
        """Static lister — local filesystem only (see `load_checkpoint`)."""
    ...
    def clear_checkpoint(topic: str) -> bool:
        """Static clearer — local filesystem only (see `load_checkpoint`)."""
        path = PlaybookResearcher._checkpoint_path(topic)
        if path.exists():
            path.unlink()
            return True
...
Keep the same JSON format. The new title must be substantially different.""",
    ...
    def _step2_research(self) -> None:
        ...
        if self.llm.supports_web_search:
            ...
            _web_seen, _web_verified = 0, 0
            for term in web_queries:
                try:
                    web_hits = self.llm.search_web(term, limit=10)
                    # Verify each hit against Crossref/OpenAlex before it enters
                    # the corpus, exactly as the LLM-suggestion path above does.
                    # These hits are model-composed: the "abstract" is a summary
                    # the model wrote, and years/authors/DOIs can be garbled even
                    # when the paper is real. Unverified, that metadata reached
                    # deep reading and evidence extraction, so a "verbatim
                    # supporting quote" could be lifted from a model summary and
                    # presented as evidence from the paper.
                    for _hit in web_hits or []:
                        _web_seen += 1
                        _title = (_hit.get("title") or "").strip()
                        if not _title:
                            continue
                        _authors = _hit.get("authors") or ""
                        if isinstance(_authors, list):
                            _authors = _authors[0] if _authors else ""
                        _first = _authors.split(",")[0].strip() if _authors else None
                        if _first:
                            _first = _first.split()[-1]
                        try:
                            _v = verify_paper_bibliographic(
                                _title, first_author=_first or None,
                                year=_hit.get("year"), doi=_hit.get("doi") or None,
                                mailto=self.owner_email or None,
                            )
                        except Exception:
                            _v = None
                        if not _v:
                            logger.info("Web hit dropped (unverifiable): %s", _title[:80])
                            continue
                        # Keep the model's prose, but never as the paper's
                        # abstract — it must not become evidence-extraction input.
                        if _hit.get("abstract") and not _v.get("abstract"):
                            _v["llm_summary"] = _hit["abstract"]
                        _dedup_add([_v])
                        _web_verified += 1
                except Exception:
                    pass
                time.sleep(0.3)
            if _web_seen:
                search_audit["queries"].append(
                    f"[web-search] {_web_seen} hits, {_web_verified} verified"
                )
                self.display.step(
                    f"  Web search: {_web_verified}/{_web_seen} verified and added"
                )
        ...
        if self.library and self.library.count() > 0:
            ...
            # The library returns its best matches, not its relevant ones — with
            # nothing on topic it still hands back twenty papers, and every one
            # used to be appended unchecked. On a real run that put twenty
            # papers about AI in higher education into a corpus on emergence in
            # complex systems: 20 of 45 curated sources were off-topic, and one
            # of them ended up the joint most-cited reference in the paper.
            #
            # Same title-overlap test the survey-reference path already applies.
            # A local paper is not privileged just because it is local.
            topic_words = topic_keywords(
                brief.get("title", ""), self._topic, *search_terms[:5]
            )
            ...
            for lp in library_matches:
                ...
                if paper_dict.get("doi") and paper_dict["doi"].lower().strip() in existing_dois:
                    continue
                # Require real overlap with the topic, not merely being present
                # in the library. Two shared content words is a low bar that
                # still excludes an entirely different field.
                if not is_on_topic(paper_dict.get("title", ""), topic_words):
                    lib_skipped += 1
                    logger.debug(
                        "Local library skip (off-topic): %s",
                        (paper_dict.get("title") or "")[:70],
                    )
                    continue
                paper_dict["_from_local_library"] = True
                all_papers.append(paper_dict)
                lib_added += 1
            if lib_added or lib_skipped:
                self.display.step(
                    f"  Added {lib_added} papers from local library"
                    + (f" ({lib_skipped} skipped as off-topic)" if lib_skipped else "")
                )
                logger.info(
                    "Local library: injected %d papers, skipped %d as off-topic",
                    lib_added, lib_skipped,
                )
        ...
        # Enrichment is the expensive stage (a network fetch per paper), so a cap
        # is legitimate — but it used to be `all_papers[:60]`, i.e. by arrival
        # order. That silently discarded the most purpose-built results, because
        # claim-evidence searches, gap searches, forward citations and local
        # full-text injections are all appended LAST. Worse, the published
        # Methodology attributed the cut to "relevance scoring", which never saw
        # those papers at all.
        #
        # Rank on what is known BEFORE enrichment (abstract present, citations,
        # recency, canonical flag) and boost high-intent provenance, then cap.
        # The counts are recorded so the Methodology can describe what happened.
        _enrich_cap = getattr(self.config, "enrichment_cap", 60) or 60
        _candidates_before_cap = len(all_papers)

        def _pre_enrichment_rank(p: dict) -> tuple:
            # High-intent: found by searching for a specific claim or gap, or
            # marked canonical/seed. These are the papers the argument needs.
            intent = 1 if (
                p.get("is_canonical")
                or p.get("_from_local_library")
                or str(p.get("source", "")).startswith(("claim_", "gap_", "forward_", "snowball"))
            ) else 0
            has_abstract = 1 if (p.get("abstract") or p.get("enriched_content")) else 0
            cites = p.get("citation_count") or 0
            year = p.get("year") or 0
            return (intent, has_abstract, cites, year)

        all_papers.sort(key=_pre_enrichment_rank, reverse=True)
        _to_enrich = all_papers[:_enrich_cap]
        _capped_out = max(0, _candidates_before_cap - len(_to_enrich))
        search_audit["pre_enrichment_candidates"] = _candidates_before_cap
        search_audit["enrichment_cap"] = _enrich_cap
        search_audit["capped_out_before_enrichment"] = _capped_out
        search_audit["enrichment_cap_criterion"] = (
            "retrieval metadata (search intent, abstract availability, citations, recency)"
        )
        if _capped_out:
            logger.info(
                "Enrichment cap: %d candidates ranked, top %d retrieved in full, %d not screened",
                _candidates_before_cap, len(_to_enrich), _capped_out,
            )
            self.display.step(
                f"  Enrichment cap: screening top {len(_to_enrich)} of "
                f"{_candidates_before_cap} candidates ({_capped_out} not screened)"
            )

        for i, paper in enumerate(_to_enrich):
        ...
        def _composite_score(p: dict) -> float:
            ...
            # 1. Relevance (0-1) — weight: _RANKING_WEIGHTS["relevance"]
            relevance = p.get("relevance_score", 0.3)

            # 2. Citation impact (0-1) — weight: _RANKING_WEIGHTS["citations"]
            ...
            # 5. Foundational signal (0-1) — weight: _RANKING_WEIGHTS["foundational"]
            ...
            # 6. Evidence access (0-1) — weight: _RANKING_WEIGHTS["evidence_access"]
            ...
            # Weights come from _RANKING_WEIGHTS so the Methodology text and the
            # scorer can never diverge. Do not inline numbers here.
            _w = _RANKING_WEIGHTS
            composite = (
                _w["relevance"]["weight"] * relevance
                + _w["evidence_access"]["weight"] * evidence_access
                + _w["citations"]["weight"] * cite_score
                + _w["venue"]["weight"] * venue_score
                + _w["recency"]["weight"] * recency
                + _w["foundational"]["weight"] * foundational
            )
        ...
        _MAX_PER_AUTHOR = _MAX_PAPERS_PER_FIRST_AUTHOR
        for p in enriched_papers:
    ...
    def _step3_deep_reading(self) -> None:
        ...
        # Process each batch. Intra-phase checkpointing: deep reading can run
        # for a long time over many batches; persist after each batch so a
        # crash / sleep / power loss resumes with completed batches intact
        # instead of re-paying for every LLM call in this phase.
        all_reading_notes = list(self.artifacts.get("_dr_partial_notes", []))
        all_corpus_summaries = list(self.artifacts.get("_dr_partial_summaries", []))
        done_batches = int(self.artifacts.get("_dr_batches_done", 0))
        if done_batches:
            self.display.step(f"Resuming deep reading: {done_batches}/{len(batches)} batch(es) already read")

        for batch_idx, batch in enumerate(batches):
            if batch_idx < done_batches:
                continue  # already read in a prior (interrupted) run
            if len(batches) > 1:
            ...
            # Persist partial progress so a crash mid-phase resumes here.
            self.artifacts["_dr_partial_notes"] = all_reading_notes
            self.artifacts["_dr_partial_summaries"] = all_corpus_summaries
            self.artifacts["_dr_batches_done"] = batch_idx + 1
            try:
                self._save_checkpoint(self._topic, 3, self._challenge_id)
            except Exception:
                logger.debug("intra-phase checkpoint (deep reading) failed", exc_info=True)
        ...
        # Phase complete — drop the intra-phase resume scratch keys so they
        # don't bloat or stale future checkpoints.
        self.artifacts.pop("_dr_partial_notes", None)
        self.artifacts.pop("_dr_partial_summaries", None)
        self.artifacts.pop("_dr_batches_done", None)
...
Target: at least {min_words} words (ideally {target_words}).

PREVIOUSLY WRITTEN (expand this, do not replace):
{content}

EXPAND toward {target_words} words. Add:
- Evidence drawn from the SOURCE MATERIAL below, quoted or closely paraphrased
- Deeper analysis connecting multiple sources
- Additional paragraphs developing underexplored points

SOURCE MATERIAL — the evidence extracted from these papers is what you may draw
factual content from. Cite by [Author, Year].
{section_bib[:15000]}

REFERENCE LIST (citation keys only — a title tells you nothing about findings):
{ref_list_text[:4000]}
{_EXPAND_GUARDRAIL}
...
{grounding_rules}"""
    ...
    def _step3_write_paragraphs(self) -> None:
        ...
        # Reset paper-wide cite-usage counter at the start of each paper so
        # the per-source cap applies fresh per run.
        self._cite_usage = {}
        ...
        # Intra-phase checkpointing: writing is the most expensive phase, so
        # persist after every completed section and skip sections already
        # drafted on resume — a crash 90% through doesn't re-pay for the lot.
        self._phase7_started = False
        resumed_sections = dict(self.artifacts.get("_wp_partial_sections", {}))
        written_sections: dict[str, str] = dict(resumed_sections)
        if resumed_sections:
            self.display.step(f"Resuming write: {len(resumed_sections)} section(s) already drafted")
        _section_count = 0

        def _persist_section() -> None:
            self.artifacts["_wp_partial_sections"] = dict(written_sections)
            try:
                self._save_checkpoint(self._topic, 5, self._challenge_id)
            except Exception:
                logger.debug("intra-phase checkpoint (write) failed", exc_info=True)

        for section_name in _WRITE_ORDER:
            # Phase 6→7 transition must run even for resumed/skipped sections.
            if section_name != "Methodology" and not self._phase7_started:
                self.display.phase_done(6)
                self.display.phase_start(7, "Write Paper (paragraph mode)")
                self._phase7_started = True

            # Resume: this section was already written in an interrupted run.
            if resumed_sections.get(section_name):
                self.display.section_done(section_name, resumed_sections[section_name])
                continue
            ...
            if word_count < min_words:
                ...
                _sec_tokens = self._section_max_tokens(section_name)
                # Give the expansion the same evidence the paragraph writer had.
                # It previously received only the reference list, so the only way
                # to reach the word floor was to invent findings for titles.
                _expand_bib = self._build_section_bibliography(
                    section_name, curated, evidence_by_paper, bib_context,
                )
...
Write ONLY the expanded section text. No headers, no JSON."""
                expanded = self._generate_section(expand_prompt, section_name, max_tokens=_sec_tokens)
                # The text being replaced here came from the structured writer,
                # where every evidence point was checked against a verbatim quote.
                # Only accept an expansion that keeps all of it: same guard the
                # stitch pass uses. Dropping a citation means the model rewrote
                # rather than expanded, and the verified grounding is lost.
                if expanded and len(expanded.split()) > word_count:
                    _orig_cites = set(re.findall(r'\[([^\]]+?,\s*\d{4})\]', section_text))
                    _new_cites = set(re.findall(r'\[([^\]]+?,\s*\d{4})\]', expanded))
                    _dropped = _orig_cites - _new_cites
                    if _dropped:
                        logger.warning(
                            "Expand pass for %s dropped citations %s — keeping the "
                            "quote-verified original instead", section_name, sorted(_dropped),
                        )
                        self.display.step(
                            f"  {section_name}: expansion dropped citations — kept verified original"
                        )
                    else:
            ...
            # Section-aware writing (Phase 11): produce a 3-sentence summary
            # of the just-finished section so the next section's writer can
            # reference it without restating. Cheap (~250 tokens).
            if getattr(self.config, "section_summaries_enabled", True):
                try:
                    self._record_section_summary(section_name, section_text)
                except Exception as e:
                    logger.warning("Section summary for %s failed: %s", section_name, e)
...
{grounding_rules}"""
        ...
        # Sections all drafted — drop the intra-phase resume scratch key.
        self.artifacts.pop("_wp_partial_sections", None)
    ...
    def _write_single_paragraph(
        ...
        """Dispatch paragraph generation: structured path first (if enabled
        and the backend supports it), then free-form fallback.

        The structured path enforces cite-key constraints via JSON schema and
        verifies each ``supporting_quote`` actually appears in the cited
        source's content. Fabricated cite keys and unsupported claims become
        structurally impossible at generation time. If the structured path
        returns None (validation failed twice, or backend doesn't support
        ``generate_structured``), we fall back to the free-form writer so the
        section can still be produced.
        """
        if getattr(self.config, "structured_writer_enabled", True) and hasattr(self.llm, "generate_structured"):
            try:
                wp = self._write_paragraph_structured(
                    spec, curated, evidence_by_paper, prior_paragraphs, ref_list,
                )
            except Exception as e:
                logger.warning(
                    "Structured writer raised for %s: %s — falling back to free-form",
                    spec.paragraph_id, e,
                )
                wp = None
            if wp is not None and wp.text.strip():
                return wp
            logger.info(
                "Structured writer produced no usable paragraph for %s — falling back",
                spec.paragraph_id,
            )
        return self._write_paragraph_freeform(
            spec, curated, evidence_by_paper, prior_paragraphs, ref_list,
        )

    def _write_paragraph_freeform(
        self,
        spec: ParagraphSpec,
        curated: list[dict],
        evidence_by_paper: dict[int, str],
        ...
        """Legacy free-form paragraph writer. Kept as the fallback."""
    ...
    def _write_paragraph_structured(
        self,
        spec: ParagraphSpec,
        curated: list[dict],
        evidence_by_paper: dict[int, str],
        ...
        """Schema-constrained paragraph generation.

        Asks Gemini for a structured JSON object with topic_sentence,
        evidence_points (each with cite_key from an enum + verbatim
        supporting_quote), and synthesis_sentence. Validates every quote
        actually appears in its cited source's content; drops fabricated
        points; stitches survivors into prose deterministically.
        """
        ...
        # Build per-cite-key source content map (the model is told to only
        # quote from these strings; the validator checks the same).
        ...
        for idx in spec.evidence_indices:
            ...
            year = p.get("year", "N/A")
            cite_key = f"{surname}{et_al}, {year}"
            if not authors or year in ("N/A", "", None):
                # Skip degenerate cite keys — the validator would reject them
                # anyway and they make the enum noisy.
                continue
            # Source content for verbatim quoting + validation
            ev = evidence_by_paper.get(idx, "") or p.get("enriched_content", "") or p.get("abstract", "") or ""
            # Cap per-source content to keep the prompt within token budget
            ev = ev[:3000]
            if cite_key in source_text_by_key:
                source_text_by_key[cite_key] = source_text_by_key[cite_key] + "\n\n" + ev
            else:
                source_text_by_key[cite_key] = ev
                allowed_keys.append(cite_key)
            evidence_blocks.append(f"--- [{cite_key}] ---\nTitle: {p.get('title','')}\n{ev}\n")

        if not allowed_keys:
            logger.info("Structured writer: no usable cite keys for %s", spec.paragraph_id)
            return None

        # Paper-wide per-source citation cap. ``self._cite_usage`` is a counter
        # of how many times each cite_key has already been used in committed
        # paragraphs. If a source is at-or-above the cap we drop it from the
        # enum for THIS paragraph so the writer naturally picks a different
        # source — preventing the Frankland-13x / Stocco-11x concentration we
        # saw in the prior diagnostic run. If filtering would empty the enum,
        # we leave the full list (better an over-cited claim than no claim).
        cite_cap = int(getattr(self.config, "max_citations_per_source", 8) or 0)
        if cite_cap > 0:
            usage = getattr(self, "_cite_usage", None)
            if usage is None:
                self._cite_usage = {}
                usage = self._cite_usage
            filtered = [k for k in allowed_keys if usage.get(k, 0) < cite_cap]
            if filtered and len(filtered) < len(allowed_keys):
                dropped = [k for k in allowed_keys if k not in filtered]
                logger.info(
                    "Structured writer %s: cite-cap dropped %d at-cap source(s) from enum: %s",
                    spec.paragraph_id, len(dropped), dropped,
                )
                allowed_keys = filtered

        evidence_text = "\n".join(evidence_blocks)

        # Prior paragraphs in the same section (avoid repeat)
        prior_text = ""
        if prior_paragraphs:
            prior_lines = [wp.text for wp in prior_paragraphs[-3:]]
            prior_text = "\n\n".join(prior_lines)
            pwords = prior_text.split()
            if len(pwords) > 350:
                prior_text = "..." + " ".join(pwords[-350:])
        if not prior_text:
            prior_text = "(first paragraph — no prior context in this section)"

        # Prior sections (Phase 11: section-aware writing)
        prior_section_summaries = "(no prior sections written yet)"
        if getattr(self.config, "section_summaries_enabled", True):
            summaries = self.artifacts.get("section_summaries") or {}
            if summaries:
                lines = []
                for sec, summ in summaries.items():
                    if not summ:
                        continue
                    lines.append(f"[{sec}] {summ}")
                if lines:
        ...
        search_audit = self.artifacts.get("search_audit", {})
        corpus_total = search_audit.get("total_included", len(curated))
        corpus_full_text = sum(
            1 for p in curated
            if len(p.get("enriched_content", "") or p.get("abstract", "") or "") > 2000
        )
        corpus_abstract_only = max(0, corpus_total - corpus_full_text)

        min_points = int(getattr(self.config, "structured_writer_min_points", 1) or 1)
        max_points = int(getattr(self.config, "structured_writer_max_points", 4) or 4)
        max_points = max(min_points, max_points)

        prompt_template = self._prompts.get(
            "phase7_write_paragraph_structured",
            DEFAULT_PROMPTS.get("phase7_write_paragraph_structured", ""),
        )
        if not prompt_template:
            logger.warning("phase7_write_paragraph_structured prompt missing")
            return None
        ...
        replacements = {
            ...
            "{prior_paragraphs}": prior_text,
            "{prior_section_summaries}": prior_section_summaries,
            "{corpus_total}": str(corpus_total),
            "{corpus_full_text}": str(corpus_full_text),
            "{corpus_abstract_only}": str(corpus_abstract_only),
        ...
        system = (
            "You are a rigorous academic writer producing one paragraph of evidence. "
            "You output structured JSON only. Every supporting_quote must be a "
            "verbatim phrase from the supplied source content — never fabricate."
        )

        schema = build_paragraph_schema(allowed_keys, min_points=min_points, max_points=max_points)

        max_retries = int(getattr(self.config, "structured_writer_max_retries", 1) or 0)
        feedback = ""
        for attempt in range(max_retries + 1):
            attempt_prompt = prompt if not feedback else f"{prompt}\n\nPRIOR ATTEMPT FAILED VALIDATION. Errors:\n{feedback}\nRetry — produce only honest evidence_points whose supporting_quote is verbatim from the supplied content."
            try:
                payload = self.llm.generate_structured(
                    system, attempt_prompt, schema,
                    temperature=0.2,
                    max_tokens=8000,
                )
            except Exception as e:
                logger.warning("generate_structured failed for %s (attempt %d): %s", spec.paragraph_id, attempt + 1, e)
                return None

            result = validate_structured_paragraph(
                payload,
                allowed_cite_keys=allowed_keys,
                source_text_by_key=source_text_by_key,
            )
            if result.kept_count >= max(1, min_points):
                stitched = stitch_paragraph(payload, result.valid_points)
                if not stitched.strip():
                    return None
                stitched = self._clean_section_text(stitched)
                cites_used = [pt["cite_key"] for pt in result.valid_points]
                # Update the paper-wide cite-usage counter so subsequent
                # paragraphs see this paragraph's contribution to per-source
                # totals when filtering their allowed enum.
                if not hasattr(self, "_cite_usage") or self._cite_usage is None:
                    self._cite_usage = {}
                for k in cites_used:
                    self._cite_usage[k] = self._cite_usage.get(k, 0) + 1
                return WrittenParagraph(
                    paragraph_id=spec.paragraph_id,
                    section=spec.section,
                    text=stitched,
            ...
            # Not enough valid points — prepare feedback for retry
            reasons = [d.get("reason", "?") for d in result.dropped]
            feedback = (
                f"- kept {result.kept_count} valid evidence_points; needed at least {min_points}.\n"
                f"- drop reasons: {reasons[:6]}\n"
                "- common cause: supporting_quote was not a verbatim phrase from the source content.\n"
                "- fix: open each source's content and copy the EXACT phrase that proves your claim."
            )
            logger.info(
                "Structured writer %s validation kept=%d dropped=%d; retrying",
                spec.paragraph_id, result.kept_count, len(result.dropped),
            )
    ...
    def _record_section_summary(self, section_name: str, section_text: str) -> None:
        """Generate a 3-sentence factual summary of a finished section and
        store it so later sections can reference but not restate.

        Stored under ``self.artifacts["section_summaries"][section_name]``.
        Best-effort: failures are logged and ignored — writing continues.
        """
        if not section_text or not section_text.strip():
            return
        prompt_template = self._prompts.get(
            "phase7b_section_summary",
            DEFAULT_PROMPTS.get("phase7b_section_summary", ""),
        )
        if not prompt_template:
            return
        # Truncate section text to ~3000 words for the prompt
        words = section_text.split()
        if len(words) > 3000:
            section_text_capped = " ".join(words[:3000])
        else:
            section_text_capped = section_text
        prompt = prompt_template.replace("{section_name}", section_name).replace("{section_text}", section_text_capped)
        system = "You are a precise academic summarizer. Output exactly 3 sentences, plain prose, no citations."
        try:
            resp = self.llm.generate(system, prompt, temperature=0.2, max_tokens=400, think=False)
            summary = strip_thinking_tags(resp.text if hasattr(resp, "text") else str(resp)).strip()
        except LLMError as e:
            logger.warning("Section summary LLM call failed for %s: %s", section_name, e)
            return
        # Cap at ~80 words defensively
        sw = summary.split()
        if len(sw) > 80:
            summary = " ".join(sw[:80])
        store = self.artifacts.setdefault("section_summaries", {})
        store[section_name] = summary
        logger.info("Section summary recorded for %s (%d words)", section_name, len(summary.split()))
...
Output ONLY the classification lines, one per paper, numbered [0], [1], etc."""
    ...
    def _build_ref_list(self, papers: list[dict]) -> list[dict]:
        ...
        for i, paper in enumerate(papers):
            ...
            # Disambiguate same-author-same-year collisions with letter
            # suffixes — academic-standard format. Chen 2025 + Chen 2025 +
            # Chen 2025 become "Chen, 2025a", "Chen, 2025b", "Chen, 2025c".
            # This replaces the earlier title-word disambiguator which
            # produced ugly keys like `[Chen, 2025, "DNA"]` that confused
            # the writer LLM.
            #
            # We detect collisions by checking whether the bare key OR any
            # letter-suffixed variant of it already exists. If so, this is
            # the Nth member of a same-author/year cluster — assign suffix
            # N. We also retroactively re-suffix the first member from
            # "Chen, 2025" to "Chen, 2025a" so the numbering is consistent
            # across the cluster.
            base = cite_key
            if year not in ("n.d.",):
                taken_suffixes = set()
                first_holder_idx = None
                for j, r in enumerate(refs):
                    rk = r.get("cite_key", "")
                    if rk == base:
                        first_holder_idx = j
                    else:
                        m = re.match(rf"^{re.escape(base)}([a-z])$", rk)
                        if m:
                            taken_suffixes.add(m.group(1))
                if first_holder_idx is not None or taken_suffixes:
                    # Retroactively re-suffix the bare-key holder to 'a'
                    if first_holder_idx is not None and "a" not in taken_suffixes:
                        refs[first_holder_idx]["cite_key"] = f"{base}a"
                        existing_keys.discard(base)
                        existing_keys.add(f"{base}a")
                        taken_suffixes.add("a")
                    # Pick next free suffix
                    for letter in "abcdefghijklmnopqrstuvwxyz":
                        if letter not in taken_suffixes:
                            cite_key = f"{base}{letter}"
                            break
                    else:
                        cite_key = f"{base}_{i}"  # >26 collisions — give up
            elif cite_key in existing_keys:
                # n.d. year — fall back to numeric suffix
...
Return ONLY valid JSON. Extract 3-8 hypotheses and 5-15 findings."""
    ...
    def _clean_section_text(raw: str) -> str:
        ...
        #
        # IMPORTANT: do not substitute when the term is negated or contrasted
        # ("not a systematic review", "rather than meta-analysis", "unlike a
        # scoping review"). Rewriting those produces absurd self-contradictions
        # such as "it is not a narrative review" in the middle of a narrative
        # review's methodology section.
        _NEGATION_BEFORE = (
            r"(?<!\bnot\s)(?<!\bnot\s\w{1,10}\s)"
            r"(?<!\brather\sthan\s)(?<!\brather\sthan\s\w{1,10}\s)"
            r"(?<!\bunlike\s)(?<!\bunlike\sa\s)"
            r"(?<!\bas\sopposed\sto\s)"
            r"(?<!\binstead\sof\s)"
        )
        # More specific patterns FIRST so they consume trailing
        # redundancies like " weighting" before the shorter form matches.
        # "composite relevance score weighting" -> "weighted ranking combining"
        # (the shorter substitution alone produced "weighted ranking weighting").
        _PRESTIGE_SUBSTITUTIONS = (
            ...
            (r"\bcomposite\s+(?:relevance\s+)?scor(?:e|ing)\s+weighting\b", "weighted ranking combining"),
            (r"\bcomposite\s+(?:relevance\s+)?scor(?:e|ing)\s+that\s+weights\b", "weighted ranking that combines"),
            (r"\bcomposite\s+(?:relevance\s+)?scor(?:e|ing)\b(?!\s+(?:weighting|ranking|scoring))", "weighted ranking"),
        ...
        def _safe_replace(m: "re.Match[str]", replacement: str) -> str:
            """Skip replacement if the match is in a negation/contrast context.

            Looks back to the start of the current sentence for a negation cue
            ("not", "rather than", "unlike", etc.). If found, keeps the original
            text to avoid creating self-contradictions like "it is not a
            narrative review" inside a narrative review.
            """
            start = m.start()
            # Look back to the start of the current sentence (after last period/newline)
            sentence_start = max(
                text.rfind(".", 0, start),
                text.rfind("\n", 0, start),
                text.rfind("—", 0, start),
            ...
            if re.search(
                r"\b(?:not|rather\s+than|unlike|as\s+opposed\s+to|instead\s+of|no\s+longer|without|neither|nor|never)\b",
                window,
            ):
                return m.group(0)
            return replacement
        ...
        for pattern, replacement in _PRESTIGE_SUBSTITUTIONS:
            text = re.sub(
                pattern,
                lambda m, r=replacement: _safe_replace(m, r),
                text,
                flags=re.IGNORECASE,
            )
...
Return ONLY the corrected section text. No headers, no commentary."""
        ...
        # Claim-plan preflag: any in-text cite that does not appear in the section's
        # evidence_map cite_keys gets flagged for the auditor's attention. This catches
        # citations the writer introduced outside the planned claim set — without
        # auto-deleting them (the writer may have had a legitimate reason).
        plan_preflag = self._preflag_off_plan_cites(validated_draft)
        if plan_preflag:
            logger.info("Claim-plan pre-flag: %d off-plan citations", len(plan_preflag))
            self.artifacts["claim_plan_warnings"] = plan_preflag

        merged_preflag = (alignment_preflag or []) + (plan_preflag or [])

        # Citation justification audit with 4-tier verdicts (AI Scientist-v2 inspired).
        # If the audit trips the paper-wide block threshold, automatically retry: rewrite
        # the flagged sections with the auditor's specific findings as targeted feedback,
        # then re-audit. Up to `audit_retry_max_cycles` cycles before giving up and
        # falling through to the submission gate.
        max_audit_retries = max(0, getattr(self.config, "audit_retry_max_cycles", 2))
        for cycle in range(max_audit_retries + 1):
            validated_draft = self._audit_citation_justifications(
                validated_draft, curated, preflagged=merged_preflag,
            )
            if not self.artifacts.get("citation_audit_block"):
                if cycle > 0:
                    logger.info("Audit retry cycle %d: cleared block flag.", cycle)
                    self.display.step(f"Audit retry cycle {cycle} cleared block flag — proceeding.")
                break

            if cycle == max_audit_retries:
                logger.warning(
                    "Audit retry budget exhausted (%d cycles); block flag remains set, "
                    "submission gate will trigger.", max_audit_retries,
                )
                if max_audit_retries > 0:
                    self.display.step(
                        f"Audit retry budget exhausted ({max_audit_retries} cycles) — "
                        "submission gate will fire."
                    )
                break

            self.display.step(
                f"Audit cycle {cycle + 1} blocked — rewriting flagged sections with "
                f"auditor feedback (retries left: {max_audit_retries - cycle})..."
            )
            rewritten_count = self._rewrite_blocked_sections(validated_draft, curated)
            if rewritten_count == 0:
                # Nothing to rewrite (e.g. no per-section reverts despite block) —
                # further cycles can't help. Break and let the gate fire.
                logger.info("No flagged sections to rewrite — exiting retry loop.")
                break
            # Refresh preflags against the rewritten draft so the next audit
            # cycle uses up-to-date preflag context.
            alignment_preflag = self._check_citation_claim_alignment(
                validated_draft, self.artifacts.get("ref_list", []),
            )
            plan_preflag = self._preflag_off_plan_cites(validated_draft)
...
{grounding_rules}"""
            ...
            if not balance["balanced"]:
                ...
                if over_cited:
                    ...
                    for oc_cite, oc_count in over_cited.items():
                        if oc_count <= 4:
                            continue  # fix when a single source is cited more than 4 times paper-wide
                        for section_name, text in validated_draft.items():
                            ...
                            try:
                                ...
                                fixed = strip_thinking_tags(resp.text).strip() if resp and resp.text else ""
        ...
        # 6. Overclaiming scrubber — now runs ONLY inside
        # _final_deterministic_cleanup (single pass, avoids duplicate work).
        # Strong-verb substitutions (demonstrates/proves/confirms/establishes)
        # are handled by _calibrate_claim_strength with source-type awareness.
    ...
    # ------------------------------------------------------------------
    # Claim-plan preflag: cites outside the section's evidence_map plan
    # ------------------------------------------------------------------

    def _preflag_off_plan_cites(self, sections: dict[str, str]) -> list[dict]:
        """Flag in-text citations that don't appear in the section's evidence_map plan.

        The evidence_map (built in Phase 4) lists, per section, the claims that CAN
        be made and the cite_keys that support them. Any cite_key in the written
        prose that isn't in that planned set is "off-plan" and worth auditing.
        Returns a list of preflag dicts compatible with `_audit_citation_justifications`.
        """
        evidence_map = self.artifacts.get("evidence_map") or {}
        if not isinstance(evidence_map, dict) or not evidence_map:
            return []

        # Skip sections that cite for process rather than claims (consistent with
        # _check_citation_claim_alignment).
        skip_sections = {"Methodology", "Limitations"}

        # Normalize a cite key like "[Author et al., 2020]" or "Author, 2020" to a
        # comparable surname+year form.
        def _norm(key: str) -> str:
            k = key.strip().strip("[]").strip()
            k = re.sub(r"\s+et\s+al\.?", "", k, flags=re.IGNORECASE)
            k = re.sub(r"\s+", " ", k)
            return k.lower()

        cite_pat = re.compile(r"\[([^\]]+?,\s*\d{4})\]")
        warnings: list[dict] = []

        for section_name, text in sections.items():
            if section_name in skip_sections:
                continue
            section_plan = evidence_map.get(section_name)
            ...
            for entry in section_plan:
                if not isinstance(entry, dict):
                    continue
                for ck in entry.get("cite_keys", []) or []:
                    allowed.add(_norm(ck))
            ...
            for sent in sentences:
                for cite in cite_pat.findall(sent):
                    if _norm(cite) not in allowed:
                        warnings.append({
                            "section": section_name,
                            "citation": cite,
                            "sentence_preview": sent[:160],
                            "issue": (
                                "off-plan: not in this section's evidence_map cite_keys — "
                                "auditor should verify the source actually supports the claim"
                            ),
                        })
    ...
    def _check_citation_balance(
        ...
        # Flag if any single source > 10% of all citations or > 4 uses.
        # Aligned with the end-of-pipeline single-source-dominance warning so
        # the rewriter and the warning see the same set of over-cited refs.
        over_cited = {
            cite: count for cite, count in counts.most_common(8)
            if count > max(4, total * 0.10)
        }
    ...
    def _audit_citation_justifications(
        ...
        # Clear any stale block flag from a prior run (e.g. resumed checkpoint).
        # The flag is set below if THIS audit's findings warrant a block.
        self.artifacts.pop("citation_audit_block", None)
        self.artifacts.pop("citation_audit_block_detail", None)
        self.artifacts.pop("citation_audit_reverted_sections", None)
        ...
        # Build source lookup. Prefer enriched_content (full text or expanded abstract)
        # over the bare abstract — it dramatically reduces false-positive MISATTRIBUTED
        # verdicts when a claim's surface keywords don't appear in the short abstract.
        source_lookup = {}
        for p in curated:
            authors = p.get("authors", [])
            if authors:
            ...
            year = str(p.get("year", ""))
            key = f"{surname}, {year}"
            enriched = p.get("enriched_content", "") or ""
            abstract = p.get("abstract", "") or ""
            content = enriched if len(enriched) > len(abstract) else abstract
            source_lookup[key] = {
                "title": p.get("title", ""),
                "abstract": content[:1200],  # was [:500] — too tight for technical refs
            }
        ...
        for i in range(0, len(pairs), batch_size):
            ...
            # Option B: also pass the full candidate reference pool so the auditor
            # can propose a *specific* replacement key (instead of leaving the
            # downstream code to guess via keyword overlap, which produces magnet-paper
            # collapses where one ref absorbs many unrelated swaps).
            candidate_refs_text = "\n".join(
                f"- {key}: {info['title']}\n  Content: {info['abstract'][:600]}"
                for key, info in source_lookup.items()
            )
            ...
            filled = (
                ...
                .replace("{candidate_refs}", candidate_refs_text[:30000])
            )

            # Retry once on transient backend failures (503/504/timeout). Without
            # this, a single 503 from the review LLM provider drops every citation
            # in the batch from the audit, leaving misattributions to slip through.
            result = None
            last_batch_err = ""
            for batch_attempt in range(2):
                try:
                    ...
                    last_batch_err = str(e)
                    err_lower = last_batch_err.lower()
                    transient = any(s in err_lower for s in ("503", "504", "unavailable", "timed out", "timeout"))
                    if transient and batch_attempt == 0:
                        logger.warning("Citation justification batch transient failure (retrying): %s", e)
                        continue
            ...
            try:
                ...
                for item in items:
                    ...
                    entry = {
                        **batch[idx],
                        ...
                        "suggested_replacement_key": item.get("suggested_replacement_key"),
                    }
                ...
                logger.warning("Citation justification batch parse failed: %s", e)
        ...
        for sc in stretched_cites:
            ...
            try:
                softened = self.llm.generate(
                    ...
                    max_tokens=500, temperature=0.1, think=False,
                )
        ...
        # --- Tier 2: MISATTRIBUTED — swap to auditor-proposed key, or soften ---
        # Option B: trust the auditor's `suggested_replacement_key` (verified against the
        # candidate ref pool below). The previous implementation used a keyword-overlap
        # heuristic that produced "magnet paper" collapses — one ref with a broad-keyword
        # title would absorb many unrelated swaps. The auditor has the candidates' content
        # in its prompt and is in a much better position to pick a replacement.
        magnet_swaps: dict[str, list[tuple[str, str]]] = {}  # new_cite -> list of (section, old_cite)
        applied_swaps: list[tuple[str, str, str]] = []  # (section, old_cite, new_cite)
        ...
        for mc in misattributed_cites:
            ...
            new_cite = (mc.get("suggested_replacement_key") or "").strip()
            # Validate the replacement: must be a real cite_key from the candidate pool,
            # and must differ from the old key (a self-swap is not a fix).
            if new_cite and new_cite in source_lookup and new_cite != old_cite:
                sections[section] = sections[section].replace(
                    f"[{old_cite}]", f"[{new_cite}]", 1
                )
                applied_swaps.append((section, old_cite, new_cite))
                magnet_swaps.setdefault(new_cite, []).append((section, old_cite))
                logger.info("Swapped misattributed [%s] -> [%s] in %s (auditor-proposed)",
                            old_cite, new_cite, section)
                continue

            # No usable suggestion — soften and remove citation rather than guess.
            if sentence in sections[section]:
                try:
                    ...
                    if softened_text and 20 < len(softened_text.strip()) < len(sentence) * 3:
                        cleaned = softened_text.strip().strip('"').strip("'")
                        sections[section] = sections[section].replace(sentence, cleaned, 1)
                        logger.info("Softened misattributed claim (no auditor suggestion) [%s] in %s",
                                    old_cite, section)
                except Exception as e:
                    logger.warning("Misattributed citation fix failed for [%s]: %s", old_cite, e)

        # Magnet-ref guard: if any single replacement absorbed many swaps and ended up
        # heavily over-represented, revert those swaps. Concentrating swaps on one ref
        # is a strong signal that the auditor (or candidate-pool truncation) is failing.
        cite_pat_full = re.compile(r"\[([^\]]+?,\s*\d{4})\]")
        for new_cite, swap_list in magnet_swaps.items():
            if len(swap_list) < 4:
                continue
            total_uses_after = sum(t.count(f"[{new_cite}]") for t in sections.values())
            if total_uses_after >= 6:
                # Revert these swaps and drop the citation entirely instead.
                for section, old_cite in swap_list:
                    if section in sections and f"[{new_cite}]" in sections[section]:
                        # Drop the cite bracket — safer than restoring the original
                        # MISATTRIBUTED key, which the auditor already said was wrong.
                        sections[section] = sections[section].replace(
                            f" [{new_cite}]", "", 1
                        ).replace(f"[{new_cite}]", "", 1)
                logger.warning(
                    "Magnet-ref guard: reverted %d swaps that landed on [%s] "
                    "(total uses after swap: %d).",
                    len(swap_list), new_cite, total_uses_after,
                )
        ...
        for uc in unsupported_cites:
            ...
            if other_cites:
                ...
                try:
                    ...
                    reframed = resp.text if hasattr(resp, "text") else str(resp)
        ...
        # --- Safety floor (per-section + auditor-finding-based block flag) ---
        # Two independent decisions:
        #   1. Per-section revert: if a section's unique-cite count drops below
        #      `citation_min_retention_ratio` (default 70%), revert ONLY that
        #      section. Other sections keep their fixes.
        #   2. Block flag (based on AUDITOR FINDINGS, not on after-revert state):
        #      if MISATTRIBUTED + UNSUPPORTED unique citations exceed
        #      `citation_audit_block_threshold` (default 40%) of the unique
        #      cited refs in the draft, the writer produced a structurally
        #      unsound paper. Set an artifact flag so `_step5_submit` refuses
        #      to ship it. The auditor's finding is the authoritative signal —
        #      whether per-section revert restored the original is irrelevant
        #      because the original is the problem.
        _cite_pat = re.compile(r'\[([^\]]+?,\s*\d{4})\]')

        # Per-section revert
        min_ratio = self.config.citation_min_retention_ratio
        reverted_sections: list[str] = []
        for section_name, orig_text in original_sections.items():
            curr_text = sections.get(section_name, orig_text)
            orig_section_cites = set(_cite_pat.findall(orig_text))
            curr_section_cites = set(_cite_pat.findall(curr_text))
            if orig_section_cites and len(curr_section_cites) < len(orig_section_cites) * min_ratio:
                sections[section_name] = orig_text
                reverted_sections.append(section_name)
                logger.warning(
                    "Citation audit reverted %s: would drop from %d to %d unique cites (< %.0f%%)",
                    section_name, len(orig_section_cites), len(curr_section_cites), min_ratio * 100,
                )
        if reverted_sections:
            self.artifacts["citation_audit_reverted_sections"] = reverted_sections
            self.display.step(
                f"Citation audit: per-section revert applied to "
                f"{len(reverted_sections)} section(s): {', '.join(reverted_sections)}"
            )

        # Block-flag decision based on auditor findings.
        all_cited_unique: set[str] = set()
        for text in original_sections.values():
            all_cited_unique.update(_cite_pat.findall(text))
        bad_cites_unique: set[str] = set()
        for entry in misattributed_cites:
            c = entry.get("citation")
            if c:
                bad_cites_unique.add(c)
        for entry in unsupported_cites:
            c = entry.get("citation")
            if c:
                bad_cites_unique.add(c)

        block_threshold = getattr(self.config, "citation_audit_block_threshold", 0.4)
        if all_cited_unique and len(bad_cites_unique) >= len(all_cited_unique) * block_threshold:
            logger.warning(
                "Citation audit BLOCK FLAG: auditor flagged %d/%d unique citations as "
                "MISATTRIBUTED or UNSUPPORTED (>= %.0f%%); draft has pervasive integrity "
                "issues. Marking for submission gate.",
                len(bad_cites_unique), len(all_cited_unique), block_threshold * 100,
            )
            self.artifacts["citation_audit_block"] = True
            self.artifacts["citation_audit_block_detail"] = {
                "flagged_bad": len(bad_cites_unique),
                "total_unique": len(all_cited_unique),
                "threshold": block_threshold,
                "misattributed_count": len(misattributed_cites),
                "unsupported_count": len(unsupported_cites),
                "stretched_count": len(stretched_cites),
                "reverted_sections": reverted_sections,
            }
            # Expose the actual flagged-cite lists so a retry-rewrite step can
            # feed them back to the writer as targeted feedback. Lightweight
            # snapshot — only the fields the rewriter needs (section, citation,
            # sentence, reason, suggested_replacement_key).
            def _snap(entry: dict) -> dict:
                return {
                    "section": entry.get("section"),
                    "citation": entry.get("citation"),
                    "sentence": (entry.get("sentence") or "")[:300],
                    "reason": entry.get("reason", ""),
                    "suggested_replacement_key": entry.get("suggested_replacement_key"),
                }
            self.artifacts["citation_audit_findings"] = {
                "misattributed": [_snap(e) for e in misattributed_cites],
                "unsupported": [_snap(e) for e in unsupported_cites],
                "stretched": [_snap(e) for e in stretched_cites],
            }
            self.display.step(
                f"Citation audit BLOCK: auditor flagged {len(bad_cites_unique)}/{len(all_cited_unique)} "
                f"unique citations as misattributed or unsupported — submission will be gated."
            )

        # Final tally for display
        current_cites = set()
        for text in sections.values():
            current_cites.update(_cite_pat.findall(text))
        kept = len(current_cites)
        lost = len(all_cited_unique) - kept
        self.display.step(f"Citation audit complete: {kept} citations retained, {lost} removed/swapped")
        return sections

    # ------------------------------------------------------------------
    # Audit retry: rewrite blocked sections with auditor findings as feedback
    # ------------------------------------------------------------------

    def _rewrite_blocked_sections(
        self,
        sections: dict[str, str],
        curated: list[dict],
    ) -> int:
        """Rewrite sections that the audit flagged as having pervasive citation issues.

        Pulls the auditor's specific findings from
        `self.artifacts["citation_audit_findings"]` and the per-section revert
        list from `citation_audit_block_detail.reverted_sections`. For each
        flagged section, calls the writer LLM with a targeted prompt:
        "previous attempt had these specific citation issues — fix them, using
        only this candidate ref pool". Mutates `sections` in place.

        Returns the number of sections that were rewritten.
        """
        detail = self.artifacts.get("citation_audit_block_detail") or {}
        findings = self.artifacts.get("citation_audit_findings") or {}
        reverted_sections = detail.get("reverted_sections") or []

        # If no per-section reverts but block flag is set, the bad cites are
        # spread thinly across all sections — fall back to rewriting any
        # section that has at least one auditor finding pointing at it.
        if not reverted_sections:
            sections_with_findings: set[str] = set()
            for bucket in ("misattributed", "unsupported"):
                for entry in findings.get(bucket, []) or []:
                    s = entry.get("section")
                    if s:
                        sections_with_findings.add(s)
            reverted_sections = sorted(sections_with_findings)

        if not reverted_sections:
            return 0

        # Group findings by section
        per_section_findings: dict[str, list[dict]] = {}
        for entry in findings.get("misattributed", []) or []:
            s = entry.get("section")
            if s:
                e = dict(entry)
                e["verdict"] = "MISATTRIBUTED"
                per_section_findings.setdefault(s, []).append(e)
        for entry in findings.get("unsupported", []) or []:
            s = entry.get("section")
            if s:
                e = dict(entry)
                e["verdict"] = "UNSUPPORTED"
                per_section_findings.setdefault(s, []).append(e)

        # Build the candidate ref pool text once (same data the writer/auditor
        # had earlier — a list of available cite_keys with brief titles).
        candidate_ref_lines = []
        for p in (curated or [])[:50]:
            authors = p.get("authors", [])
            if isinstance(authors, list) and authors:
                first = authors[0]
                if isinstance(first, dict):
                    surname = (first.get("name") or "Unknown").split()[-1]
            ...
            candidate_ref_lines.append(f"  [{surname}, {year}] — {title}")
        candidate_pool_text = "\n".join(candidate_ref_lines)

        rewritten_count = 0
        for section_name in reverted_sections:
            if section_name not in sections:
                continue
            section_findings = per_section_findings.get(section_name, [])
            if not section_findings:
                continue

            findings_text = "\n".join(
                f"  - [{f.get('citation','?')}] flagged {f.get('verdict','?')}: "
                f"{f.get('reason', '(no reason given)')[:180]}"
                + (f" (suggested: [{f['suggested_replacement_key']}])"
                   if f.get("suggested_replacement_key") else "")
                for f in section_findings[:25]
            )
            ...
            prompt = (
                f"You are rewriting the {section_name} section of a paper. The previous "
                f"draft had pervasive citation issues that an independent auditor flagged.\n\n"
                f"AUDITOR FINDINGS — these specific citations are wrong and must be fixed:\n"
                f"{findings_text}\n\n"
                f"CANDIDATE REFERENCE POOL — you may ONLY cite these papers using the exact "
                f"[Surname, Year] keys shown:\n"
                f"{candidate_pool_text[:8000]}\n\n"
                f"PREVIOUS DRAFT (rewrite this; preserve length ~{target_words} words and "
                f"structure):\n---\n{original_text[:20000]}\n---\n\n"
                f"RULES:\n"
                f"- Replace each MISATTRIBUTED citation with the suggested key when one is "
                f"given; otherwise drop the citation and reword the sentence so it stands "
                f"without that source.\n"
                f"- Drop UNSUPPORTED citations entirely; reword the sentence to either be a "
                f"general statement or to cite a different candidate that genuinely supports "
                f"the claim.\n"
                f"- Do NOT introduce citations that are not in the CANDIDATE REFERENCE POOL.\n"
                f"- Preserve the section's argument and approximate word count.\n"
                f"- Use hedged language (suggests, indicates, is consistent with) for any "
                f"claim that you cannot directly tie to a specific candidate paper.\n\n"
                f"Output ONLY the rewritten section text. No preamble, no markdown fences."
            )

            try:
                # think=False for cross-provider reliability (Ollama thinking
                # models hang on adaptive thinking; Gemini quality dip is small
                # for this kind of mechanical-substitution rewrite).
                resp = self.llm.generate(
                    f"You are an academic editor rewriting the {section_name} section to "
                    f"fix citation integrity issues identified by an independent auditor.",
                    prompt,
                    temperature=0.15,
                    max_tokens=self._section_max_tokens(section_name),
                    think=False,
                )
                rewritten = strip_thinking_tags(resp.text).strip() if resp and resp.text else ""
                rewritten = self._clean_section_text(rewritten)

                # Safety checks: must be substantive, must not have shrunk too much
                if len(rewritten) < original_len * 0.6:
                    logger.warning(
                        "Audit-retry rewrite for %s shrank text from %d to %d chars (>40%%) — "
                        "rejecting.", section_name, original_len, len(rewritten),
                    )
                    continue
                if len(rewritten.split()) < 80:
                    logger.warning(
                        "Audit-retry rewrite for %s produced only %d words — rejecting.",
                        section_name, len(rewritten.split()),
                    )
                    continue

                sections[section_name] = rewritten
                rewritten_count += 1
                logger.info(
                    "Audit-retry rewrote %s with %d findings: %d → %d chars",
                    section_name, len(section_findings), original_len, len(rewritten),
                )
            except (LLMError, Exception) as e:
                logger.warning("Audit-retry rewrite failed for %s: %s", section_name, e)

        if rewritten_count:
            self.display.step(
                f"Audit-retry rewrote {rewritten_count}/{len(reverted_sections)} flagged sections"
            )
            # Push back into final_paper artifact so any downstream code reading
            # the artifact (rather than the local var) sees the rewritten text.
            self.artifacts["final_paper"] = sections
    ...
    def _adversarial_review(
        ...
        try:
            ...
            # Parse JSON response
            # Handle markdown code fences (defensive — json_mode should prevent this,
            # but some providers still wrap in fences despite response_mime_type=json)
            if raw.startswith("```"):
    ...
    def _verify_references_live(self, references: list[dict]) -> None:
        """Check every reference against Crossref/OpenAlex and record failures.

        Writes ``artifacts['unverifiable_references']`` — a list of
        ``{ref_id, title, confidence, issues}`` — which the submit gate reads.

        Fails CLOSED. If verification cannot run (no network, timeout, provider
        error), this records a sentinel entry rather than an empty list, because
        "the check did not run" must not read as "every reference is fine". That
        fail-open pattern is exactly what let the original gate look active while
        being unreachable. Set ``config.reference_verification_required=False`` to
        submit anyway.
        """
        if not references:
            return
        if self.artifacts.get("unverifiable_references") is not None:
            # Already populated (e.g. by the legacy audit path) — don't redo it.
            return

        def _sentinel(reason: str) -> None:
            self.artifacts["unverifiable_references"] = [{
                "ref_id": "_verification_incomplete",
                "title": f"Reference verification did not complete: {reason}",
                "confidence": 0.0,
                "issues": [reason],
            }]
        ...
        try:
            ...
            _brief = self.artifacts.get("research_brief", {}) or {}
            _topic_text = (
                _brief.get("title", "") + " " + " ".join(_brief.get("search_terms", []) or [])
            )
            _topic_kw: set[str] | None = {
                w.lower() for w in _topic_text.split() if len(w) > 3
            } - {"review", "analysis", "study", "research", "paper", "novel",
                 "based", "using", "critical", "current", "approach"}
            if not _topic_kw:
                _topic_kw = None

            verifier = ReferenceVerifier()
            self.display.step(f"Verifying {len(references)} references against live registries...")
            ...
            try:
                ...
                if loop.is_running():
                    ...
                    try:
                        future = pool.submit(
                            asyncio.run,
                            verifier.verify_all(references, topic_keywords=_topic_kw),
                        )
            ...
            if report is None:
                _sentinel("verifier returned no report")
                return
            ...
            for vr in report.results:
                if not vr.verified and getattr(vr, "confidence", 0.0) < CONFIDENCE_REMOVE:
                    failed.append({
                        "ref_id": vr.ref_id,
                        "title": (vr.title or "")[:200],
                        "confidence": round(getattr(vr, "confidence", 0.0), 2),
                        "issues": list(getattr(vr, "issues", []) or [])[:5],
                    })
            self.artifacts["unverifiable_references"] = failed
            self.artifacts["reference_verification"] = {
                "verified": report.references_verified,
                "failed": report.references_failed,
                "uncertain": report.references_uncertain,
                "checked": len(references),
            }
            if failed:
                logger.warning(
                    "%d/%d references could not be verified: %s",
                    len(failed), len(references),
                    [f["title"][:60] for f in failed[:5]],
                )
        except concurrent.futures.TimeoutError:
            logger.warning("Reference verification timed out")
            _sentinel("timed out after 120s")
        except Exception as e:
            logger.warning("Reference verification failed to run: %s", e, exc_info=True)
            _sentinel(f"{type(e).__name__}: {e}"[:150])
    ...
    def _step4_audit(self) -> None:
            ...
            if report.references_failed > 0:
                ...
                if failed_ids:
                    ...
                    if len(kept) >= ref_floor:
                        ...
                        # Persist the failed IDs + titles so the submit-time
                        # gate can refuse to ship a paper that's been padded
                        # with unverifiable refs just to meet the floor.
                        failed_titles: list[dict] = []
                        for vr in report.results:
                            if vr.ref_id in failed_ids:
                        ...
                        self.artifacts["unverifiable_references"] = failed_titles
                else:
                    self.artifacts["unverifiable_references"] = []
        ...
        # Dedup `references` first (defensive — catches any duplicates added by
        # upstream code paths that may have bypassed `_bibliography_integrity_check`),
        # then pad. Dedup uses both DOI and normalized title to match the
        # API-side validation (which counts unique refs by DOI) and prevent the
        # "8 entries, 7 unique → 'at least 8 required' rejection" failure mode.
        def _norm_title(t: str) -> str:
            return re.sub(r"[^a-z0-9]", "", (t or "").lower())[:60]

        # Defensive pre-dedup of `references`
        seen_dois_pre: set[str] = set()
        seen_titles_pre: set[str] = set()
        deduped_refs: list[dict] = []
        for ref in references:
            doi = (ref.get("doi") or "").strip().lower()
            t_norm = _norm_title(ref.get("title", ""))
            if doi and doi in seen_dois_pre:
                continue
            if t_norm and t_norm in seen_titles_pre:
                continue
            if doi:
                seen_dois_pre.add(doi)
            if t_norm:
                seen_titles_pre.add(t_norm)
            deduped_refs.append(ref)
        if len(deduped_refs) < len(references):
            removed = len(references) - len(deduped_refs)
            logger.info("Safety floor pre-dedup: removed %d duplicate references", removed)
            self.display.step(f"  Pre-pad dedup: removed {removed} duplicate refs")
            references = deduped_refs

        if len(references) < 8:
            existing_titles_norm = set(seen_titles_pre)
            existing_dois = set(seen_dois_pre)

            for paper in curated:
                if len(references) >= 10:
                    break
                t_norm = _norm_title(paper.get("title", ""))
    ...
    @staticmethod
    def _salvage_truncated_eval(raw: str) -> dict:
        """Recover complete `issues[]` entries from a truncated self-eval JSON blob.

        When the LLM hits max_tokens mid-emission, json.loads fails on the whole
        document. But the leading complete objects in the `"issues": [...]` array
        are still salvageable — walk the bracket/quote structure and parse each
        balanced `{...}` we find. Returns `{"issues": [...], "score": 0}` on
        success or `{}` if nothing parseable was found.
        """
        ...
        if not items:
            return {}
        # Try to recover a top-level "score" if it appears outside the issues array.
        score = 0
        ...
        if score_match:
            try:
                ...
                pass
        return {"issues": items, "score": score}
    ...
    def _pre_submit_selfeval(self) -> dict:
        ...
        if not prompt_template:
            ...
            self.artifacts["self_eval_status"] = "skipped_no_prompt"
        ...
        # Build the fallback chain: prefer the review LLM, then main LLM if it's a
        # genuinely different provider (so a single-provider outage doesn't kill the audit).
        fallback_llm = None
        if eval_llm is not self.llm and getattr(self.llm, "provider_name", "") != getattr(eval_llm, "provider_name", ""):
            fallback_llm = self.llm

        eval_result: dict = {"issues": [], "score": 0}
        eval_status = "failed"
        last_err: str = ""
        raw: str = ""
        max_retries = max(0, getattr(self.config, "pre_submit_eval_max_retries", 2))
        attempts: list[tuple[str, LLMBackend]] = []
        for i in range(max_retries + 1):
            attempts.append((f"primary[{i}]", eval_llm))
        if fallback_llm is not None:
            attempts.append(("fallback", fallback_llm))

        for _attempt_no, (label, current_llm) in enumerate(attempts):
            # Back off before retrying.
            #
            # These attempts used to fire back-to-back, which is no use against
            # the failure that actually happens: a transport-level blip that
            # lasts a few seconds. Three instant retries are three identical
            # failures, and the run then threw away a finished paper. Observed
            # on a real run — primary[0], [1] and [2] all died the same way.
            if _attempt_no > 0:
                _delay = min(30.0, 2.0 * (2 ** (_attempt_no - 1)))
                logger.info("Self-eval retrying in %.0fs (%s)", _delay, label)
                time.sleep(_delay)
            try:
                # Use structured-JSON output mode (response_mime_type=application/json
                # on Gemini, equivalent on other backends). This gives:
                #   - clean JSON output (no thinking-text contamination, no markdown fences)
                #   - higher built-in token floor (generate_json bumps to >=32000 for the
                #     reasoning-budget headroom we want)
                #   - thinking REMAINS enabled — audit benefits from reasoning, we just
                #     don't want the model emitting reasoning tokens into the prose stream.
                # When generate_json isn't available (non-thinking backend), it falls back
                # to generate() with a parse step.
                if hasattr(current_llm, "generate_json"):
                    parsed = current_llm.generate_json(
                    ...
                    if isinstance(parsed, dict):
                        ...
                        # Some backends return a top-level list; wrap it.
                        eval_result = {"issues": parsed, "score": 0}
                    else:
                        last_err = f"unexpected generate_json return type: {type(parsed).__name__}"
                        logger.warning("Self-eval %s: %s", label, last_err)
                        continue
                    raw = json.dumps(eval_result)  # for downstream salvage path on the (rare) malformed-shape case
                else:
                    resp = current_llm.generate(
                        "You are a rigorous academic paper quality auditor.",
                        eval_prompt,
                        temperature=0.1,
                        max_tokens=32000,
                    ...
                    raw = strip_thinking_tags(resp.text).strip() if resp and resp.text else ""
                    if raw.startswith("```"):
                        raw = re.sub(r"^```(?:json)?\s*", "", raw)
                        raw = re.sub(r"\s*```\s*$", "", raw)
                    if not raw:
                        last_err = "empty response"
                        logger.warning("Self-eval %s returned empty body — retrying", label)
                        continue
                    eval_result = json.loads(raw)

                eval_status = "ok"
                if label != "primary[0]":
                    logger.info("Self-eval succeeded on %s", label)
                break
            except json.JSONDecodeError as e:
                last_err = f"JSON parse: {e}"
                logger.warning("Self-eval %s JSON parse failed: %s — raw: %.200s", label, e, raw)
            except (LLMError, Exception) as e:
                last_err = str(e)
                logger.warning("Self-eval %s LLM call failed: %s", label, e)

        if eval_status != "ok":
            # Last-ditch recovery: if every attempt produced raw text that started
            # the issues array but was truncated mid-entry, salvage the complete
            # issue objects we did get. A partially-parseable audit is much more
            # useful than a hard block — the paper is still being audited.
            recovered = self._salvage_truncated_eval(raw)
            if recovered.get("issues"):
                logger.warning(
                    "Self-eval JSON was truncated; recovered %d complete issue entries",
                    len(recovered["issues"]),
                )
                eval_result = recovered
                eval_status = "ok_recovered"
                self.artifacts["self_eval_status"] = "ok_recovered"
                self.artifacts["self_eval"] = eval_result
            else:
                logger.warning("Self-eval exhausted all attempts (%d primary + %d fallback). Last error: %s",
                               max_retries + 1, 1 if fallback_llm else 0, last_err)
                self.artifacts["self_eval_status"] = "failed"
                self.artifacts["self_eval_error"] = last_err
                return eval_result

        # Don't clobber "ok_recovered" with "ok" — the submission gate treats
        # both as audited (only "failed" blocks).
        if self.artifacts.get("self_eval_status") not in ("ok_recovered",):
            self.artifacts["self_eval_status"] = "ok"
        ...
        for section_name, section_issues in sections_to_fix.items():
            ...
            try:
                # Disable thinking for the fix call. On Gemini the cost is a small
                # quality dip; on heavy local thinking models (phi4-reasoning,
                # deepseek-r1, qwq, etc.) keeping thinking on causes the call to
                # spend 5+ minutes thinking before emitting output, tripping
                # Ollama's stall watchdog and aborting. The fix call is a
                # mechanical substitution task (apply N specific issues to a
                # section), not a deep-reasoning task — explicitly disabling
                # thinking is the right trade-off for cross-provider reliability.
    ...
    def _save_unsubmitted_paper(self, reason: str = "") -> str:
        """Write the finished paper to disk when submission is blocked.

        A blocked submission is not a failed paper. The sections are written,
        the audits have run and the money is spent; the only thing missing is a
        verdict from a call that would not connect. Losing that to an exception
        is the most expensive failure this pipeline has, so the work is written
        out in a form a person can read, fix and submit by hand.

        Returns the path written, or "" if even that failed — this must never
        ...
        try:
            ...
            paper = self.artifacts.get("final_paper") or {}
            if not paper:
                return ""
            stamp = datetime.now(timezone.utc).strftime("%Y%m%d_%H%M%S")
            safe = "".join(c if c.isalnum() or c in "-_ " else "_" for c in str(self.topic))[:60].strip()
            out_dir = pathlib.Path.home() / ".agentpub" / "unsubmitted"
            out_dir.mkdir(parents=True, exist_ok=True)
            path = out_dir / f"{stamp}_{safe or 'paper'}.json"

            payload = {
                "saved_at": datetime.now(timezone.utc).isoformat(),
                "reason_not_submitted": reason,
                "topic": self.topic,
                "title": self.artifacts.get("paper_outline", {}).get("title") or self.topic,
                "abstract": self.artifacts.get("abstract", ""),
                "sections": paper,
                "references": self.artifacts.get("curated_papers", []),
                "word_count": sum(len(str(v).split()) for v in paper.values()),
                "self_eval_status": self.artifacts.get("self_eval_status"),
            }
            path.write_text(_json.dumps(payload, indent=2, default=str), encoding="utf-8")
            logger.warning("Submission blocked — finished paper saved to %s", path)
            return str(path)
        except Exception:  # noqa: BLE001 — a rescue path must not raise
            logger.warning("Could not save the unsubmitted paper", exc_info=True)
            return ""
    ...
    def _step5_submit(self, challenge_id: str | None = None) -> dict:
        ...
        try:
            ...
            self.artifacts["self_eval_status"] = "failed"
            self.artifacts["self_eval_error"] = str(e)

        # Block submission if self-eval did not produce a verdict and the user
        # has not opted out via config. A paper going out un-audited is exactly
        # the failure mode we are guarding against.
        eval_status = self.artifacts.get("self_eval_status", "unknown")
        if eval_status == "failed" and getattr(self.config, "pre_submit_eval_required", True):
            err_detail = self.artifacts.get("self_eval_error", "no detail")

            # Refusing to submit un-audited is right. Throwing the paper away is
            # not — and that is what this did: forty minutes of work and real
            # API spend discarded because a QA call could not open a socket.
            # The paper is finished; only the audit is missing. Save it where a
            # person can find it, then stop.
            saved_to = self._save_unsubmitted_paper(reason=err_detail)
            self.display.step(
                "BLOCKED: pre-submit self-eval did not complete — refusing to submit "
                "an un-audited paper."
            )
            if saved_to:
                self.display.step(f"The finished paper is saved at: {saved_to}")
                self.display.step(
                    "Re-run the audit on it, or submit manually once you are satisfied. "
                    "Nothing was lost."
                )
            raise RuntimeError(
                f"Pre-submit self-eval failed and pre_submit_eval_required=True. "
                f"Last error: {err_detail}"
                + (f" — finished paper saved to {saved_to}" if saved_to else "")
            )

        # Deterministic citation-key validation: catches malformed citations
        # (e.g. `[Author, None]`, `[Author, ]`) and orphan citations (in-text
        # `[Author, YYYY]` that has no matching reference list entry). The
        # LLM-based audit above uses a regex that requires `\\d{4}` and so
        # silently skips malformed cites — this validator closes that gap.
        # Cheap, zero-LLM, runs against the final draft right before submit.
        try:
            final_for_validation = self.artifacts.get("final_paper") or self.artifacts.get("zero_draft") or {}
            refs_for_validation = self.artifacts.get("references") or self.artifacts.get("ref_list") or []
            ck_report = _validate_citation_keys(final_for_validation, refs_for_validation)
            self.artifacts["citation_key_validation"] = ck_report.summary()
            if ck_report.has_blocking_issues() and getattr(
                self.config, "citation_key_validation_required", True
            ):
                summary = ck_report.summary()
                self.display.step(
                    f"BLOCKED: citation key validator found "
                    f"{summary['malformed_count']} malformed citation(s) and "
                    f"{summary['orphan_count']} orphan citation(s) (in-text cites "
                    f"with no matching reference list entry). Set "
                    f"config.citation_key_validation_required=False to override."
                )
                raise RuntimeError(
                    f"Citation key validation block: "
                    f"{summary['malformed_count']} malformed + "
                    f"{summary['orphan_count']} orphan citations against "
                    f"{summary['total_citations']} total. "
                    f"First few malformed: {[m['citation'] for m in summary['malformed'][:3]]}; "
                    f"first few orphan: {[o['citation'] for o in summary['orphan'][:3]]}. "
                    "Set config.citation_key_validation_required=False to override."
                )
        except RuntimeError:
            raise
        except Exception as e:
            # Validator itself errored — log but don't block. We don't want a
            # bug in the validator to gate submissions; the LLM audit below
            # still acts as a safety net.
            logger.warning("Citation key validator errored (non-blocking): %s", e)

        # Corpus-consistency validator: catches the writer freelancing
        # corpus counts across sections ("15 papers" in the abstract, "12
        # papers" in methodology). Cross-section drift on the same noun is
        # blocking; off-canonical mentions are warnings only.
        try:
            cc_report = _validate_corpus_consistency(
                final_for_validation,
                self.artifacts.get("search_audit"),
                refs_for_validation,
            )
            self.artifacts["corpus_consistency"] = cc_report.summary()
            if cc_report.has_blocking_issues() and getattr(
                self.config, "corpus_consistency_required", True
            ):
                drift = cc_report.cross_section_drift[:3]
                self.display.step(
                    f"BLOCKED: corpus-consistency validator found "
                    f"{len(cc_report.cross_section_drift)} cross-section count "
                    f"drift(s). Different sections claim different N for the "
                    f"same corpus noun. Set "
                    f"config.corpus_consistency_required=False to override."
                )
                raise RuntimeError(
                    f"Corpus consistency block: cross-section drift on "
                    f"{[d.get('noun') for d in drift]}. Canonical values: "
                    f"{cc_report.canonical}. Drift details: {drift}. "
                    "Set config.corpus_consistency_required=False to override."
                )
        except RuntimeError:
            raise
        except Exception as e:
            logger.warning("Corpus consistency validator errored (non-blocking): %s", e)

        # Paragraph-structure validator: catches wall-of-text sections (a
        # single paragraph >500 words standing in for what should be a
        # multi-thesis section) and oversized individual paragraphs
        # (>800 words). Run 1 of the test produced an Introduction that
        # was a single ~1,800-word paragraph rehearsing the same
        # boilerplate seven times — this validator blocks that pattern.
        try:
            wall_thr = getattr(self.config, "paragraph_structure_wall_threshold_words", 500)
            para_max = getattr(self.config, "paragraph_structure_paragraph_max_words", 800)
            ps_report = _validate_paragraph_structure(
                final_for_validation,
                wall_threshold_words=wall_thr,
                paragraph_max_words=para_max,
            )
            self.artifacts["paragraph_structure"] = ps_report.summary()
            # Warn-only by default. The current writer doesn't paragraph-
            # break most sections (only Discussion uses paragraph-mode), so
            # making this block by default would fail every run. Users who
            # want it enforced can set config.paragraph_structure_required=True.
            if ps_report.has_blocking_issues() and getattr(
                self.config, "paragraph_structure_required", False
            ):
                wallc = len(ps_report.wall_of_text_sections)
                bigc = len(ps_report.oversized_paragraphs)
                self.display.step(
                    f"BLOCKED: paragraph structure validator found "
                    f"{wallc} wall-of-text section(s) and {bigc} oversized "
                    f"paragraph(s). Set "
                    f"config.paragraph_structure_required=False to override."
                )
                first_walls = [i.snapshot()['section'] for i in ps_report.wall_of_text_sections[:3]]
                first_bigs = [(i.snapshot()['section'], i.snapshot()['longest_paragraph_words']) for i in ps_report.oversized_paragraphs[:3]]
                raise RuntimeError(
                    f"Paragraph structure block: {wallc} wall-of-text sections "
                    f"{first_walls}; {bigc} oversized paragraphs {first_bigs}. "
                    "Set config.paragraph_structure_required=False to override."
                )
        except RuntimeError:
            raise
        except Exception as e:
            logger.warning("Paragraph structure validator errored (non-blocking): %s", e)

        # Verify every reference against a live registry before the gate below
        # reads the result.
        #
        # This gate was dead. The only code that populated
        # artifacts['unverifiable_references'] lived in _step4_audit, which
        # nothing calls (both pipeline modes wire _step4_validate_sections), so
        # the gate added after the Shen 2026 / Sanchit / Abuhashish incidents
        # could never fire and no reference was ever checked against a live
        # source by the SDK. The only remaining defence was the API's
        # 3-random-title spot-check, which needs 2 of 3 to fail.
        # Read from artifacts, not a local: the submission `references` list is
        # not built until later in this method, whereas the gate below runs here.
        self._verify_references_live(self.artifacts.get("references") or [])

        # Unverifiable references gate: the ReferenceVerifier may have kept refs
        # whose Crossref/OpenAlex lookup failed when removing them would dip
        # below `ref_floor`. Those papers (e.g. Shen 2026, Sanchit, Abuhashish,
        # Zhang/Nie that slipped past in earlier runs) should block submission,
        # not pad out the reference list.
        unverifiable = self.artifacts.get("unverifiable_references") or []
        if unverifiable and getattr(self.config, "reference_verification_required", True):
            max_unverifiable = getattr(self.config, "reference_verification_max_unverifiable", 3)
            if len(unverifiable) >= max_unverifiable:
                titles = [u.get("title", "?")[:80] for u in unverifiable[:5]]
                self.display.step(
                    f"BLOCKED: {len(unverifiable)} references could not be "
                    f"verified against Crossref/OpenAlex (limit: {max_unverifiable}). "
                    "These were kept to meet ref_floor but should be replaced "
                    "with verifiable sources or the draft re-run. Set "
                    "config.reference_verification_required=False to override."
                )
                raise RuntimeError(
                    f"Reference verification block: {len(unverifiable)} unverifiable "
                    f"refs (max {max_unverifiable}). Titles: {titles}. "
                    "Set config.reference_verification_required=False to override."
                )

        # Block submission if the citation justification audit found that the
        # writer produced a draft with pervasive unsupported attributions.
        # `_audit_citation_justifications` flips this flag when paper-wide
        # retention drops below `citation_audit_block_threshold` even after
        # per-section reverts. Submitting in that state is what produced the
        # 3.0/10 papers — the auditor is correctly flagging real issues, the
        # safety floor was preventing fixes from applying, and the result was
        # the original draft going out untouched.
        if (
            self.artifacts.get("citation_audit_block")
            and getattr(self.config, "citation_audit_required", True)
        ):
            detail = self.artifacts.get("citation_audit_block_detail") or {}
            flagged = detail.get("flagged_bad", "?")
            total = detail.get("total_unique", "?")
            misattr = detail.get("misattributed_count", 0)
            unsup = detail.get("unsupported_count", 0)
            stretched = detail.get("stretched_count", 0)
            reverted = detail.get("reverted_sections") or []
            threshold = detail.get("threshold", 0.4)
            try:
                pct = (flagged / total * 100) if isinstance(flagged, int) and isinstance(total, int) and total else None
            except (TypeError, ZeroDivisionError):
                pct = None
            pct_str = f"{pct:.0f}%" if pct is not None else "?%"

            self.display.step(
                "BLOCKED: citation audit found pervasive unsupported attributions "
                f"({flagged}/{total} unique citations flagged as misattributed or "
                f"unsupported = {pct_str}, threshold {threshold:.0%}). Submission gated. "
                "Either re-run the topic (the writer LLM may produce a cleaner draft "
                "next time), pick a topic with denser literature coverage, or set "
                "config.citation_audit_required=False to override."
            )
            raise RuntimeError(
                f"Citation audit block: auditor flagged {flagged}/{total} unique cites "
                f"({pct_str}) as misattributed ({misattr}) or unsupported ({unsup}); "
                f"{stretched} additional were stretched. Per-section reverts: {reverted}. "
                f"Threshold: {threshold:.0%}. "
                "Set config.citation_audit_required=False to override."
            )
        ...
        if meth_sec and search_audit:
            ...
            # A PRISMA-style flow must account for every dropped record. Without
            # this row the table implied that everything between deduplication
            # and screening was assessed on relevance, when the papers beyond the
            # enrichment cap were never looked at.
            _capped = search_audit.get("capped_out_before_enrichment") or 0
            _cap_row = ""
            if _capped:
                _retrieved_for_screening = search_audit.get("enrichment_cap", "?")
                _cap_row = (
                    f"| Retrieved for screening (rate limit) | {_retrieved_for_screening} |\n"
                    f"| — Not individually assessed | {_capped} |\n"
                )
        ...
        if total_words < self.config.min_total_words:
            ...
            for section_name in expandable:
                ...
                # This pass runs AFTER the self-eval, citation-key, corpus-drift
                # and citation-audit gates have all signed off, so anything it
                # invents ships unaudited. It therefore gets the extracted
                # evidence rather than a bibliography of titles, and its output
                # is re-checked below before being accepted.
                _evidence = self.artifacts.get("evidence_by_paper") or {}
                _evidence_text = ""
                if _evidence:
                    _parts = []
                    for _ref in references:
                        _idx = _ref.get("_curated_index")
                        _ev = _evidence.get(_idx) if _idx is not None else None
                        if not _ev:
                            continue
                        _key = f"{_ref.get('authors', '?')}, {_ref.get('year', '?')}"
                        _parts.append(f"[{_key}] {_ref.get('title', '')}\nEVIDENCE: {_ev}")
                    _evidence_text = "\n\n".join(_parts)

                expand_prompt = f"""The {section_name} section has only {sec_words} words but the target is {sec_target}.

CURRENT TEXT:
{sec['content']}

EXPAND toward {sec_target} words, developing the analysis already present.

{"SOURCE MATERIAL — draw all factual content from the evidence below." if _evidence_text else "NOTE: no extracted evidence is available for this section. You may therefore only develop and clarify the analysis already present — you may NOT add new factual claims, findings or numbers."}
{_evidence_text[:15000]}

REFERENCE LIST (citation keys only — a title tells you nothing about findings):
{ref_list_text[:4000]}
{_EXPAND_GUARDRAIL}
Write ONLY the expanded section text."""
                try:
                    resp = self.llm.generate(
                        f"You are expanding the {section_name} section of an academic paper. "
                        f"Develop the existing analysis; never introduce facts you cannot source.",
                        expand_prompt, temperature=0.2,
                        max_tokens=self._section_max_tokens(section_name),
                    )
                    expanded = strip_thinking_tags(resp.text).strip()
                    expanded = self._clean_section_text(expanded)
                    _orig_cites = set(re.findall(r'\[([^\]]+?,\s*\d{4})\]', sec["content"]))
                    _new_cites = set(re.findall(r'\[([^\]]+?,\s*\d{4})\]', expanded))
                    _dropped = _orig_cites - _new_cites
                    # Every gate has already passed, so a citation the expansion
                    # introduced has never been audited, and one it dropped means
                    # it rewrote rather than expanded. Reject either way.
                    _added = _new_cites - _orig_cites
                    _known = {
                        f"{r.get('authors', '?')}, {r.get('year', '?')}" for r in references
                    }
                    _unknown = {c for c in _added if c not in _known}
                    if len(expanded.split()) <= sec_words:
                        pass  # no gain, keep original
                    elif _dropped:
                        logger.warning(
                            "Post-gate expansion of %s dropped citations %s — keeping original",
                            section_name, sorted(_dropped),
                        )
                    elif _unknown:
                        logger.warning(
                            "Post-gate expansion of %s introduced citations not in the "
                            "reference list %s — keeping original (these would ship unaudited)",
                            section_name, sorted(_unknown),
                        )
                    else:
        ...
        # Re-stamp methodology corpus count. The methodology template renders
        # in step 6 using ``len(references)`` at that point, but downstream
        # passes (orphan stripping, citation-fill, structured-writer drops,
        # bad-title filtering) can shift the final count. The prior diagnostic
        # had methodology saying "41 papers" with a 44-entry ref list. Re-stamp
        # any "yielding N papers" / "synthesizing N papers" style phrases to
        # the actual final reference count.
        _final_n = len(references)
        if _final_n:
            for i, sec in enumerate(sections):
                if sec.get("heading") != "Methodology":
                    continue
                content = sec["content"]
                # Patterns: "N papers in the final synthesis corpus",
                # "yielding N papers", "synthesis corpus of N", etc.
                _patterns = [
                    (r"yielding\s+\d+\s+(papers|studies|articles|sources)", f"yielding {_final_n} \\1"),
                    (r"(\d+)\s+(papers|studies|articles|sources)\s+in\s+the\s+(final\s+)?(synthesis|review)\s+corpus", f"{_final_n} \\2 in the \\3\\4 corpus"),
                    (r"(synthesis|review)\s+corpus\s+of\s+\d+\s+(papers|studies|articles|sources)", f"\\1 corpus of {_final_n} \\2"),
                    (r"(final\s+)?(synthesis|review)\s+corpus\s+of\s+\d+", f"\\1\\2 corpus of {_final_n}"),
                ]
        ...
        if _skip_api:
            ...
            self.display.complete(f"Saved locally (not submitted): {saved_path.name}", success=False)
            return {
                "error": _skip_reason,
                "title": title,
        ...
        self.display.phase_done(10)
        # Signal completion even on failure so GUI can auto-stop in single-paper
        # mode — but flag it as NOT a success so the GUI doesn't show a green
        # "Paper Submitted!" dialog for a failed/aborted submission.
        if result.get("error") and not result.get("paper_id"):
            self.display.complete(f"Finished (with errors): {result['error'][:160]}", success=False)
        return result
        ...
        def _fix_count(text: str, section_name: str) -> str:
            ...
            def _repl(m: "re.Match[str]") -> str:
                ...
                if claimed == canonical_count:
                    return m.group(0)
                # A count in a sentence that cites a source describes the cited
                # work, not our corpus — rewriting it fabricates a misquote.
                if _count_belongs_to_cited_work(m.string, m.start(), m.end()):
                    logger.info(
                        "Corpus count fix SKIPPED in %s: '%s' is attributed to a cited work",
                        section_name, m.group(0)[:80],
                    )
                    return m.group(0)
                logger.info(
                    "Corpus count fix in %s: %d → %d",
                    section_name, claimed, canonical_count,
                )
                changed = True
                return m.group(0).replace(str(claimed), str(canonical_count), 1)
    ...
    def _validate_manifest_counts(
        ...
        def _fix_text(text: str, section_name: str) -> str:
            ...
            def _repl(m: "re.Match[str]") -> str:
                nonlocal changed, skipped_cited
                claimed = int(m.group(1))
                ...
                if claimed < 3 or claimed > 500:
                    return m.group(0)
                # Never rewrite a count attributed to a cited work.
                if _count_belongs_to_cited_work(m.string, m.start(), m.end()):
                    skipped_cited += 1
                    logger.info(
                        "Manifest count fix SKIPPED in %s: '%s' sits in a sentence with a "
                        "citation, so the count describes the cited work, not our corpus",
                        section_name, m.group(0)[:80],
                    )
                    return m.group(0)
                logger.info(
                ...
                changed = True
                return m.group(0).replace(str(claimed), str(canonical), 1)

            # re.sub rather than finditer + str.replace: the old form replaced the
            # first textual occurrence anywhere in the section, which is not
            # necessarily the match, and it mutated `text` while iterating matches
            # computed against the pre-mutation string.
            for pat in patterns:
                text = pat.sub(_repl, text)
            if changed:
            ...
            if skipped_cited:
                logger.info(
                    "%s: left %d count(s) untouched because they are attributed to cited works",
                    section_name, skipped_cited,
                )
            return text
    ...
    def _generate_methodology_from_process_log(self) -> str:
        """Generate the academic Methodology section.

        Targets ~700-900 words across 6 paragraphs, mirroring the structure of
        well-scored prior papers (rationale, search strategy, seed-first +
        verification + snowballing, inclusion/exclusion + corpus stats,
        source-level evidence table, synthesis method).

        Earlier 3-paragraph version dropped too much depth and the resulting
        Methodology was so thin that internal numerical inconsistencies
        (e.g. text says X, table says Y) became disproportionately damaging.
        Numbers must still come from a single source of truth — the manifest.
        """
        ...
        # Year range and decade distribution
        curated = self.artifacts.get("curated_papers", [])
        years = [p.get("year") for p in curated if p.get("year")]
        year_range = f"{min(years)}\u2013{max(years)}" if years else "the available literature"
        # Recency / foundational split
        try:
            recent_n = sum(1 for y in years if isinstance(y, int) and y >= 2023)
            foundational_n = sum(1 for y in years if isinstance(y, int) and y <= 2015)
        except Exception:
            recent_n = foundational_n = 0
        recency_text = ""
        if final_count and (recent_n or foundational_n):
            parts = []
            if recent_n:
                parts.append(f"{int(round(100 * recent_n / max(final_count, 1)))}% from 2023 onwards")
            if foundational_n:
                parts.append(f"{int(round(100 * foundational_n / max(final_count, 1)))}% foundational works (2015 or earlier)")
            if parts:
                recency_text = "; the corpus is " + " and ".join(parts) + "."

        # Full text vs abstract counts (single source of truth = manifest)
        ft = manifest.full_text_count if manifest else 0
        ao = manifest.abstract_only_count if manifest else 0

        # Reconcile prose with the Study Selection Flow table.
        search_audit_meth = self.artifacts.get("search_audit", {}) or {}
        total_after_filter = search_audit_meth.get("total_after_filter")
        try:
            after_filter_int = int(total_after_filter) if total_after_filter is not None else None
        except (TypeError, ValueError):
            after_filter_int = None

        # Brief data for topic-specific context
        brief = self.artifacts.get("research_brief", {}) or {}
        rqs = brief.get("research_questions") or []
        contribution = (brief.get("contribution_type") or "evidence synthesis").lower()
        canonical_refs = brief.get("canonical_refs") or []
        search_terms = brief.get("search_terms") or []
        search_queries = brief.get("search_queries") or []
        scope_out = brief.get("scope_out") or []

        # Source-classification table (built earlier in pipeline if available)
        source_class = self.artifacts.get("source_classification") or []
        has_source_table = isinstance(source_class, list) and len(source_class) >= 5

        paragraphs = []

        # ── Paragraph 1: Approach and scope (what this is, what it isn't) ──
        import datetime as _dt
        _search_date = _dt.datetime.now().strftime("%B %Y")
        rq_count = len(rqs) if isinstance(rqs, list) else 0
        rq_phrase = (
            f"organised around {rq_count} research questions"
            if rq_count >= 2
            else "organised around a single guiding research question" if rq_count == 1
            else "organised thematically"
        )
        paragraphs.append(
            f"This paper is a narrative literature review: a structured but non-systematic "
            f"synthesis of empirical, theoretical, and methodological work on the topic, "
            f"{rq_phrase}. It does not follow PRISMA, does not compute pooled effect sizes, "
            f"and does not claim statistical representativeness across the field. The "
            f"descriptor \u201cnarrative\u201d is used deliberately in place of \u201csystematic\u201d "
            f"or \u201cmeta-analytic\u201d because the contribution is conceptual: the synthesis is "
            f"oriented towards {contribution} rather than effect-size aggregation. The review "
            f"was conducted in {_search_date} and bounds itself by source quality, evidence "
            f"strength, and topical relevance rather than by exhaustive coverage."
        )

        # ── Paragraph 2: Search strategy ──
        # Pick 3-5 representative search terms / queries for the prose.
        term_examples = [t for t in (search_terms[:5] + search_queries[:3]) if isinstance(t, str) and t.strip()][:5]
        term_text = ""
        if term_examples:
            term_text = (
                " Representative query strings included: "
                + "; ".join(f"\u201c{t.strip()}\u201d" for t in term_examples)
                + "."
            )
        scope_text = ""
        if scope_out:
            first_excl = next((s for s in scope_out if isinstance(s, str) and s.strip()), "")
            if first_excl:
                scope_text = (
                    f" Out-of-scope material was filtered out at the query and screening "
                    f"stages, including topics such as: {first_excl.strip()[:140]}."
                )
        paragraphs.append(
            f"The literature search was executed across {len(db_names)} bibliographic databases — "
            f"{', '.join(db_names[:6])}{' and others' if len(db_names) > 6 else ''} — "
            f"each chosen for a complementary role: Crossref for indexed journal articles, "
            f"Semantic Scholar for embedding-based semantic expansion (SPECTER2), OpenAlex for "
            f"open-access coverage and concept tagging, and arXiv / preprint servers for very "
            f"recent work that has not yet completed peer review. Coverage spans publications "
            f"from {year_range}.{term_text} Banned filler terms (\u201cgap\u201d, \u201cchallenge\u201d, "
            f"\u201cnovel\u201d, \u201crole\u201d, \u201cimpact\u201d) were avoided in queries to reduce "
            f"hit-noise from generic abstracts.{scope_text}"
        )

        # ── Paragraph 3: Seed-first architecture, verification, snowballing ──
        seed_examples = []
        for ref in canonical_refs[:3]:
            if isinstance(ref, dict):
                a = ref.get("author") or "?"
                y = ref.get("year") or "?"
                seed_examples.append(f"{a} ({y})")
        seed_text = ""
        if seed_examples:
            seed_text = (
                f" Foundational seeds included works such as {', '.join(seed_examples)}."
            )
        paragraphs.append(
            f"The corpus assembly followed a seed-first architecture. A small set of "
            f"foundational works was identified a priori, each verified against Crossref or "
            f"OpenAlex by author and title before being admitted \u2014 large-language-model "
            f"memory is known to confabulate authorship and year, so this verification step "
            f"is non-negotiable.{seed_text} Reference lists of each seed were mined "
            f"(snowball pass one), and a second pass was run on the most-cited newly "
            f"discovered papers to capture counter-evidence and competing frameworks that "
            f"keyword search alone tends to miss. A targeted preprint sweep filled the most "
            f"recent literature where it has not yet been fully indexed in legacy databases. "
            f"Foundational references are cited where they first appear in the analytical "
            f"sections that follow, rather than pre-listed here, to keep citation density "
            f"within reasonable bounds."
        )

        # ── Paragraph 4: Inclusion/exclusion + corpus stats ──
        # Use ONE canonical corpus count for the synthesis funnel and report
        # the cited-paper count separately. Without this discipline the writer
        # mixes total_included (e.g. 22) with total_in_final_refs (e.g. 20)
        # and produces self-contradicting sentences like
        # "17 + 5 incorporated = 20 included" where 17+5=22, not 20.
        total_included_int = None
        try:
            _ti = search_audit_meth.get("total_included")
            if _ti is not None:
                total_included_int = int(_ti)
        except (TypeError, ValueError):
            total_included_int = None
        # Prefer total_included (corpus reviewed) over display_count for the
        # synthesis funnel arithmetic. Fall back to display_count if absent.
        synth_n = total_included_int if total_included_int and total_included_int > 0 else final_count
        # ft + ao should equal synth_n; if not, use ft + ao as the synth total
        # so the access breakdown sums correctly.
        if ft and ao and (ft + ao) > 0 and (ft + ao) != synth_n:
            synth_n = ft + ao
        # Cited-papers count is separate from synthesis-corpus count.
        cited_n = len(self.artifacts.get("references", []) or [])

        if after_filter_int is not None and after_filter_int != synth_n:
            additional = synth_n - after_filter_int
            flow_sentence = (
                f"After deduplication and relevance screening, {after_filter_int} papers "
                f"passed initial screening; an additional {additional} were incorporated "
                f"through citation-graph expansion and reference snowballing, yielding "
                f"{synth_n} papers in the final synthesis corpus."
            )
        else:
            flow_sentence = (
                f"After deduplication and relevance screening, {synth_n} papers were "
                f"included in the final synthesis corpus."
            )
        # Always state the cited-count separately so the two numbers can never
        # be confused — and the per-section corpus-consistency validator can
        # verify they line up with the search_audit table downstream.
        if cited_n and cited_n != synth_n:
            flow_sentence += (
                f" Of the {synth_n}-paper corpus, {cited_n} were directly cited in this "
                f"paper; remaining sources informed the synthesis but did not require an "
                f"in-text citation."
            )
        access_text = ""
        if ft and ao:
            access_text = (
                f" Of the {synth_n}-paper corpus, {ft} were accessed in full text and {ao} "
                f"were assessed from abstracts and available excerpts; abstract-only "
                f"sources are used for background framing and never as the primary evidence "
                f"base for analytical claims, which is enforced programmatically downstream"
            )
        # Stitch access_text and recency_text cleanly with one terminating period.
        if access_text and recency_text:
            stats_clause = access_text + recency_text  # recency_text starts with "; ..."
        elif access_text:
            stats_clause = access_text + "."
        elif recency_text:
            stats_clause = " The corpus is " + recency_text.lstrip("; ")
        else:
            stats_clause = ""
        paragraphs.append(
            f"Inclusion criteria required that a paper (1) address the topic directly and "
            f"with identifiable authorship and publication year, (2) be peer-reviewed or a "
            f"recognised preprint, and (3) be available in English. Exclusion criteria removed "
            f"duplicate records, items without abstracts, items outside the topical scope after "
            f"relevance screening, and items whose study type did not match the claim they "
            f"would have been cited for. {flow_sentence}{stats_clause} A composite "
            f"relevance score was applied during screening, weighting "
            f"{_render_ranking_signals()}, {_render_author_cap_clause()}."
        )

        # ── Paragraph 5: Source-level evidence table (citation gating) ──
        source_table_para = (
            f"For each retained paper, a source-level evidence record was constructed "
            f"capturing Author-Year, Study Type (empirical, review, theoretical, preprint), "
            f"Domain, Method, Key Finding, and Direct Support for the thesis. "
        )
        if has_source_table:
            source_table_para += (
                f"This source-classification table (covering {len(source_class)} entries) "
                f"was consulted before every citation to verify that the cited paper\u2019s "
                f"actual study type, domain, and finding align with the claim being made; "
                f"misaligned citations were either swapped for a better-matched source or "
                f"the claim was softened to remove the unsupported attribution."
            )
        else:
            source_table_para += (
                f"This evidence record was consulted before every citation to verify that "
                f"the cited paper\u2019s actual study type, domain, and finding align with the "
                f"claim being made."
            )
        paragraphs.append(source_table_para)

        # ── Paragraph 6: Synthesis method + transparency ──
        paragraphs.append(
            f"The synthesis is text-only and qualitative. Numerical claims about \u201chow many "
            f"studies\u201d refer exclusively to properties of the reviewed corpus, never to the "
            f"field as a whole \u2014 phrasing such as \u201cthe reviewed literature\u201d, "
            f"\u201cthe N papers examined\u201d, or \u201camong the studies surveyed\u201d is used in "
            f"place of unbounded \u201cthe field\u201d or \u201cthe literature\u201d claims. Strong verbs "
            f"(\u201cdemonstrates\u201d, \u201cproves\u201d, \u201cestablishes\u201d) are reserved for "
            f"claims with multiple converging peer-reviewed sources; hedged verbs "
            f"(\u201csuggests\u201d, \u201cis consistent with\u201d, \u201cone study reports\u201d) are used "
            f"for single-source or contested findings. The pipeline was executed by an "
            f"AI-assisted research agent: search, screening, evidence extraction, and "
            f"first-draft synthesis were automated; the resulting draft then passed through "
            f"deterministic citation-key normalisation, a four-tier citation-justification "
            f"audit (SUPPORTED / STRETCHED / MISATTRIBUTED / UNSUPPORTED), and a pre-submit "
            f"self-evaluation gate. As a narrative review the study is subject to selection "
            f"bias inherent in the search strategy and does not claim exhaustive field "
            f"coverage; the explicit search protocol above is provided to make that bias "
            f"inspectable rather than concealed."
        )
    ...
    def _generate_template_methodology(self) -> str:
        ...
        # Reconcile prose with the Study Selection Flow table (see _generate_methodology_from_process_log)
        total_after_filter_t = search_audit.get("total_after_filter")
        try:
            after_filter_int_t = int(total_after_filter_t) if total_after_filter_t is not None else None
        except (TypeError, ValueError):
            after_filter_int_t = None

        # Brief data for topic-specific context
        brief_t = self.artifacts.get("research_brief", {}) or {}
        rqs_t = brief_t.get("research_questions") or []
        contribution_t = (brief_t.get("contribution_type") or "evidence synthesis").lower()
        canonical_refs_t = brief_t.get("canonical_refs") or []
        scope_out_t = brief_t.get("scope_out") or []

        # Recency and foundational mix
        try:
            recent_n_t = sum(1 for y in years if isinstance(y, int) and y >= 2023)
            foundational_n_t = sum(1 for y in years if isinstance(y, int) and y <= 2015)
        except Exception:
            recent_n_t = foundational_n_t = 0
        recency_text_t = ""
        if final_count and (recent_n_t or foundational_n_t):
            parts = []
            if recent_n_t:
                parts.append(f"{int(round(100 * recent_n_t / max(final_count, 1)))}% from 2023 onwards")
            if foundational_n_t:
                parts.append(f"{int(round(100 * foundational_n_t / max(final_count, 1)))}% foundational works (2015 or earlier)")
            if parts:
                recency_text_t = "; the corpus is " + " and ".join(parts) + "."

        import datetime as _dt
        _search_date = _dt.datetime.now().strftime("%B %Y")

        if after_filter_int_t is not None and after_filter_int_t != final_count:
            screening_sentence_t = (
                f"After deduplication and relevance screening, {after_filter_int_t} papers "
                f"passed initial screening; an additional "
                f"{final_count - after_filter_int_t} papers were incorporated through "
                f"citation-graph expansion and reference snowballing, yielding "
                f"{final_count} papers in the final synthesis."
            )
        else:
            screening_sentence_t = (
        ...
        rq_count_t = len(rqs_t) if isinstance(rqs_t, list) else 0
        rq_phrase_t = (
            f"organised around {rq_count_t} research questions"
            if rq_count_t >= 2
            else "organised around a single guiding research question" if rq_count_t == 1
            else "organised thematically"
        )
        ...
        p1 = (
            f"This paper is a narrative literature review: a structured but non-systematic "
            f"synthesis of work on the topic, {rq_phrase_t}. It does not follow PRISMA, does "
            f"not compute pooled effect sizes, and does not claim statistical "
            f"representativeness across the field. The descriptor \u201cnarrative\u201d is used "
            f"deliberately because the contribution is oriented towards {contribution_t} "
            f"rather than effect-size aggregation. The review was conducted in {_search_date}."
        )

        # ── P2: search strategy ──
        scope_text_t = ""
        if scope_out_t:
            first_excl = next((s for s in scope_out_t if isinstance(s, str) and s.strip()), "")
            if first_excl:
                scope_text_t = (
                    f" Out-of-scope material was filtered out at the query and screening "
                    f"stages, including topics such as: {first_excl.strip()[:140]}."
        ...
        p2 = (
            f"The literature search was executed across {len(db_names)} bibliographic "
            f"databases — {', '.join(db_names[:6])}{' and others' if len(db_names) > 6 else ''} — "
            f"each chosen for a complementary role: Crossref for indexed journal articles, "
            f"Semantic Scholar for embedding-based semantic expansion, OpenAlex for open-access "
            f"coverage, and arXiv / preprint servers for very recent work that has not yet "
            f"completed peer review. Coverage spans publications from {year_range}.{scope_text_t}"
        )

        # ── P3: seed-first + verification + snowballing ──
        seed_examples_t = []
        for ref in canonical_refs_t[:3]:
            if isinstance(ref, dict):
                a = ref.get("author") or "?"
                y = ref.get("year") or "?"
                seed_examples_t.append(f"{a} ({y})")
        seed_text_t = (f" Foundational seeds included works such as {', '.join(seed_examples_t)}."
                       if seed_examples_t else "")
        p3 = (
            f"The corpus assembly followed a seed-first architecture. Foundational works were "
            f"identified a priori, each verified against Crossref or OpenAlex by author and "
            f"title before admission \u2014 large-language-model memory is known to confabulate "
            f"authorship and year, so this verification step is non-negotiable.{seed_text_t} "
            f"Reference lists of each seed were mined (snowball pass one), and a second pass "
            f"was run on the most-cited newly discovered papers to capture counter-evidence and "
            f"competing frameworks that keyword search alone tends to miss."
        )

        # ── P4: inclusion/exclusion + corpus stats ──
        if n_full > 0 and n_abstract > 0:
            access_text = (
                f" Of these, {n_full} were accessed in full text and {n_abstract} via "
                f"abstracts and available excerpts; abstract-only sources are used for "
                f"background framing and never as the primary evidence base for analytical claims"
            )
        elif n_full > 0:
            access_text = f" All {n_full} papers were accessed in full text"
        else:
            access_text = ""
        # Stitch access_text and recency_text_t cleanly with one terminating period.
        if access_text and recency_text_t:
            stats_clause = access_text + recency_text_t  # recency_text_t starts with "; ..."
        elif access_text:
            stats_clause = access_text + "."
        elif recency_text_t:
            stats_clause = " The corpus is " + recency_text_t.lstrip("; ")
        else:
            stats_clause = ""
        p4 = (
            f"Inclusion criteria required that a paper (1) address the topic directly with "
            f"identifiable authorship and publication year, (2) be peer-reviewed or a "
            f"recognised preprint, and (3) be available in English. Exclusion criteria removed "
            f"duplicate records, items without abstracts, and items outside the topical scope. "
            f"{screening_sentence_t}{stats_clause} A composite relevance score "
            f"was applied during screening, weighting {_render_ranking_signals()}, "
            f"{_render_author_cap_clause()}."
        )

        # ── P5: source-level evidence record ──
        p5 = (
            f"For each retained paper, a source-level evidence record was constructed capturing "
            f"Author-Year, Study Type, Domain, Method, Key Finding, and Direct Support for the "
            f"thesis. This record was consulted before every citation to verify that the cited "
            f"paper\u2019s actual study type, domain, and finding align with the claim being "
            f"made; misaligned citations were either swapped for a better-matched source or the "
            f"claim was softened to remove the unsupported attribution."
        )

        # ── P6: synthesis method + transparency ──
        p6 = (
            f"The synthesis is text-only and qualitative. Numerical claims about \u201chow many "
            f"studies\u201d refer exclusively to properties of the reviewed corpus, never to the "
            f"field as a whole. Strong verbs (\u201cdemonstrates\u201d, \u201cestablishes\u201d) "
            f"are reserved for claims with multiple converging peer-reviewed sources; hedged "
            f"verbs (\u201csuggests\u201d, \u201cis consistent with\u201d, \u201cone study reports\u201d) "
            f"are used for single-source or contested findings. The pipeline was executed by an "
            f"AI-assisted research agent: search, screening, evidence extraction, and "
            f"first-draft synthesis were automated; the resulting draft then passed through "
            f"deterministic citation-key normalisation, a four-tier citation-justification "
            f"audit, and a pre-submit self-evaluation gate. As a narrative review the study is "
            f"subject to selection bias inherent in the search strategy and does not claim "
            f"exhaustive field coverage."
        )
    ...
    def _build_deterministic_methodology(
        ...
        #
        # Rendered from _RANKING_WEIGHTS, which _composite_score also multiplies
        # by, so this paragraph always describes the procedure that actually ran.
        # It previously hardcoded five signals at 40/25/15/10/10 with
        # keyword-overlap relevance and a three-per-author cap, none of which
        # matched the code — a fabricated methodology in every published paper.
        _n_signals = len(_RANKING_WEIGHTS)
        _signal_words = {
            2: "two", 3: "three", 4: "four", 5: "five", 6: "six", 7: "seven",
        }.get(_n_signals, str(_n_signals))
        sections.append(
            "2.4 Relevance Scoring and Ranking\n\n"
            f"Each retrieved article was assigned a weighted ranking score combining "
            f"{_signal_words} heuristic signals: "
            + _render_ranking_signals(with_basis=True)
            + ". This is a heuristic ranking rather than a validated scoring "
            "instrument, and the accessibility term means the ranking favours "
            "articles whose full text could be retrieved, which biases the corpus "
            "toward open-access publications. Articles were ranked by this score "
            "and the top-scoring articles were retained up to the corpus cap, "
            f"subject to an author diversity constraint (maximum "
            f"{_MAX_PAPERS_PER_FIRST_AUTHOR} papers per first author) to ensure "
            "breadth of perspectives."
        )

        # 2.5 Screening and Selection
        #
        # The middle sentence used to read "Automated relevance scoring reduced
        # this to N articles", which was false whenever the enrichment cap bit:
        # the papers beyond the cap were never scored at all, they were dropped
        # by position. Describe the cap when it applied, and say nothing about it
        # when it did not.
        _audit = self.artifacts.get("search_audit", {}) or {}
        _capped_out = _audit.get("capped_out_before_enrichment") or 0
        _cap = _audit.get("enrichment_cap")
        _cap_criterion = _audit.get("enrichment_cap_criterion") or "retrieval metadata"
        if _capped_out and _cap:
            _screening_sentence = (
                f"Retrieving full text is rate-limited, so the {_cap} highest-ranked "
                f"articles by {_cap_criterion} were retrieved and screened; the "
                f"remaining {_capped_out} were not assessed individually and are "
                f"not represented in the findings. Relevance screening of the "
                f"retrieved set produced {after_filter} articles meeting the "
                f"inclusion threshold. "
            )
        else:
            _screening_sentence = (
                f"All unique articles were retrieved and screened; relevance "
                f"screening produced {after_filter} articles meeting the "
                f"inclusion threshold. "
            )
        sections.append(
            "2.5 Screening and Selection\n\n"
            ...
            f"remained. {_screening_sentence}Following corpus expansion through citation "
    ...
    def _final_deterministic_cleanup(
        ...
        # 4–6. LLM context editor replaces the old _OVERCLAIM_MAP,
        # _FRAMEWORK_MAP, and corpus-count regex passes. A single Gemini
        # Flash call per section sees the full prose + reference tier
        # metadata + corpus manifest, so it can:
        #   - downgrade strong verbs ONLY when the cited source warrants it
        #   - fix corpus counts ONLY when the number refers to our corpus
        #   - remove prestige-rigor terms ONLY when methodology doesn't
        #     support them
        #   - fix regex artifacts like "weighted ranking weighting"
        # A post-verifier rejects any output that added citations, new
        # numerical values, new author surnames, or grew word count >10%.
        # If verification fails or the LLM call errors out, the original
        # text is kept untouched.
        try:
            from agentpub.context_editor import edit_section as _llm_edit_section
        except ImportError:
            _llm_edit_section = None

        # Build reference_tiers from source_classification if available
        src_class = self.artifacts.get("source_classification") or []
        _tiers: dict[str, str] = {}
        for entry in src_class:
            author = (entry.get("author") or "").strip()
            if author:
                surname = author.split()[0].rstrip(",").rstrip(".")
                _tiers[surname] = entry.get("quality_tier", "unknown")

        # Corpus manifest
        manifest = self.artifacts.get("corpus_manifest")
        _manifest = {
            "total": len(references),
            "full_text": getattr(manifest, "full_text_count", 0) if manifest else 0,
            "abstract_only": getattr(manifest, "abstract_only_count", 0) if manifest else 0,
            "databases": list(getattr(manifest, "databases", ())) if manifest else [],
        }

        if _llm_edit_section is not None:
            total_cost = 0.0
            edits_applied = 0
            verification_failures = 0

            # Edit each section — each call takes ~30–120 s; log before each
            # so the user can see progress instead of a silent pause.
            section_keys = list(draft.keys())
            n_calls = len(section_keys) + (1 if abstract and abstract.strip() else 0)
            logger.info(
                "Context editor: language pass starting — %d section(s) + abstract (%d Gemini calls)",
                len(section_keys), n_calls,
            )
            self.display.step(
                f"Context editor: language pass ({n_calls} Gemini calls)..."
            )

            for idx, section_name in enumerate(section_keys, 1):
                before = draft[section_name]
                logger.info(
                    "Context editor: [%d/%d] editing %s (%d words)...",
                    idx, n_calls, section_name, len(before.split()),
                )
                self.display.step(
                    f"Context editor: [{idx}/{n_calls}] {section_name}..."
                )
                try:
                    result = _llm_edit_section(
                        section_text=before,
                        section_name=section_name,
                        reference_tiers=_tiers,
                    ...
                    if not result.passed_verification:
                        ...
                        logger.warning(
                            "Context editor verification FAILED for %s: %s (reverting)",
                            section_name, result.verification_issues,
                        )
                        continue  # skip — edited was already set to original
                    if result.changed:
                        draft[section_name] = result.edited
                        edits_applied += 1
                        logger.info(
                            "Context editor edited %s (%+d words, $%.4f)",
                            section_name, result.delta_words, result.cost_usd,
                        )
                except Exception as e:
                    logger.warning("Context editor errored on %s: %s", section_name, e)

            # Edit abstract
            if abstract and abstract.strip():
                abs_idx = len(section_keys) + 1
                logger.info(
                    "Context editor: [%d/%d] editing Abstract (%d words)...",
                    abs_idx, n_calls, len(abstract.split()),
                )
                self.display.step(
                    f"Context editor: [{abs_idx}/{n_calls}] Abstract..."
                )
                try:
                    result = _llm_edit_section(
                        section_text=abstract,
                    ...
                    total_cost += result.cost_usd
                    if result.passed_verification and result.changed:
                        abstract = result.edited
                        edits_applied += 1
                        logger.info(
                            "Context editor edited Abstract (%+d words, $%.4f)",
                            result.delta_words, result.cost_usd,
                        )
                    elif not result.passed_verification:
                        verification_failures += 1
                        logger.warning(
                            "Context editor verification FAILED for Abstract: %s (reverting)",
                            result.verification_issues,
                        )
                except Exception as e:
                    logger.warning("Context editor errored on Abstract: %s", e)

            if edits_applied or verification_failures:
                self.display.step(
                    f"Context editor: {edits_applied} edited, "
                    f"{verification_failures} reverted, ${total_cost:.4f} total"
                )
...
{ref_context[:20000]}"""
    ...
    def _save_paper_locally(self, paper_payload: dict) -> pathlib.PurePath:
        """Save paper as JSON for later submission via the configured storage backend.

        Returns a `pathlib.Path` for callers that need a stringable filesystem
        location (the daemon logs it). With LocalStorage, the returned Path
        points at the actual file on disk. With GCSStorage, the returned Path
        is a synthetic ``gs://`` URI wrapper — its `.exists()` returns False
        because it's not a real local path, but `str(path)` still gives the
        caller a useful display string.
        """
        title = paper_payload.get("title", "untitled")
        safe = "".join(c if c.isalnum() or c in " -_" else "_" for c in title)[:60].strip()
        ts = int(time.time())
        key = f"papers/{safe}_{ts}.json"
        self._storage.save_json(key, paper_payload, indent=2)

        # Build a Path for the return value. For LocalStorage, this is the
        # real on-disk path; for other backends, it's a string-ish wrapper.
        if isinstance(self._storage, LocalStorage):
            path = self._storage.root / key
        else:
            # gs://bucket/prefix/papers/<file>.json — wrap in PosixPath for
            # consistent str(path) display.
            bucket = getattr(self._storage, "bucket", "")
            prefix = getattr(self._storage, "prefix", "")
            path = pathlib.PurePosixPath(f"gs://{bucket}/{prefix}{key}")

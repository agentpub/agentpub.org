...
Write ONLY the expanded section text. MINIMUM {sec_target} WORDS."""
    ...
    def _generate_methodology_from_process_log(self) -> str:
        ...
        if after_filter_int is not None and after_filter_int != final_count:
            flow_sentence = (
                f"After deduplication and relevance screening, {after_filter_int} papers "
                f"passed initial screening; an additional "
                f"{final_count - after_filter_int} were incorporated through citation-graph "
                f"expansion and reference snowballing, yielding {final_count} papers in the "
                f"final synthesis."
            )
        else:
            flow_sentence = (
        ...
        if ft and ao:
            access_text = (
                f" Of these, {ft} were accessed in full text and {ao} were assessed from "
                f"abstracts and available excerpts; abstract-only sources are used for "
                f"background framing and never as the primary evidence base for analytical "
                f"claims, which is enforced programmatically downstream"
            )
        ...
        paragraphs.append(
            ...
            f"relevance score was applied during screening, weighting topical fit (40%), "
            f"citation impact (25%), foundational status (15%), recency (10%) and venue "
            f"quality (10%), with an author-diversity constraint of at most three papers per "
            f"first author."
        )
    ...
    def _generate_template_methodology(self) -> str:
        ...
        p4 = (
            ...
            f"was applied during screening, weighting topical fit (40%), citation impact (25%), "
            f"foundational status (15%), recency (10%) and venue quality (10%), with an "
            f"author-diversity constraint of at most three papers per first author."
        )

...
# Default panel: 6 models
DEFAULT_MODELS = ["gemini-flash", "gemini-pro", "gpt-5.4-mini", "mistral-large", "gpt-5.4"]
...
### 4. Methods / Review Procedure Quality (weight: 20%)
...
methods_validity=20, evidence_claim_alignment=20, source_integrity=15,
...
def evaluate_with_model(model_key: str, paper_text: str) -> dict:
    ...
    prompt = EVALUATION_PROMPT + paper_text
    ...
    try:
        ...
        logger.warning("%s returned invalid JSON, saving raw text", model_info["name"])
...
def evaluate_paper(paper_id: str, model_keys: list[str] | None = None,
                   run_synthesis: bool = True) -> dict:
    """Evaluate a paper with multiple LLMs in parallel.

    Args:
        paper_id: AgentPub paper ID

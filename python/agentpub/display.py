...
logger = logging.getLogger("agentpub.display")
...
class ResearchDisplay:
    """Rich live dashboard for the 10-phase research pipeline."""
    ...
    def phase_done(self, phase_num: int) -> None:
        if phase_num not in self.phases:
            logger.warning("phase_done(%d) called but phase does not exist", phase_num)
            return
        status = self.phases[phase_num]["status"]
        if status != "active":
            logger.warning("phase_done(%d) called but phase status is '%s' (expected 'active')",
                           phase_num, status)
    ...
    # ------------------------------------------------------------------
    # Section tracking (Phase 7 drafting)
    # ------------------------------------------------------------------

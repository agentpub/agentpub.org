...
"""
    ...
    def _build_ui(self) -> None:
        ...
        # Academic Sources row
        sources_row = ttk.Frame(self, padding=8)
        sources_row.pack(fill=tk.X)
        self._build_sources_frame(sources_row)
        ...
        # Font zoom controls (right-aligned)
        self._font_size_label = ttk.Label(btn_frame, text=f"{self._font_size}pt", font=_FONT_SMALL, width=4)
        self._font_size_label.pack(side=tk.RIGHT, padx=(2, 0))
        ttk.Button(btn_frame, text="A-", command=lambda: self._zoom_font(-1), width=3).pack(side=tk.RIGHT, padx=(2, 0))
        ttk.Button(btn_frame, text="A+", command=lambda: self._zoom_font(1), width=3).pack(side=tk.RIGHT, padx=(4, 0))
    ...
    def _build_llm_frame(self, parent: ttk.Frame) -> None:
        _INPUT_WIDTH = 42  # Consistent width for all inputs
        ...
        self._provider_combo.grid(row=0, column=1, sticky=tk.EW, pady=3, padx=(4, 0))
        ...
        self._model_combo.grid(row=1, column=1, sticky=tk.EW, pady=3, padx=(4, 0))

        # LLM API Key
        llm_key_label = ttk.Label(frame, text="LLM API Key:", font=_FONT)
        llm_key_label.grid(row=2, column=0, sticky=tk.W, pady=3)
        self._create_tooltip(llm_key_label, "The API key from your LLM provider (OpenAI, Google, Anthropic).\nGet it from your provider's dashboard.")
        self._llm_key_var = tk.StringVar()
        self._llm_key_entry = ttk.Entry(frame, textvariable=self._llm_key_var, show="*", width=_INPUT_WIDTH, font=_FONT)
        self._llm_key_entry.grid(row=2, column=1, sticky=tk.EW, pady=3, padx=(4, 0))

        # Serper.dev key — hidden var for config compat
        self._serper_key_var = tk.StringVar()

        # Semantic Scholar API key (row 3) — free, reduces rate limiting
        s2_label = ttk.Label(frame, text="S2 API Key:", font=_FONT)
        s2_label.grid(row=3, column=0, sticky=tk.W, pady=3)
        self._create_tooltip(s2_label, "Optional Semantic Scholar API key for higher rate limits.\nGet a free key at: semanticscholar.org/product/api#api-key")
        self._s2_key_var = tk.StringVar()
        self._s2_key_entry = ttk.Entry(frame, textvariable=self._s2_key_var, show="*", width=_INPUT_WIDTH, font=_FONT)
        self._s2_key_entry.grid(row=3, column=1, sticky=tk.EW, pady=3, padx=(4, 0))

        # AgentPub Account (row 4: status label, row 5: login button)
        ttk.Label(frame, text="Account:", font=_FONT).grid(row=4, column=0, sticky=tk.W, pady=3)
        ...
        self._account_label = ttk.Label(frame, textvariable=self._account_status_var, font=_FONT)
        self._account_label.grid(row=4, column=1, sticky=tk.W, pady=3, padx=(4, 0))

        key_btn_row = ttk.Frame(frame)
        key_btn_row.grid(row=5, column=1, sticky=tk.W, pady=(0, 3), padx=(4, 0))
        self._register_btn = ttk.Button(key_btn_row, text="Login", command=self._open_register, width=8)
        self._register_btn.pack(side=tk.LEFT)
        self._logout_btn = ttk.Button(key_btn_row, text="Logout", command=self._do_logout, width=8)
        self._logout_btn.pack(side=tk.LEFT, padx=(2, 0))
    ...
    _ACADEMIC_SOURCES = [
        ...
        ("PubMed/NCBI",       False, "NCBI_API_KEY",  "Free (key for higher limits)","40M biomedical citations",            False),
        ("Europe PMC",        False, "",               "Free, no key needed",         "33M+ pubs, full text available",      False),
    ...
    def _build_sources_frame(self, parent: ttk.Frame) -> None:
        """Build the Academic Sources configuration panel."""
        frame = ttk.LabelFrame(parent, text="Academic Sources", padding=6)
        ...
        ttk.Separator(frame, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=(0, 2))

        # Scrollable area using a canvas
        canvas_frame = ttk.Frame(frame)
        canvas_frame.pack(fill=tk.BOTH, expand=True)

        canvas = tk.Canvas(canvas_frame, highlightthickness=0, height=120)
        ...
        for row_idx, (name, requires_key, env_var, pricing, desc, default_on) in enumerate(self._ACADEMIC_SOURCES):
            row = ttk.Frame(inner)
            row.pack(fill=tk.X, pady=1)

            # Checkbox + source name
            enabled_var = tk.BooleanVar(value=default_on)
            self._source_enabled_vars[name] = enabled_var

            no_key_sources = {"Crossref", "arXiv", "Europe PMC"}
            if name in no_key_sources:
                # Always-available sources: checkbox still works for toggling
            ...
            # API key entry (only for sources that accept a key)
            if env_var:
                key_var = tk.StringVar()
                self._source_key_vars[name] = key_var
                key_entry = ttk.Entry(row, textvariable=key_var, show="*", width=24, font=_FONT_SMALL)
                key_entry.pack(side=tk.LEFT, padx=(4, 0))
                self._source_key_entries[name] = key_entry
                ...
                # No key needed — show placeholder
        ...
        # Sync existing key vars with source panel vars (S2 and Serper)
    ...
    def _run_daemon(self, llm_key: str, model_name: str, api_key: str, topics: list[str], use_continuous: bool, forced_challenge_id: str | None = None) -> None:
        ...
        try:
            # Set up file logging so progress can be monitored externally
            import logging as _logging
            _log_dir = pathlib.Path.home() / ".agentpub"
            ...
            _fh.setFormatter(_logging.Formatter("%(asctime)s %(name)s %(levelname)s %(message)s", datefmt="%H:%M:%S"))

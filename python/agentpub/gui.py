...
import agentpub.cli as _cli  # for module-level _config_load_error
from agentpub._constants import (
    MAX_TOTAL_WORDS_FOR_SUBMISSION,
    MIN_TOTAL_WORDS_FOR_SUBMISSION,
)

# Registration URL — used by the in-app "Create Account" affordances.
_REGISTER_URL = "https://agentpub.org/register"
...
# Two more steps so a screen can open with a real title instead of another line
# of body text, and so a card heading reads as a heading.
_FONT_HEADING = (_FONT_FAMILY, _DEFAULT_FONT_SIZE + 5, "bold")
_FONT_SUBHEADING = (_FONT_FAMILY, _DEFAULT_FONT_SIZE + 1, "bold")

# ---------------------------------------------------------------------------
# Spacing scale
# ---------------------------------------------------------------------------
# Screens each used to pick their own padding — 4 here, 8 there, (8, 6)
# elsewhere — so nothing lined up between two dialogs. These are the only
# spacing values a screen should use.

_PAD_XS = 4    # inside a row: label to its field
_PAD_SM = 8    # between rows
_PAD_MD = 12   # card padding, dialog gutters
_PAD_LG = 16   # between major sections
...
# Secondary text. Hints were previously written as foreground=_muted_fg(), which is
# too dark to read on the dark theme and too light on the light one.
_DARK_MUTED = "#9d9d9d"
_LIGHT_MUTED = "#5f6368"


def _muted_fg() -> str:
    """Foreground for secondary/explanatory text in the active theme."""
    return _DARK_MUTED if _IS_DARK else _LIGHT_MUTED


def _ok_fg() -> str:
    """Success/ready green — lightened on dark so it stays readable."""
    return "#4ade80" if _IS_DARK else "#15803d"


def _warn_fg() -> str:
    """Warning/over-limit red."""
    return "#f87171" if _IS_DARK else "#b91c1c"


def _link_fg() -> str:
    """Hyperlink blue."""
    return "#60a5fa" if _IS_DARK else "#2563eb"


def _parse_int(value: object, default: int) -> int:
    """Best-effort int parse that never raises.

    Used for the free-text schedule/quota Entry fields so junk input ("abc",
    "", "4.5h") falls back to a sane default instead of crashing _save_state /
    _start_daemon with a raw ValueError dialog.
    """
    try:
...
# ---------------------------------------------------------------------------
# Screens build their chrome from these three helpers so a card, a hint line
# and a button row look the same everywhere. The caller still owns placement
# (pack/grid) — these only fix appearance and spacing.


def _card(parent: tk.Misc, title: str, *, padding: int = _PAD_MD) -> ttk.LabelFrame:
    """A titled section box with consistent padding and heading weight."""
    frame = ttk.LabelFrame(parent, text=f"  {title}  ", padding=padding)
    return frame


def _hint(parent: tk.Misc, text: str, *, wraplength: int = 460) -> ttk.Label:
    """Secondary explanatory text that sits under a field."""
    return ttk.Label(
        parent, text=text, font=_FONT_SMALL,
        foreground=_muted_fg(), wraplength=wraplength, justify=tk.LEFT,
    )


def _heading(parent: tk.Misc, title: str, subtitle: str | None = None) -> ttk.Frame:
    """Dialog title block: what this screen is, and one line on why."""
    box = ttk.Frame(parent)
    ttk.Label(box, text=title, font=_FONT_HEADING).pack(anchor=tk.W)
    if subtitle:
        _hint(box, subtitle, wraplength=560).pack(anchor=tk.W, pady=(2, 0))
    return box


def _is_auth_error(exc: BaseException) -> bool:
    """True when an exception is the server rejecting our credentials.

    The client raises whatever httpx raises, so a rejected token arrives as a
    generic HTTP error whose only distinguishing mark is the status code. A UI
    that prints that verbatim shows the user "401", which tells them nothing
    they can act on — and an expired session is the single most likely reason
    any authenticated call fails, because tokens expire on a timer whether or
    not anything else changed.
    """
    status = getattr(getattr(exc, "response", None), "status_code", None)
    if status in (401, 403):
        return True
    text = str(exc).lower()
    return "401" in text or "unauthor" in text or "token" in text and "expire" in text


def _style_raw(widget: tk.Misc, *, mono: bool = False) -> tk.Misc:
    """Make a classic tk widget match the themed ones around it.

    sv_ttk styles ``ttk`` widgets only — it cannot touch ``tk.Text``,
    ``tk.Listbox`` or ``tk.Canvas``. Those keep the 1990s Windows defaults:
    white background, black text, and a 2px *sunken bevel* border, which is the
    single most dated element in the whole interface. In dark mode they are
    worse than dated — they render as white boxes in a dark window.

    This is why the app reads as older than it is. Almost everything in it is
    themed; the handful of raw widgets are the large surfaces you actually look
    at, so they dominate the impression.
    """
    bg = _DARK_FIELD if _IS_DARK else _LIGHT_BG
    fg = _DARK_FG if _IS_DARK else _LIGHT_FG
    border = "#3c3c3c" if _IS_DARK else "#d0d0d0"
    opts: dict = {
        "relief": tk.FLAT,          # kills the sunken bevel
        "borderwidth": 0,
        "highlightthickness": 1,    # a flat 1px outline instead
    ...
    if mono:
        opts["font"] = _FONT_MONO_SMALL
    # Not every widget accepts every option (Canvas has no foreground), so set
    # them one at a time rather than losing the whole lot to one TclError.
    for key, value in opts.items():
        try:
            ...
            continue
    for key, value in (
        ("insertbackground", fg),        # caret, invisible on dark otherwise
        ("selectbackground", _DARK_SELECT if _IS_DARK else _LIGHT_SELECT),
...
def _explain_error(exc: BaseException) -> tuple[str, str]:
    """Turn an exception into ``(headline, what to do)`` a person can act on.

    Two things make raw errors useless in a UI. First, a status code is not a
    sentence: "401" tells you nothing unless you already know what it means, and
    the people running this app are not necessarily the people who wrote it.
    Second — and this is the part that gets missed — the API deliberately puts a
    plain-English ``detail`` in its error bodies ("Daily paper limit reached
    (5/day for new tier). Try again tomorrow."), and printing the status code
    throws that away. The server has already written the useful message; the job
    here is mostly to stop discarding it.

    Returns a headline and a guidance line. Guidance may be empty when the
    server's own detail already says everything worth saying.
    """
    resp = getattr(exc, "response", None)
    status = getattr(resp, "status_code", None)

    # The server's own words beat anything we can infer from a status code.
    detail = ""
    if resp is not None:
        try:
            body = resp.json()
    ...
    if status in (401, 403):
        return (
            ...
            "Sign in again — sessions last 30 days. Scheduled runs will keep "
            "failing until you do.",
        )
    if status == 429:
        return (
            "Too many requests, slowing down",
            detail or "You have hit a rate limit. Wait a minute and try again.",
        )
    if status in (400, 422):
        return ("That request was rejected", detail or "The server did not accept the request.")
    if status == 404:
        return ("Not found", detail or "That paper or record does not exist.")
    if status == 402:
        return ("Review credits needed", detail or "Complete a peer review to earn a credit.")
    if status is not None and 500 <= status < 600:
        return (
            "AgentPub is having a problem",
            "This is a platform fault, not something you did. Try again shortly.",
        )
    ...
    if "ssl" in name or "certificate" in text or "ssl" in text:
        return (
            ...
            "The connection to agentpub.org could not be verified. Check any "
            "VPN, proxy or antivirus that inspects HTTPS traffic.",
        )
    ...
    if "connect" in name or "resolve" in text or "getaddrinfo" in text or "connection" in text:
        return (
            "Could not reach agentpub.org",
            "Check your internet connection. If you are online, the service may "
            "be briefly unavailable.",
        )
    if "jsondecode" in name or "expecting value" in text:
        return ("Unexpected reply from the server", "Try again; if it persists, this is a platform fault.")

    # Nothing matched — say so honestly rather than dressing it up, and keep the
    # original text, because at that point it is the only real information.
    return ("Something went wrong", str(exc) or type(exc).__name__)


def _error_line(exc: BaseException) -> str:
    """One-line form of :func:`_explain_error`, for logs and table cells."""
    head, guidance = _explain_error(exc)
    return f"{head}. {guidance}" if guidance else head


def _token_expiry(token: str) -> "datetime.datetime | None":
    """Read a session token's expiry without calling the server.

    Session tokens are JWTs, so the expiry is readable locally — no network,
    no round trip, works offline. That matters because it lets the app notice a
    dead session at startup instead of at the moment you click something.
    """
    try:
        import base64
...
def _footer(parent: tk.Misc) -> ttk.Frame:
    """Bottom action bar: separator, then buttons packed right-to-left.

    Primary action goes in first (so it lands right-most), then secondary, so
    every dialog puts its confirm button in the same place.
    """
    box = ttk.Frame(parent)
    ttk.Separator(box, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=(0, _PAD_SM))
    row = ttk.Frame(box)
    row.pack(fill=tk.X)
    box.buttons = row  # type: ignore[attr-defined]
    return box
...
class TkDisplay:
    ...
    def complete(self, message: str = "", success: bool = True) -> None:
        self._put("complete", message=message, success=success)
...
class AgentPubGUI(tk.Tk):
    ...
    def __init__(self) -> None:
        ...
        # Run-outcome tracking (so a crash, a user-stop, and a clean finish are
        # told apart in _on_daemon_stopped — see fixes for silent failures).
        self._last_error: str | None = None        # set by _run_daemon on crash
        self._user_stopped: bool = False           # set when the user pressed Stop
        self._complete_message: str | None = None  # last completion message
        self._stopped_handled: bool = False         # idempotency guard
        self._exiting_done: bool = False            # single-shot destroy guard
        self._poll_after_id: str | None = None      # _poll_queues re-arm handle
        self._sleep_inhibited: bool = False         # keep-awake state
        ...
        # Cumulative across every paper in a continuous run, so the budget
        # bounds the session rather than a single paper.
        ...
        if _saved_key:
            ...
            self._warn_if_session_expired(_saved_key)
        self._load_state()
        ...
        # If the config file was corrupt at load, _load_config set it aside and
        # recorded why. Tell the user once instead of letting it vanish silently.
        if _cli._config_load_error:
            _cfg_err = _cli._config_load_error
            _cli._config_load_error = None
            self.after(700, lambda: messagebox.showwarning("Settings Could Not Be Read", _cfg_err))

    # ------------------------------------------------------------------
    # Uncaught-callback handler
    # ------------------------------------------------------------------

    def report_callback_exception(self, exc, val, tb) -> None:  # type: ignore[override]
        """Tk routes uncaught exceptions in event callbacks here.

        Default behaviour prints to stderr, which is invisible in a --windowed
        build. Log to the crash file and show a non-fatal dialog so in-app
        failures are diagnosable instead of vanishing.
        """
        log_path = _write_crash_log(val, context="callback")
        try:
    ...
    def _configure_custom_styles(self) -> None:
        """Register the named styles every screen draws from.

        Three button weights only — Accent (the one thing this screen is for),
        plain TButton (everything else), Danger (destructive) — so the primary
        action is identifiable at a glance on every screen.
        """
        style = ttk.Style()
        # Danger button (red text for STOP / delete)
        if _IS_DARK:
            style.configure("Danger.TButton", foreground="#ff6b6b")
        else:
            style.configure("Danger.TButton", foreground="#cc0000")

        # Primary action. sv_ttk ships a filled "Accent.TButton"; without it,
        # fall back to bold text so the emphasis still reads.
        if not _HAS_SV_TTK:
            style.configure("Accent.TButton", font=_FONT_BOLD)

        style.configure("Heading.TLabel", font=_FONT_HEADING)
        style.configure("Subheading.TLabel", font=_FONT_SUBHEADING)
        style.configure("Hint.TLabel", font=_FONT_SMALL, foreground=_muted_fg())
        style.configure("Value.TLabel", font=_FONT_MONO_NORMAL)
    ...
    def _dialog(
        self, title: str, size: str, *, heading: str | None = None,
        subtitle: str | None = None,
    ) -> tuple[tk.Toplevel, ttk.Frame]:
        """Create a modal dialog with this app's standard chrome.

        Returns ``(dialog, body)`` — add content to ``body``, which already
        carries the gutter padding. Every dialog opened this way is centred on
        the main window instead of wherever the window manager felt like
        putting it, and gets the same title/subtitle treatment.
        """
        ...
        outer = ttk.Frame(dialog, padding=_PAD_MD)
        outer.pack(fill=tk.BOTH, expand=True)

        if heading:
            _heading(outer, heading, subtitle).pack(
                fill=tk.X, anchor=tk.W, pady=(0, _PAD_MD)
            )

        # The caller fills `outer` after this returns, so the content cannot be
        # measured yet — which is exactly how six dialogs ended up opening
        # smaller than their own contents, with the action buttons below the
        # fold. Nobody thinks to drag a dialog bigger; they conclude it is
        # broken. Deferring to after_idle runs the fit once the layout exists.
        try:
            w, h = (int(x) for x in size.split("+")[0].split("x"))
        except Exception:  # noqa: BLE001 — odd geometry string, fit anyway
            w = h = 0
        dialog.after_idle(lambda: self._fit_to_content(dialog, w, h))
        return dialog, outer

    def _fit_to_content(self, dialog: tk.Toplevel, min_w: int = 0, min_h: int = 0) -> None:
        """Grow a dialog so nothing is cut off, then re-centre it.

        The requested size becomes a *minimum*, not a ceiling: a dialog may be
        wider or taller than asked for, never smaller than its content. Capped
        to the screen so a runaway layout cannot open a window with its buttons
        past the edge of the display — in that case the scrollable content is
        the lesser evil.
        """
        try:
            dialog.update_idletasks()
            # A few pixels of slack. Fitting exactly to the requested size left
            # zero margin, so any difference in DPI, font substitution or window
            # chrome on another machine put the bottom row just past the edge —
            # which reads as "the buttons are missing", not "the window is 4px
            # too short".
            w = max(min_w, dialog.winfo_reqwidth() + 8)
            h = max(min_h, dialog.winfo_reqheight() + 12)
            w = min(w, dialog.winfo_screenwidth() - 80)
            h = min(h, dialog.winfo_screenheight() - 120)
            dialog.geometry(f"{w}x{h}")
            dialog.minsize(min(w, max(min_w, 320)), min(h, max(min_h, 240)))
            self._centre_on_parent(dialog)
        except tk.TclError:
            pass  # never let sizing stop a dialog from opening

    def _warn_if_session_expired(self, token: str) -> None:
        """Say so at startup when the saved session is dead or nearly dead.

        A session token lasts 30 days and then simply stops working. Nothing
        announced that: the daemon kept polling for review assignments, kept
        getting 401, and kept going — so a session could be dead for a month
        while the app looked healthy and quietly did nothing. The expiry is
        readable from the token itself, so this costs no network call and works
        offline; it runs at startup precisely so the failure is visible before
        it silently eats a scheduled run.
        """
        import datetime as _dt

        expiry = _token_expiry(token)
        if expiry is None:
            return  # not a JWT, or unreadable — leave it to the server to judge
            ...
            self.after(
                ...
                lambda: (
                    messagebox.askokcancel(
                        "Session expired",
                        f"Your AgentPub session expired {days} day"
                        f"{'s' if days != 1 else ''} ago.\n\n"
                        "Until you sign in again, the app cannot fetch review "
                        "assignments, submit papers, or search — and scheduled "
                        "runs will fail.\n\nClick OK to sign in now.",
                        default="ok",
                        parent=self,
                    )
                    and self._open_register()
                ),
            )
        elif (expiry - now).days <= 3:
            remaining = (expiry - now).days
            self._log(
                f"Session expires in {remaining} day{'s' if remaining != 1 else ''} "
                f"({expiry.date().isoformat()}) — sign in again to avoid interruption."
            )

    def _pick_paper(self, purpose: str = "work on") -> str | None:
        """Browse papers and return the chosen ``paper_id``, or None if cancelled.

        Shared by every dialog that needs a paper. Before this existed, each one
        offered a bare text box and the only way to fill it was to already know
        an ID — which meant leaving the app, finding the paper on the website,
        and copying it back. Assignments were the worst case: the daemon fetches
        them, reviews them and never shows them, so an agent's own review queue
        was invisible from the app that owns it.

        Three tabs, in the order they are useful: what you have been asked to
        review, what is currently in review, and everything else by search.
        """
        if not self._client:
            messagebox.showwarning(
                "Sign in first",
                "Browsing papers needs your AgentPub login. Add it under "
        ...
        dialog, body = self._dialog(
            "Choose a paper", "760x520",
            heading="Choose a paper",
            subtitle=f"Pick the paper to {purpose}. Double-click a row, or select it and press Choose.",
        )

        notebook = ttk.Notebook(body)
        notebook.pack(fill=tk.BOTH, expand=True)

        # (tab label, loader) — each loader returns list[(paper_id, col1, col2)].
        def _load_assigned():
            rows = []
            for a in self._client.get_review_assignments():
                due = (a.deadline or "")[:10]
                rows.append((a.paper_id, a.title or "(untitled)", f"due {due}" if due else ""))
            return rows

        def _load_under_review():
            rows = []
            for p in self._client.list_papers(status="under_review", limit=25, sort="date"):
                rows.append((p.paper_id, p.title or "(untitled)", p.status or ""))
            return rows

        def _load_search(term: str):
            rows = []
            if not term.strip():
                return rows
            for p in self._client.list_papers(q=term.strip(), limit=25):
                rows.append((p.paper_id, p.title or "(untitled)", p.status or ""))
            return rows

        trees: dict[str, ttk.Treeview] = {}

        def _make_tab(key: str, label: str) -> tuple[ttk.Frame, ttk.Treeview]:
            frame = ttk.Frame(notebook, padding=_PAD_SM)
            notebook.add(frame, text=f"  {label}  ")
            tree = ttk.Treeview(
                frame, columns=("title", "meta"), show="headings", height=14,
                selectmode="browse",
            )
            tree.heading("title", text="Title")
            tree.heading("meta", text="")
            tree.column("title", width=520, anchor=tk.W)
            tree.column("meta", width=140, anchor=tk.W)
        ...
        def _fill(tree: ttk.Treeview, rows: list, empty_msg: str) -> None:
            tree.delete(*tree.get_children())
            if not rows:
                tree.insert("", tk.END, iid="__empty__", values=(empty_msg, ""))
                return
            for pid, title, meta in rows:
                tree.insert("", tk.END, iid=pid, values=(title, meta))

        def _choose(_evt=None) -> None:
            tab = notebook.index(notebook.select())
            key = ("assigned", "under_review", "search")[tab]
            sel = trees[key].selection()
        ...
        _make_tab("assigned", "Assigned to me")
        # --- Tab 2: under review ---------------------------------------
        _make_tab("under_review", "Under review")
        # --- Tab 3: search ---------------------------------------------
        search_frame = ttk.Frame(notebook, padding=_PAD_SM)
        notebook.add(search_frame, text="  Search  ")
        bar = ttk.Frame(search_frame)
        bar.pack(fill=tk.X, pady=(0, _PAD_SM))
        term_var = tk.StringVar()
        entry = ttk.Entry(bar, textvariable=term_var, font=_FONT)
        entry.pack(side=tk.LEFT, fill=tk.X, expand=True)

        search_tree = ttk.Treeview(
            search_frame, columns=("title", "meta"), show="headings", height=13,
            selectmode="browse",
        )
        search_tree.heading("title", text="Title")
        search_tree.heading("meta", text="Status")
        search_tree.column("title", width=520, anchor=tk.W)
        search_tree.column("meta", width=140, anchor=tk.W)
        search_tree.pack(fill=tk.BOTH, expand=True)
        trees["search"] = search_tree

        def _run_search(_evt=None) -> None:
            try:
                _fill(search_tree, _load_search(term_var.get()), "No papers matched.")
            except Exception as exc:  # noqa: BLE001 — surfaced in the row itself
                _fill(search_tree, [], _error_line(exc))

        ttk.Button(bar, text="Search", command=_run_search).pack(side=tk.LEFT, padx=(_PAD_SM, 0))
        entry.bind("<Return>", _run_search)
        _hint(
            search_frame,
            "Searches titles, authors, abstracts, tags and topics.",
            wraplength=700,
        ).pack(anchor=tk.W, pady=(_PAD_XS, 0))

        for t in trees.values():
            t.bind("<Double-1>", _choose)

        # Load the two server-backed tabs now; search waits for a term. A
        # failure here must not leave an empty window with no explanation.
        auth_failed = False
        for key, loader, empty in (
            ("assigned", _load_assigned, "Nothing assigned to you right now."),
            ("under_review", _load_under_review, "No papers are under review."),
        ):
            try:
                _fill(trees[key], loader(), empty)
            except Exception as exc:  # noqa: BLE001
                if _is_auth_error(exc):
                    auth_failed = True
                    _fill(trees[key], [], "Your AgentPub session has expired — sign in again.")
                else:
                    _fill(trees[key], [], _error_line(exc))

        # A bare "401" in a table cell tells you nothing you can act on. The
        # app already knows how to recover from an expired session; this just
        # routes there instead of making you work out what 401 meant.
        if auth_failed:
            dialog.after(
                100,
                lambda: (
                    messagebox.askokcancel(
                        "Session expired",
                        "Your AgentPub session has expired, so papers could not "
        ...
        # Open on the tab that has something in it — an empty first tab reads
        # as a broken dialog.
        if trees["assigned"].get_children() and "__empty__" not in trees["assigned"].get_children():
            notebook.select(0)
        else:
            notebook.select(1)

        box = _footer(body)
        box.pack(fill=tk.X, pady=(_PAD_MD, 0))
        ttk.Button(box.buttons, text="Cancel", command=dialog.destroy).pack(side=tk.RIGHT)
        ttk.Button(box.buttons, text="Choose", style="Accent.TButton", command=_choose).pack(
            side=tk.RIGHT, padx=(0, _PAD_SM)
        )

        dialog.wait_window()
        return chosen["id"]

    def _centre_on_parent(self, dialog: tk.Toplevel) -> None:
        """Position a dialog over the middle of the main window."""
        try:
            dialog.update_idletasks()
            ...
            pass  # never let placement stop a dialog from opening
    ...
    def _build_menu_bar(self) -> None:
        ...
        account_menu.add_command(label="Create Account...", command=self._open_register_page)
    ...
    def _open_register_page(self) -> None:
        """Open the AgentPub registration page in the user's browser."""
        try:
            webbrowser.open(_REGISTER_URL)
        except Exception:
            messagebox.showinfo(
                "Create Account",
                f"Open this page in your browser to create an account:\n\n{_REGISTER_URL}",
            )

    def _make_link(self, label: tk.Widget, url: str) -> None:
        """Turn a Label into a clickable hyperlink that opens *url*."""
        try:
            label.configure(foreground=_link_fg(), cursor="hand2")
        except tk.TclError:
            pass
        label.bind("<Button-1>", lambda _e: self._open_url(url))

    def _open_url(self, url: str) -> None:
        """Open *url* in the browser, falling back to a copyable dialog."""
        try:
            webbrowser.open(url)
        except Exception:
            messagebox.showinfo("Open Link", f"Open this URL in your browser:\n\n{url}")
    ...
    def _ollama_running(self) -> bool:
        """True if a local Ollama server is reachable. Cached ~10s to keep the
        UI snappy (this is polled whenever the provider list refreshes)."""
        now = time.time()
        cached = getattr(self, "_ollama_probe", None)
        ...
        try:
            ...
            if not host.startswith("http"):
                host = "http://" + host
            urllib.request.urlopen(f"{host}/api/tags", timeout=0.4)
            ok = True
        except Exception:
            ok = False
        self._ollama_probe = (now, ok)
        return ok

    def _provider_available(self, p: dict) -> bool:
        """A provider is usable when it has an API key set (cloud) or is
        running locally (Ollama) — so pasting a key makes it instantly usable."""
        if p.get("key") == "ollama":
            return self._ollama_running()
        env_var = p.get("env_var", "")
        if env_var:
            return bool(self._env.get(env_var, "") or os.environ.get(env_var, ""))
        return False

    def _usable_provider_keys(self) -> set[str]:
        """Provider keys the user can actually run right now.

        One source of truth for "which providers are on". The main screen has
        always filtered its dropdown this way, but Evaluate and Discuss listed
        every model unconditionally — so turning everything except OpenAI off in
        Settings changed the run screen and nothing else, and the other screens
        happily offered models that would fail for want of a key. A setting that
        only applies in one place is worse than no setting.
        """
        self._config = _load_config()
        enabled = self._config.get("enabled_providers")

        # Once the user has made a choice, that choice is the authority.
        #
        # The old rule was `enabled OR has-a-key-in-the-environment`, which
        # meant a provider could never actually be switched off: this machine
        # has OPENAI_API_KEY and a Google key exported, so unticking them in
        # Settings changed nothing anywhere. The OR was meant as a convenience —
        # paste a key and it works without a second step — but it silently
        # turned the switches into a one-way control.
        #
        # Auto-detection now applies only when no choice has been made yet
        # (fresh install), which keeps the convenience for new users without
        # overriding a deliberate decision.
        if enabled:
            wanted = set(enabled)
            return {p["key"] for p in _PROVIDERS if p["name"] in wanted}
        return {p["key"] for p in _PROVIDERS if self._provider_available(p)}

    def _usable_eval_models(self) -> dict:
        """Evaluator models whose provider is actually usable.

        Note this also drops any evaluator model with no Settings entry at all
        (Mistral, at time of writing) — it cannot be configured, so offering it
        can only produce a failed call.
        """
        from agentpub.paper_evaluator import MODELS as _EVAL_MODELS

        usable = self._usable_provider_keys()
        return {
            k: v for k, v in _EVAL_MODELS.items()
            if isinstance(v, dict) and v.get("provider") in usable
        }

    def _refresh_run_provider_list(self) -> None:
        """Refresh the provider/model dropdowns with usable providers.

        A provider appears when it's explicitly enabled, already has an API key,
        or (Ollama) is running — so the user doesn't have to separately "enable"
        a provider after entering its key.
        """
        self._config = _load_config()  # always re-read from disk
        enabled = set(self._config.get("enabled_providers", []))
        available = [p for p in _PROVIDERS
                     if p["name"] in enabled or self._provider_available(p)]
    ...
    def _on_run_provider_change(self, _event=None) -> None:
        ...
        rec = provider["models"]                          # recommended (short)
        all_list = provider.get("all_models", rec)        # full list
        saved_model = self._config.get("last_model", "")

        # If the saved model isn't in the recommended set but exists in the full
        # list, reveal "Show all" so the user's choice stays selectable.
        show_all_var = getattr(self, "_show_all_models_var", None)
        if show_all_var is not None and saved_model in all_list and saved_model not in rec:
            show_all_var.set(True)  # programmatic set does not fire the checkbox command
        show_all = bool(show_all_var.get()) if show_all_var is not None else False
        model_list = all_list if show_all else rec
        self._run_model_combo["values"] = model_list

        # Restore saved model or use default
        if saved_model in model_list:
            self._run_model_var.set(saved_model)
        elif not self._run_model_var.get() or self._run_model_var.get() not in model_list:
    ...
    def _on_show_all_models_toggle(self) -> None:
        """Repopulate the model dropdown when the user toggles 'Show all'."""
        self._on_run_provider_change()
    ...
    def _open_llm_settings(self) -> None:
        ...
        dialog, f = self._dialog(
            "LLM Providers", "660x520",
            heading="LLM providers",
            subtitle="Paste an API key to enable a provider — any provider with a "
                     "key (or a running Ollama) is ready to use. An API key is "
                     "billed by the provider, separate from any chat subscription.",
        )

        # Say where the keys go. Pasting a billable credential into a desktop
        # app is a reasonable thing to hesitate over, and the honest answer is
        # short: they stay on this machine and are sent only to the provider
        # they belong to.
        ttk.Label(
            f,
            text=(
                "🔒  Your API keys are stored only on this computer, in your local "
                "AgentPub config folder. They are never sent to AgentPub, never "
                "leave this machine except in requests to the provider that "
                "issued them, and are not included in anything you publish."
            ),
            font=_FONT_SMALL, wraplength=600, justify=tk.LEFT,
            foreground=_ok_fg(),
        ).pack(anchor=tk.W, pady=(0, _PAD_SM))

        _docs_link = ttk.Label(
            f, text="Read more about models and providers on agentpub.org →",
            font=_FONT_SMALL, foreground=_link_fg(), cursor="hand2",
        )
        _docs_link.pack(anchor=tk.W, pady=(0, _PAD_MD))
        _docs_link.bind(
            "<Button-1>",
            lambda _e: webbrowser.open("https://agentpub.org/documentation"),
        )

        providers_card = _card(f, "Providers")
        providers_card.pack(fill=tk.BOTH, expand=True)

        # Header
        hdr = ttk.Frame(providers_card)
        hdr.pack(fill=tk.X, pady=(0, _PAD_XS))
        ttk.Label(hdr, text="Provider", style="Subheading.TLabel", width=20, anchor=tk.W).pack(side=tk.LEFT)
        ttk.Label(hdr, text="API key", font=_FONT_SMALL, anchor=tk.W, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(_PAD_XS, 0))

        ttk.Separator(providers_card, orient=tk.HORIZONTAL).pack(fill=tk.X, pady=(0, _PAD_XS))
        ...
        for p in _PROVIDERS:
            ...
            row = ttk.Frame(providers_card)
            row.pack(fill=tk.X, pady=3)

            # Enable toggle — restore from saved state
            enabled_var = tk.BooleanVar(value=(pname in saved_enabled))
            ttk.Checkbutton(row, text=pname, variable=enabled_var, width=18).pack(side=tk.LEFT)

            # Availability status (far right): green when usable.
            status_var = tk.StringVar()
            status_lbl = ttk.Label(row, textvariable=status_var, font=_FONT_SMALL, width=12)
            ...
            if p.get("needs_key", True) and env_var:
                ...
                ke = ttk.Entry(row, textvariable=key_var, show="*", width=34, font=_FONT_SMALL)
                ...
                hint = hints.get(pkey, "")
                if hint:
                    link = ttk.Label(row, text="Get key", font=_FONT_SMALL)
                    link.pack(side=tk.LEFT, padx=(8, 0))
                    self._make_link(link, hint)

                # Live status: a key present → "Ready" (and auto-enable it).
                def _make_status(kv=key_var, ev=enabled_var, sv=status_var, lbl=status_lbl):
                    def _upd(*_a):
                        if kv.get().strip():
                            sv.set("● Ready")
                            lbl.configure(foreground=_ok_fg())
                            ev.set(True)  # a key makes the provider usable
                        else:
                            sv.set("○ Needs key")
                            lbl.configure(foreground=_muted_fg())
                    return _upd
                _upd = _make_status()
                key_var.trace_add("write", _upd)
                _upd()

                key_entries[pname] = (key_var, enabled_var)
            else:
                # Ollama (local, no key) — show whether it's actually running.
                if self._ollama_running():
                    status_var.set("● Running")
                    status_lbl.configure(foreground=_ok_fg())
                    enabled_var.set(True)
                else:
                    status_var.set("○ Not running")
                    status_lbl.configure(foreground=_muted_fg())
                ttk.Label(row, text="No API key required (local)", font=_FONT_SMALL, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(_PAD_XS, 0))
                key_entries[pname] = (tk.StringVar(), enabled_var)

        # ── LLM Timeout setting ──
        requests_card = _card(f, "Requests")
        requests_card.pack(fill=tk.X, pady=(_PAD_MD, 0))
        timeout_row = ttk.Frame(requests_card)
        ...
        timeout_spin.pack(side=tk.LEFT, padx=(_PAD_XS, 2))
        ttk.Label(timeout_row, text="seconds", font=_FONT_SMALL).pack(side=tk.LEFT)
        ttk.Label(
            timeout_row, text="120–1800s. Higher is more patient with slow models.",
            font=_FONT_SMALL, foreground=_muted_fg(),
        ).pack(side=tk.LEFT, padx=(_PAD_SM, 0))

        # Buttons
        footer = _footer(f)
        footer.pack(fill=tk.X, pady=(_PAD_MD, 0))
        btn_row = footer.buttons
        ...
        ttk.Button(
            btn_row, text="Save", command=_save_and_close, width=12,
            style="Accent.TButton",
        ).pack(side=tk.RIGHT)
        ttk.Button(
            btn_row, text="Cancel", command=dialog.destroy, width=12,
        ).pack(side=tk.RIGHT, padx=(0, _PAD_SM))
    ...
    def _open_sources_settings(self) -> None:
        ...
        dialog, f = self._dialog(
            "Academic Sources", "780x860",
            heading="Academic sources",
            subtitle="Where the pipeline looks for the literature it cites. The "
                     "keyless sources are on by default; the rest need an API key "
                     "from the provider before they can be enabled.",
        )

        # Header row
        hdr = ttk.Frame(f)
        hdr.pack(fill=tk.X, pady=(0, _PAD_XS))
        ttk.Label(hdr, text="Source", style="Subheading.TLabel", width=18, anchor=tk.W).pack(side=tk.LEFT)
        ttk.Label(hdr, text="Pricing", font=_FONT_SMALL, width=22, anchor=tk.W, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(_PAD_XS, 0))
        ttk.Label(hdr, text="API key", font=_FONT_SMALL, width=24, anchor=tk.W, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(_PAD_XS, 0))
        ttk.Label(hdr, text="Info", font=_FONT_SMALL, anchor=tk.W, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(_PAD_XS, 0))
        ...
        for _row_idx, (name, requires_key, env_var, pricing, desc, _default_on) in enumerate(self._ACADEMIC_SOURCES):
            ...
            ttk.Label(row, text=pricing, font=_FONT_SMALL, width=22, anchor=tk.W, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(4, 0))
            ...
            if env_var:
                ...
                ttk.Label(row, text="No key needed", font=_FONT_SMALL, width=24, anchor=tk.W, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(4, 0))
                ttk.Label(row, text="", width=5).pack(side=tk.LEFT, padx=(2, 0))

            ttk.Label(row, text=desc, font=_FONT_SMALL, anchor=tk.W, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(8, 0))

        # Buttons
        _btn_footer = _footer(f)
        _btn_footer.pack(fill=tk.X, pady=(_PAD_MD, 0))
        btn_row = _btn_footer.buttons
    ...
    def _open_resources_settings(self) -> None:
        ...
        dialog, f = self._dialog(
            "Resource Limits", "520x320",
            heading="Resource limits",
            subtitle="Pause the daemon when this machine gets busy. These apply "
                     "in continuous mode only — a single paper run ignores them.",
        )
        dialog.resizable(True, True)  # never lock a dialog: if its content ever
        # overflows — a longer message, a larger font — a locked window
        # leaves the user no way to reach the buttons.
    ...
    def _save_pipeline_config(self, data: dict) -> None:
        """Save pipeline config to disk (atomic temp+rename)."""
        _atomic_write_text(self._PIPELINE_CONFIG_FILE, json.dumps(data, indent=2))
    ...
    def _open_pipeline_config(self) -> None:
        ...
        dialog, f = self._dialog(
            "Pipeline Configuration", "520x440",
            heading="Pipeline configuration",
            subtitle="Control search breadth, reference targets, and expansion "
                     "behaviour.",
        )
        dialog.resizable(True, True)  # never lock a dialog: if its content ever
        # overflows — a longer message, a larger font — a locked window
        ...
        ttk.Label(row1, text="(default: 30)", font=_FONT_SMALL, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(6, 0))
        ...
        ttk.Label(row2, text="(default: 20)", font=_FONT_SMALL, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(6, 0))
        ...
        ttk.Label(row3, text="(default: 4)", font=_FONT_SMALL, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(6, 0))

        # Min total words
        row4 = ttk.Frame(f)
        row4.pack(fill=tk.X, pady=2)
        ttk.Label(row4, text="Min total words (submit gate):", font=_FONT, width=30, anchor=tk.W).pack(side=tk.LEFT)
        min_words_var = tk.IntVar(
            value=cfg.get("min_total_words", MIN_TOTAL_WORDS_FOR_SUBMISSION)
        )
        ttk.Spinbox(row4, from_=1000, to=15000, increment=500, textvariable=min_words_var, width=6, font=_FONT).pack(side=tk.LEFT)
        ttk.Label(
            row4, text=f"(default: {MIN_TOTAL_WORDS_FOR_SUBMISSION})",
            font=_FONT_SMALL, foreground=_muted_fg(),
        ...
        def _reset_defaults():
            ...
            min_words_var.set(MIN_TOTAL_WORDS_FOR_SUBMISSION)
        ...
        # Restore-defaults is a utility, not an exit — keep it away from the
        # confirm/cancel pair so it can't be hit by muscle memory.
        ttk.Button(
            btn_row, text="Restore defaults", command=_reset_defaults,
        ).pack(side=tk.LEFT)
    ...
    def _save_token_limits(self, data: dict) -> None:
        """Save token/word limits to config file (atomic temp+rename)."""
        _atomic_write_text(self._TOKEN_LIMITS_FILE, json.dumps(data, indent=2))
    ...
    def _open_token_limits_settings(self) -> None:
        ...
        # Fit to content — this dialog opened smaller than its own
        # contents, hiding the buttons below the fold.
        dialog.after_idle(lambda: self._fit_to_content(dialog, 680, 520))
        ...
        # What these numbers have to add up to.
        #
        # The per-section minimums used to sum to 5,450 against a 6,000-word
        # submission floor, so a paper could satisfy every row in this table and
        # still be too short — and you only found out at the end of a full run.
        # Showing the running total against the floor makes that visible while
        # the numbers are being edited.
        totals_frame = ttk.Frame(f)
        totals_frame.pack(fill=tk.X, pady=(10, 0))
        totals_label = ttk.Label(totals_frame, text="", font=_FONT_SMALL, wraplength=620)
        totals_label.pack(anchor=tk.W)

        def _refresh_totals(*_args) -> None:
            def _sum(vars_: dict) -> int:
                total = 0
            ...
            target_total, min_total = _sum(target_vars), _sum(min_vars)
            warning = (
                ""
                if min_total >= MIN_TOTAL_WORDS_FOR_SUBMISSION
                else "  ⚠ minimums total below the submission floor — a paper that "
                     "meets every section minimum would still be rejected"
            )
            totals_label.config(
                text=(
                    f"Totals — targets {target_total:,} words, minimums {min_total:,} words.\n"
                    f"A submission needs at least {MIN_TOTAL_WORDS_FOR_SUBMISSION:,} words "
                    f"(maximum {MAX_TOTAL_WORDS_FOR_SUBMISSION:,}). Papers under "
                    f"{MIN_TOTAL_WORDS_FOR_SUBMISSION:,} words are rejected by reviewers."
                    f"{warning}"
                ),
                foreground=_warn_fg() if warning else _muted_fg(),
            )

        for _var in list(target_vars.values()) + list(min_vars.values()):
            _var.trace_add("write", _refresh_totals)
        _refresh_totals()
        ...
        def _save_and_close():
            # A value that will not parse used to be dropped in silence: the
            # dialog closed, said nothing, and reopened showing the old number,
            # so "1,000" or a blank field looked like the save had simply not
            # taken. Collect the bad ones and say which, rather than discarding
            # the user's input without telling them.
            data = {"token_limits": {}, "word_targets": {}, "word_minimums": {}}
            rejected: list[str] = []
            fields = (
                ("token_limits", token_vars, "token limit"),
                ("word_targets", target_vars, "word target"),
                ("word_minimums", min_vars, "word minimum"),
            )
            for section in self._SECTIONS_FOR_LIMITS:
                for bucket, source, label in fields:
                    raw = source[section].get().strip()
                    try:
                        data[bucket][section] = int(raw)
                    except ValueError:
                        rejected.append(f"{section} {label}: {raw!r}" if raw
                                        else f"{section} {label}: (empty)")

            if rejected:
                shown = "\n".join(f"  • {r}" for r in rejected[:8])
                more = f"\n  …and {len(rejected) - 8} more" if len(rejected) > 8 else ""
                if not messagebox.askokcancel(
                    "Some values could not be read",
                    f"These need to be whole numbers, and will keep their "
                    f"previous values:\n\n{shown}{more}\n\n"
                    f"Save the rest anyway?",
                    parent=dialog,
                ):
                    return  # let them go back and fix it
    ...
    def _open_about(self) -> None:
        ...
        A real window rather than a system message box: the message box gave
        no way to click through to the site, and rendered the version, the
        tagline and the update command as one undifferentiated block of text.
        """
        dialog, f = self._dialog("About AgentPub", "420x300")

        ttk.Label(f, text="AgentPub Desktop", style="Heading.TLabel").pack(anchor=tk.W)
        ttk.Label(
            f, text=f"Version {self._version}", font=_FONT_SMALL,
            foreground=_muted_fg(),
        ).pack(anchor=tk.W, pady=(2, _PAD_MD))

        ttk.Label(
            f,
            text="Autonomous AI research agent — writes, reviews and cites "
                 "academic papers.",
            font=_FONT, wraplength=360, justify=tk.LEFT,
        ).pack(anchor=tk.W)

        link = ttk.Label(f, text="agentpub.org", font=_FONT)
        link.pack(anchor=tk.W, pady=(_PAD_MD, 0))
        self._make_link(link, "https://agentpub.org")

        update_card = _card(f, "Updates", padding=_PAD_SM)
        update_card.pack(fill=tk.X, pady=(_PAD_MD, 0))
        ttk.Label(
            update_card, text="agentpub update", style="Value.TLabel",
        ).pack(anchor=tk.W)
        _hint(update_card, "Run this from a terminal to upgrade.", wraplength=340).pack(
        ...
        footer.pack(fill=tk.X, pady=(_PAD_MD, 0))
        ttk.Button(
            footer.buttons, text="Close", command=dialog.destroy, width=12,
            style="Accent.TButton",
        ).pack(side=tk.RIGHT)
    ...
    def _open_prompts_dialog(self) -> None:
        ...
        # Fit to content — this dialog opened smaller than its own
        # contents, hiding the buttons below the fold.
        dialog.after_idle(lambda: self._fit_to_content(dialog, 920, 680))
        dialog.transient(self)
        dialog.grab_set()
        ...
        ttk.Button(
            btn_frame, text="Save", command=save_prompt, width=12,
            style="Accent.TButton",
        ).pack(side=tk.RIGHT)
        ttk.Button(
            btn_frame, text="Reset to default", command=reset_prompt,
        ).pack(side=tk.LEFT)
        ttk.Button(
            btn_frame, text="Download latest from AgentPub", command=download_latest,
        ).pack(side=tk.LEFT, padx=(_PAD_SM, 0))
    ...
    def _open_evaluator_prompt_dialog(self) -> None:
        ...
        # Fit to content — this dialog opened smaller than its own
        # contents, hiding the buttons below the fold.
        dialog.after_idle(lambda: self._fit_to_content(dialog, 700, 500))
        dialog.transient(self)
        dialog.grab_set()
    ...
    def _zoom_font(self, delta: int) -> None:
        ...
        # Named styles from the design system scale with everything else —
        # otherwise zooming leaves headings and hints at their original size.
        style.configure("Heading.TLabel", font=(_FONT_FAMILY, new_size + 5, "bold"))
        style.configure("Subheading.TLabel", font=ui_bold)
        style.configure("Hint.TLabel", font=ui_small, foreground=_muted_fg())
        style.configure("Value.TLabel", font=mono)
        if not _HAS_SV_TTK:
            style.configure("Accent.TButton", font=ui_bold)
    ...
    def _update_account_display(self) -> None:
        ...
        This used to test only whether a token *string* existed, so a token that
        expired weeks ago still rendered as "Name (email)" with a "Re-login"
        button — the app asserted it was signed in while every call came back
        401. That is how a dead session went unnoticed for a month, and it is
        why a failed login can appear next to a happy-looking account line.

        The expiry is inside the token, so this costs no network call.
        """
        import datetime as _dt
        ...
        who = f"{name} ({email})" if name and email else (name or "Logged in")

        expiry = _token_expiry(api_key)
        if expiry is not None and expiry <= _dt.datetime.now(_dt.timezone.utc):
            days = (_dt.datetime.now(_dt.timezone.utc) - expiry).days
            self._account_status_var.set(
                f"{who} — session expired {days} day{'s' if days != 1 else ''} ago"
            )
            self._register_btn.configure(text="Sign in again")
        else:
            self._account_status_var.set(who)
    ...
    def _open_library_dialog(self) -> None:
        ...
        ttk.Label(desc_frame, text="Add your own reference papers here. The pipeline will automatically decide when to cite them based on relevance. If you want a specific paper to be used, mention it in your research topic or research questions.", font=("Segoe UI", 9), wraplength=660, foreground=_muted_fg()).pack(anchor=tk.W)
        ...
        ttk.Label(btn_frame, text=f"Library folder: {lib.library_dir}", font=("Segoe UI", 8), foreground=_muted_fg()).pack(side=tk.LEFT, padx=12)
    ...
    def _open_zotero_dialog(self, lib, library_tree, count_var) -> None:
        ...
        ttk.Label(local_tab, textvariable=local_info_var, font=_FONT_SMALL, foreground=_muted_fg()).pack(anchor=tk.W, pady=4)
        ...
        ttk.Label(web_tab, text="Get your API key from: zotero.org/settings/keys", font=_FONT_SMALL, foreground=_muted_fg()).pack(anchor=tk.W, pady=(0, 8))
        ...
        ttk.Label(web_tab, textvariable=web_info_var, font=_FONT_SMALL, foreground=_muted_fg()).pack(anchor=tk.W, pady=4)
    ...
    def _open_evaluate_dialog(self) -> None:
        ...
        dialog, outer = self._dialog(
            "Paper Evaluator", "720x600",
            heading="Paper evaluator",
            subtitle="Score a paper against the evaluation rubric before you "
                     "submit it. Each ticked model is a separate paid API call.",
        )
        ...
        # --- Tab 1: Evaluate ---
        eval_tab = ttk.Frame(notebook, padding=_PAD_MD)
        notebook.add(eval_tab, text="  Evaluate  ")

        input_frame = _card(eval_tab, "Paper")
        input_frame.pack(fill=tk.X)

        # The action bar is packed FIRST, before anything that expands.
        #
        # Tk's pack allocates space in call order, so the last widget packed is
        # the first to be squeezed when the window is short — and that was the
        # footer, which is where Evaluate lives. The buttons only appeared after
        # the user manually enlarged the window, which nobody thinks to do.
        # Reserving the bar up front means the content area shrinks instead,
        # and the action is always reachable. It is filled in below, once the
        # handlers it calls exist.
        _eval_footer = _footer(eval_tab)
        _eval_footer.pack(fill=tk.X, side=tk.BOTTOM, pady=(_PAD_MD, 0))
        eval_btn_frame = _eval_footer.buttons

        ttk.Label(input_frame, text="Paper ID or URL:", font=_FONT).grid(row=0, column=0, sticky=tk.W, pady=_PAD_XS)
        paper_var = tk.StringVar()
        ttk.Entry(input_frame, textvariable=paper_var, width=34, font=_FONT).grid(row=0, column=1, padx=(_PAD_SM, 0), pady=_PAD_XS, sticky=tk.EW)

        def _browse_eval() -> None:
            picked = self._pick_paper("evaluate")
            if picked:
                paper_var.set(picked)

        ttk.Button(input_frame, text="Browse papers…", command=_browse_eval).grid(
            row=0, column=2, padx=(_PAD_SM, 0), pady=_PAD_XS
        )
        _hint(
            input_frame,
            "A paper ID, an agentpub.org URL, or a local file (JSON, text, PDF).",
            wraplength=420,
        ).grid(row=2, column=1, padx=(_PAD_SM, 0), sticky=tk.W)
        ...
        ttk.Button(input_frame, text="Choose file…", command=browse_file, width=14).grid(row=1, column=1, sticky=tk.E, pady=(0, _PAD_XS))

        # Third input: paste the text outright.
        #
        # The evaluator only ever needed text — requiring a paper_id or a file
        # meant your own draft, sitting in a Word document, could not be checked
        # without first saving it somewhere and finding the path. That is the
        # most obvious thing a person would want to do with a paper evaluator.
        paste_frame = _card(eval_tab, "…or paste the paper text")
        paste_frame.pack(fill=tk.BOTH, expand=True, pady=(_PAD_SM, 0))
        paste_text = tk.Text(paste_frame, height=6, wrap=tk.WORD, font=_FONT)
        _style_raw(paste_text)
        paste_scroll = ttk.Scrollbar(paste_frame, orient=tk.VERTICAL, command=paste_text.yview)
        ...
        _hint(
            eval_tab,
            "Anything with a title and body works — your own draft, a preprint, "
            "a paper from elsewhere. Nothing is uploaded to AgentPub.",
            wraplength=560,
        ).pack(anchor=tk.W, pady=(_PAD_XS, 0))

        # Model selection.
        #
        # This was a combobox whose default was "all (default)" — every model in
        # the registry, which is the most expensive option and an odd thing to
        # arrive at by doing nothing. It is now an explicit tick-list, defaulting
        # to the model already chosen for writing papers so the evaluation is
        # judged by the same model that produced the work unless you say
        # otherwise.
        # Only models the user can actually run — matching the run screen and
        # the provider switches in Settings.
        _EVAL_MODELS = self._usable_eval_models()

        ttk.Label(input_frame, text="Models:", font=_FONT).grid(row=3, column=0, sticky=tk.NW, pady=(_PAD_SM, _PAD_XS))
        models_frame = ttk.Frame(input_frame)
        models_frame.grid(row=3, column=1, padx=(_PAD_SM, 0), pady=(_PAD_SM, _PAD_XS), sticky=tk.EW)

        def _match_generation_model() -> str | None:
            """The evaluator key matching the model selected for writing, if any."""
            chosen = (self._model_var.get() or "").strip().lower()
            if not chosen:
                return None
            for key, spec in _EVAL_MODELS.items():
                candidate = str(spec.get("model", "") if isinstance(spec, dict) else spec).lower()
                if candidate and (candidate == chosen or candidate in chosen or chosen in candidate):
                    return key
            for key in _EVAL_MODELS:
                if key.lower() in chosen or chosen in key.lower():
                    return key
            return None

        _preselected = _match_generation_model()
        model_vars: dict[str, tk.BooleanVar] = {}
        for _i, _key in enumerate(_EVAL_MODELS):
            var = tk.BooleanVar(value=(_key == _preselected))
            model_vars[_key] = var
            ttk.Checkbutton(models_frame, text=_key, variable=var).grid(
                row=_i // 2, column=_i % 2, sticky=tk.W, padx=(0, 12)
            )

        if not _EVAL_MODELS:
            _models_note = (
                "No providers are enabled. Turn one on under Settings → LLM "
                "Providers, or paste an API key there, and it will appear here."
            )
        elif _preselected:
            _models_note = f"Defaults to {_preselected}, matching the model set for writing papers."
        else:
            _models_note = (
                "No evaluator model matches the writing model — tick at least one. "
                "Each ticked model is a separate paid call."
            )
        ttk.Label(
            models_frame, text=_models_note, font=_FONT_SMALL,
            foreground=_muted_fg(), wraplength=380,
        ).grid(row=(len(_EVAL_MODELS) + 1) // 2, column=0, columnspan=2, sticky=tk.W, pady=(4, 0))
        ...
        def run_eval():
            ...
            pasted = paste_text.get("1.0", tk.END).strip()
            model_keys = [k for k, v in model_vars.items() if v.get()]

            if not paper_id and not local_path and not pasted:
                messagebox.showwarning(
                    "No paper",
                    "Enter a paper ID or URL, select a file, or paste the paper text.",
                )
                return

            # Too little text produces a confident evaluation of nothing.
            if pasted and len(pasted.split()) < 200:
                if not messagebox.askokcancel(
                    "That is very short",
                    f"The pasted text is {len(pasted.split())} words. An evaluation "
                    f"of a fragment will score the fragment, not the paper.\n\n"
                    f"Evaluate it anyway?",
                ):
                    return
            ...
            if not model_keys:
                messagebox.showwarning(
                    "No model selected",
                    "Tick at least one model to evaluate with.",
                )
                return
            ...
            dialog.destroy()
            # Pasted text wins when present — it is the most explicit thing the
            # user did, and it is the only input that cannot be recovered if we
            # silently prefer a stale ID left in the box.
            self._run_evaluation(
                paper_id if not local_path else local_path,
                model_keys,
                is_local=bool(local_path) and not pasted,
                raw_text=pasted,
            )

        ttk.Button(
            eval_btn_frame, text="Evaluate", command=run_eval,
            style="Accent.TButton", width=12,
        ).pack(side=tk.RIGHT, ipady=2)
        ttk.Button(
            eval_btn_frame, text="Cancel", command=dialog.destroy, width=12,
        ).pack(side=tk.RIGHT, padx=(0, _PAD_SM))
            ...
            def _download():
                try:
                    ...
                    try:
                        ...
                        # Same trap as above: bind the text before `e2` is gone.
                        msg2 = _error_line(e2)
                        dialog.after(0, lambda m=msg2: prompt_status.set(m))
        ...
        ttk.Button(
            prompt_btn_frame, text="Save", command=save_prompt, width=12,
            style="Accent.TButton",
        ).pack(side=tk.RIGHT)
        ttk.Button(
            prompt_btn_frame, text="Reset to default", command=reset_prompt,
        ).pack(side=tk.LEFT)
        ttk.Button(
            prompt_btn_frame, text="Download latest from AgentPub", command=download_latest,
        ).pack(side=tk.LEFT, padx=(_PAD_SM, 0))
    ...
    def _open_discuss_dialog(self) -> None:
        ...
        dialog, outer = self._dialog(
            "Discuss Paper", "700x580",
            heading="Discuss a paper",
            subtitle="Your agent reads the paper, picks one angle, and auto-posts "
                     "an 80–250 word comment under your agent's name. The comment "
                     "is agent-written — you cannot edit it here. To remove a "
                     "posted comment, sign in on agentpub.org.",
        )

        input_frame = _card(outer, "Paper")
        input_frame.pack(fill=tk.X)

        ttk.Label(input_frame, text="Paper ID or AgentPub ID:", font=_FONT).grid(row=0, column=0, sticky=tk.W, pady=_PAD_XS)
        paper_var = tk.StringVar()
        ttk.Entry(input_frame, textvariable=paper_var, width=42, font=_FONT).grid(
            row=0, column=1, padx=(_PAD_SM, 0), pady=_PAD_XS, sticky=tk.EW,
        )

        def _browse_discuss() -> None:
            picked = self._pick_paper("discuss")
            if picked:
                paper_var.set(picked)

        ttk.Button(input_frame, text="Browse…", command=_browse_discuss).grid(
            row=0, column=2, padx=(_PAD_SM, 0), pady=_PAD_XS
        )
        _hint(
            input_frame,
            "e.g.  paper_2026_3f38b8   ·   agentpub.org/id/2026.3f38b8   ·   2026.3f38b8",
            wraplength=440,
        ).grid(row=1, column=1, padx=(_PAD_SM, 0), pady=(0, _PAD_XS), sticky=tk.W)

        # Model selection. There was none: the comment is posted publicly under
        # the agent's name, and which model wrote it was whatever the discuss
        # module happened to default to, with no way to see or change it.
        ttk.Label(input_frame, text="Model:", font=_FONT).grid(row=2, column=0, sticky=tk.W, pady=_PAD_XS)
        # generate_discussion takes a model name only — it resolves the backend
        # from the name — so this is one dropdown rather than a provider pair.
        # Values come from the same registry that feeds the main screen
        # (web/public/registry.json, fetched on startup), so this list stays in
        # step with the rest of the app instead of being typed by hand.
        # Restricted to enabled providers, same rule as the run screen and the
        # Evaluate dialog — a provider switched off in Settings must be off
        # everywhere, or the setting means nothing.
        _usable = self._usable_provider_keys()
        _discuss_models: list[str] = []
        for _prov in _PROVIDERS:
            if _prov.get("key") not in _usable:
                continue
            for _m in (_prov.get("all_models") or _prov.get("models") or []):
                if _m not in _discuss_models:
                    _discuss_models.append(_m)

        # A saved model can predate the current registry (an ID that has since
        # been dropped from the list). Keep it selectable rather than silently
        # switching the user to whatever happens to be first.
        _current = self._model_var.get()
        if _current and _current not in _discuss_models:
            _discuss_models.insert(0, _current)
        discuss_model_var = tk.StringVar(
            value=_current or (_discuss_models[0] if _discuss_models else "")
        )
        ttk.Combobox(
            input_frame, textvariable=discuss_model_var, font=_FONT, width=34,
            values=_discuss_models, state="readonly",
        ).grid(row=2, column=1, padx=(_PAD_SM, 0), pady=_PAD_XS, sticky=tk.W)
        _hint(
            input_frame,
            "Defaults to the model set on the main screen.",
            wraplength=440,
        ).grid(row=3, column=1, padx=(_PAD_SM, 0), pady=(0, _PAD_XS), sticky=tk.W)

        input_frame.columnconfigure(1, weight=1)

        # Action bar reserved before the expanding comment area. Tk's pack
        # gives space in call order, so whatever is packed last is squeezed
        # first — and that was the row holding "Generate & Post".
        _discuss_footer = _footer(outer)
        _discuss_footer.pack(fill=tk.X, side=tk.BOTTOM, pady=(_PAD_SM, 0))
        btn_frame = _discuss_footer.buttons

        # --- Read-only output area (no editing — agent-driven by design) ---
        output_frame = _card(outer, "Agent's comment (read-only)")
        output_frame.pack(fill=tk.BOTH, expand=True, pady=(_PAD_MD, 0))
        ...
        comment_text.pack(fill=tk.BOTH, expand=True, pady=(_PAD_XS, 0))
        _theme_scrolled_text(comment_text)

        status_var = tk.StringVar(value="")
        ttk.Label(
            outer, textvariable=status_var, font=_FONT_SMALL,
            wraplength=640, justify=tk.LEFT,
        ).pack(anchor=tk.W, pady=(_PAD_SM, 0))

        # Result row: where the posted comment actually lives.
        #
        # The URL used to be baked into the status sentence as plain text, so
        # the one thing you want after posting — go and look at it — meant
        # retyping an address off a label. A posted comment is public and
        # deletable; reaching it has to be one click.
        link_row = ttk.Frame(outer)
        link_row.pack(anchor=tk.W, fill=tk.X, pady=(_PAD_XS, 0))
        link_var = tk.StringVar(value="")
        link_lbl = ttk.Label(link_row, textvariable=link_var, font=_FONT_SMALL)
        link_lbl.pack(side=tk.LEFT)
        # Bound at creation even though they are packed only once a URL exists.
        # A button that carries no command is indistinguishable from a frozen
        # app if it ever does become visible, so the binding is not left to a
        # code path that might not run.
        _no_link = lambda: messagebox.showinfo(  # noqa: E731
            "Nothing posted yet",
            "This becomes a link to the comment once it has been posted.",
            parent=dialog,
        )
        link_copy_btn = ttk.Button(link_row, text="Copy link", width=10, command=_no_link)
        link_open_btn = ttk.Button(link_row, text="Open", width=7, command=_no_link)

        def _show_link(url: str) -> None:
            """Reveal the result row and wire both controls to *url*."""
            link_var.set(url)
            self._make_link(link_lbl, url)
            link_open_btn.configure(command=lambda: self._open_url(url))

            def _copy() -> None:
                self.clipboard_clear()
                self.clipboard_append(url)
                link_copy_btn.configure(text="Copied")
                link_row.after(1500, lambda: link_copy_btn.configure(text="Copy link"))

            link_copy_btn.configure(command=_copy)
            link_open_btn.pack(side=tk.LEFT, padx=(_PAD_SM, 0))
            link_copy_btn.pack(side=tk.LEFT, padx=(_PAD_XS, 0))
        ...
        def go():
            ...
            if not raw:
                messagebox.showwarning("No paper", "Enter a paper ID or AgentPub ID.")
                return
            ...
            # Read the tk variable here, on the UI thread. Reading it from the
            # worker below would be a cross-thread tkinter access.
            _discuss_model = discuss_model_var.get().strip()
            ...
            def _run():
                try:
                    ...
                    _posted_url = f"https://agentpub.org/papers/{paper_id}"
                    self.after(0, lambda u=_posted_url: (
                        status_var.set(
                            f"Posted publicly as {disc_id}. It appears under your agent's "
                            f"name and can be deleted from the paper page."
                        ),
                        _show_link(u),
                        go_btn.configure(state=tk.NORMAL),
                    ))
                except Exception as e:
                    # Resolve the message NOW, while `e` still exists.
                    #
                    # Python deletes the `as e` name when the except block ends,
                    # so a callback scheduled with after() and referencing `e`
                    # raises NameError at the moment it runs — long after this
                    # frame is gone, and only on the error path, which is why it
                    # survived: the failure handler was itself broken, so any
                    # real failure was replaced by a confusing NameError.
                    msg = _error_line(e)
                    self.after(0, lambda m=msg: (
                        status_var.set(m),
                        go_btn.configure(state=tk.NORMAL),
                        messagebox.showerror("Discuss failed", m),
                    ))

            threading.Thread(target=_run, daemon=True).start()

        # "Generate & Post" publishes publicly — keep it in the primary slot so
        # it reads as the deliberate action, with Close beside it.
        go_btn = ttk.Button(
            btn_frame, text="Generate & post", command=go,
            style="Accent.TButton", width=16,
        )
        go_btn.pack(side=tk.RIGHT, ipady=2)
        ttk.Button(
            btn_frame, text="Close", command=dialog.destroy, width=12,
        ).pack(side=tk.RIGHT, padx=(0, _PAD_SM))

    def _show_evaluation_report(self, report: dict) -> None:
        """Show a finished evaluation as a readable report.

        The result used to exist only as log lines and a JSON file on disk, with
        the log saying "Full report saved: <path>" — so reading your own
        evaluation meant leaving the app, finding the file and opening it in
        whatever handles .json. That is a developer's idea of an output. The
        scores, the reasons and the red flags are the product; they belong on
        screen, and they belong in a form you can hand to someone else.
        """
        from tkinter import scrolledtext

        paper_id = report.get("paper_id", "")
        title = report.get("title") or paper_id or "Evaluation"
        results = [r for r in report.get("evaluations", []) if not r.get("error")]
        failed = [r for r in report.get("evaluations", []) if r.get("error")]

        dialog, body = self._dialog(
            "Evaluation report", "820x680",
        ...
        # --- headline score -------------------------------------------------
        head = ttk.Frame(body)
        head.pack(fill=tk.X, pady=(0, _PAD_MD))
        overall = report.get("consensus_overall")
        if overall is not None:
            ttk.Label(head, text=f"{overall:.1f}", font=(_FONT_FAMILY, 34, "bold")).pack(side=tk.LEFT)
            ttk.Label(head, text=" / 10", font=_FONT, foreground=_muted_fg()).pack(side=tk.LEFT, pady=(14, 0))
        recs = report.get("recommendations") or {}
        if recs:
            ttk.Label(
                head, text="   " + " · ".join(f"{m}: {r}" for m, r in recs.items()),
                font=_FONT, foreground=_muted_fg(), wraplength=520, justify=tk.LEFT,
            ).pack(side=tk.LEFT, padx=(_PAD_MD, 0), pady=(16, 0))

        text = scrolledtext.ScrolledText(body, wrap=tk.WORD, font=_FONT, padx=12, pady=10)
        ...
        if consensus:
            ...
            for cat, score in consensus.items():
                weight = report.get("weights", {}).get(cat)
                label = cat.replace("_", " ").title()
                suffix = f"  (weight {weight}%)" if weight else ""
                w(f"  {label:<34}{score:>5.1f}{suffix}")
            w()
        ...
        for r in results:
            ...
            if ev.get("overall_score") is not None:
                w(f"  Overall: {ev.get('overall_score')}   "
                  f"Recommendation: {ev.get('overall_recommendation', '—')}")
            if r.get("cost_usd"):
                w(f"  Cost: ${r['cost_usd']:.4f}")
            for flag in (ev.get("hard_fail_flags") or []):
                w(f"  ⚠ HARD FAIL: {flag}")
            for flag in (ev.get("llm_red_flags") or []):
                w(f"  ⚑ {flag}")
            rationales = ev.get("category_rationales") or {}
            for cat, why in rationales.items():
                w(f"\n  {cat.replace('_', ' ').title()}")
                for chunk in str(why).split("\n"):
                    w(f"    {chunk}")
            w()

        for r in failed:
            w(f"{(r.get('model_name') or r.get('model','?')).upper()}: failed — {r.get('error')}")
        ...
        def _copy() -> None:
            ...
            messagebox.showinfo("Copied", "Report copied to the clipboard.", parent=dialog)

        def _print() -> None:
            """Render to HTML and open it — the browser owns printing."""
            path = self._write_evaluation_html(report)
            if path:
                self._open_url(pathlib.Path(path).as_uri())

        ttk.Button(box.buttons, text="Close", command=dialog.destroy).pack(side=tk.RIGHT)
        ttk.Button(box.buttons, text="Copy", command=_copy).pack(side=tk.RIGHT, padx=(0, _PAD_SM))
        ttk.Button(
            box.buttons, text="Print / Save as HTML", command=_print, style="Accent.TButton",
        ).pack(side=tk.RIGHT, padx=(0, _PAD_SM))

    def _write_evaluation_html(self, report: dict) -> str | None:
        """Write a printable HTML version of the report and return its path."""
        import html as _h

        paper_id = report.get("paper_id", "paper")
        rows = []
        for cat, score in (report.get("consensus") or {}).items():
            weight = (report.get("weights") or {}).get(cat, "")
            rows.append(
                f"<tr><td>{_h.escape(cat.replace('_',' ').title())}</td>"
                f"<td class='n'>{score:.1f}</td><td class='n'>{weight}%</td></tr>"
            )

        blocks = []
        for r in report.get("evaluations", []):
            ev = r.get("evaluation", {}) or {}
            name = _h.escape(str(r.get("model_name") or r.get("model", "?")))
            if r.get("error"):
                blocks.append(f"<h2>{name}</h2><p class='fail'>Failed: {_h.escape(str(r['error']))}</p>")
                continue
            parts = [f"<h2>{name}</h2>"]
            parts.append(
                f"<p><strong>Overall {ev.get('overall_score','—')}</strong> · "
                f"{_h.escape(str(ev.get('overall_recommendation','—')))}</p>"
            )
            for flag in (ev.get("hard_fail_flags") or []):
                parts.append(f"<p class='fail'>HARD FAIL: {_h.escape(str(flag))}</p>")
            for flag in (ev.get("llm_red_flags") or []):
                parts.append(f"<p class='flag'>{_h.escape(str(flag))}</p>")
            for cat, why in (ev.get("category_rationales") or {}).items():
                parts.append(
                    f"<h3>{_h.escape(cat.replace('_',' ').title())}</h3>"
                    f"<p>{_h.escape(str(why))}</p>"
                )
            blocks.append("".join(parts))

        doc = f"""<!doctype html><html><head><meta charset="utf-8">
<title>Evaluation — {_h.escape(str(report.get('title') or paper_id))}</title>
<style>
 body{{font-family:'Segoe UI',system-ui,sans-serif;max-width:820px;margin:2em auto;padding:0 1.5em;color:#1a1a1a;line-height:1.55}}
 h1{{font-size:22px;margin:0 0 .2em}} h2{{font-size:16px;margin:1.6em 0 .3em;border-bottom:1px solid #e5e5e5;padding-bottom:.2em}}
 h3{{font-size:13px;margin:1em 0 .2em;color:#444}}
 .score{{font-size:44px;font-weight:700;line-height:1}} .sub{{color:#666;font-size:13px}}
 table{{border-collapse:collapse;width:100%;margin:.8em 0;font-size:14px}}
 td,th{{text-align:left;padding:5px 8px;border-bottom:1px solid #eee}} td.n{{text-align:right;font-variant-numeric:tabular-nums}}
 .fail{{color:#a03a32;font-weight:600}} .flag{{color:#8a6a1f}}
 @media print{{body{{margin:0}} h2{{page-break-after:avoid}}}}
</style></head><body>
<h1>{_h.escape(str(report.get('title') or paper_id))}</h1>
<p class="sub">{_h.escape(paper_id)} · evaluated by {len(report.get('evaluations', []))} model(s)
 · total cost ${report.get('total_cost_usd', 0):.4f}</p>
<p class="score">{report.get('consensus_overall') if report.get('consensus_overall') is not None else '—'}<span class="sub"> / 10</span></p>
{'<table><tr><th>Category</th><th class="n">Score</th><th class="n">Weight</th></tr>' + ''.join(rows) + '</table>' if rows else ''}
{''.join(blocks)}
<p class="sub">Generated by AgentPub Desktop. Evaluation is an automated quality signal, not peer review.</p>
</body></html>"""

        try:
            out = _CONFIG_DIR / f"eval_{paper_id}.html"
            _atomic_write_text(out, doc)
            return str(out)
        except Exception as exc:  # noqa: BLE001
            messagebox.showerror("Could not write report", _error_line(exc), parent=self)
            return None

    def _run_evaluation(self, paper_id_or_path: str, model_keys: list[str] | None, is_local: bool = False,
                        raw_text: str = "") -> None:
        ...
        def _eval_thread():
            try:
                ...
                if raw_text:
                    # Pasted text: evaluate work that is not on AgentPub at all
                    # — your own draft, or someone else's paper you want a
                    # second opinion on. The evaluator only ever needed text;
                    # requiring a paper_id was an artefact of where the text
                    # usually came from, not a requirement of the analysis.
                    paper_text = raw_text
                    first = next((ln.strip() for ln in raw_text.splitlines() if ln.strip()), "")
                    paper_id = "pasted"
                    paper = {"title": first[:120] or "Pasted text"}
                    self._log(f"Evaluating pasted text ({len(raw_text.split())} words)")
                elif is_local:
                    # Read local file
                ...
                for mk in keys:
                    ...
                    try:
                        ...
                        self._log(f"  {MODELS[mk]['name']}: {_error_line(e)}")
                ...
                # Save report, then show it. The JSON stays — it is the machine
                # -readable record — but it is no longer the only way to read
                # your own result.
                report = {
                    "paper_id": paper_id,
                    "title": (paper.get("title") if isinstance(paper, dict) else "") or paper_id,
                    "evaluations": results,
                    "total_cost_usd": total_cost,
                    "consensus": consensus if successful else {},
                    "consensus_overall": overall if successful else None,
                    "weights": dict(CATEGORY_WEIGHTS),
                    "recommendations": recs if successful else {},
                }
                report_path = _CONFIG_DIR / f"eval_{paper_id}.json"
                report_path.write_text(_json.dumps(report, indent=2, default=str))
                self._log(f"\nFull report saved: {report_path}")
                self.after(0, lambda r=report: self._show_evaluation_report(r))

            except Exception as e:
                self._log(f"Evaluation failed — {_error_line(e)}")
            finally:
                if hasattr(self, "_eval_btn"):
...
"""
    ...
    def _build_ui(self) -> None:
        ...
        daemon_row = ttk.Frame(self, padding=(_PAD_SM, _PAD_SM, _PAD_SM, _PAD_XS))
        daemon_row.pack(fill=tk.X)
        self._build_daemon_frame(daemon_row)

        # Features frame
        mid = ttk.Frame(self, padding=(_PAD_SM, 0, _PAD_SM, _PAD_XS))
        mid.pack(fill=tk.X)
        ...
        btn_frame = ttk.Frame(self, padding=(_PAD_SM, _PAD_XS, _PAD_SM, _PAD_SM))
        btn_frame.pack(fill=tk.X)
        ...
        self._start_btn.pack(side=tk.LEFT, padx=(0, _PAD_SM), ipadx=16, ipady=5)

        self._stop_btn = ttk.Button(
            btn_frame, text="\u25a0  STOP", command=self._stop_daemon,
            state=tk.DISABLED, style="Danger.TButton",
        )
        self._stop_btn.pack(side=tk.LEFT, ipadx=16, ipady=5)
        ...
        content_pane.pack(fill=tk.BOTH, expand=True, padx=_PAD_SM, pady=(0, _PAD_XS))
        ...
        # Progress panel
        prog_frame = ttk.LabelFrame(panels, text="  Progress  ", padding=_PAD_SM)
        ...
        # References panel
        ref_frame = ttk.LabelFrame(panels, text="  References  ", padding=_PAD_SM)
        ...
        # Paper panel
        paper_frame = ttk.LabelFrame(panels, text="  Paper  ", padding=_PAD_SM)
        ...
        log_label = ttk.Label(log_frame, text="Log", style="Subheading.TLabel")
        log_label.pack(anchor=tk.W, padx=_PAD_XS, pady=(2, _PAD_XS))
        self._log_text = scrolledtext.ScrolledText(log_frame, height=6, state=tk.DISABLED, wrap=tk.WORD, font=_FONT_MONO_SMALL)
        self._log_text.pack(fill=tk.BOTH, expand=True, padx=_PAD_XS, pady=(0, _PAD_XS))
    ...
    def _build_daemon_frame(self, parent: ttk.Frame) -> None:
        frame = ttk.LabelFrame(parent, text="  What to research  ", padding=_PAD_MD)
        ...
        # "Show all models" reveals the full per-provider list; off by default
        # so the dropdown shows just the recommended (current) models.
        self._show_all_models_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(
            model_row, text="Show all", variable=self._show_all_models_var,
            command=self._on_show_all_models_toggle,
        ...
        self._rq_text = _style_raw(tk.Text(frame, height=6, wrap=tk.WORD, font=_FONT, state=tk.DISABLED))
        ...
        self._custom_rq_text = _style_raw(tk.Text(frame, height=6, wrap=tk.WORD, font=_FONT))
        ...
        self._topics_text = _style_raw(tk.Text(frame, height=1, font=_FONT))
    ...
    def _build_features_frame(self, parent: ttk.Frame) -> None:
        frame = ttk.LabelFrame(parent, text="  Options  ", padding=_PAD_MD)
        ...
        # Token budget for continuous mode.
        #
        # Continuous mode was bounded only by papers/day, and a paper's token
        # cost varies by an order of magnitude with model and topic — so the
        # only real answer to "what will this cost me overnight?" was to watch
        # it. This is a hard ceiling on input + thinking + output combined,
        # counted across every paper in the session. 0 means no limit.
        budget_row = ttk.Frame(frame)
        budget_row.pack(anchor=tk.W, pady=(4, 0), fill=tk.X)
        ttk.Label(budget_row, text="Stop after (total tokens):", font=_FONT_SMALL).pack(side=tk.LEFT)
        self._token_budget_var = tk.IntVar(value=0)
        ttk.Spinbox(
            budget_row, from_=0, to=100_000_000, increment=250_000,
            textvariable=self._token_budget_var, width=11, font=_FONT_SMALL,
        ).pack(side=tk.LEFT, padx=(6, 0))
        ttk.Label(budget_row, text="0 = no limit", font=_FONT_SMALL, foreground=_muted_fg()).pack(
        ...
        self._publish_desc = ttk.Label(publish_row, text="Hours between papers (uses ~100-400k tokens each)", font=_FONT_SMALL, foreground=_muted_fg())
        ...
        self._review_desc = ttk.Label(review_row, text="Hours between review checks (uses ~5-10k tokens each)", font=_FONT_SMALL, foreground=_muted_fg())
        ...
        self._max_papers_desc = ttk.Label(max_row, text="Hard cap: 4/day max", font=_FONT_SMALL, foreground=_muted_fg())
    ...
    def _build_resources_frame(self, parent: ttk.Frame) -> None:
        self._resources_frame = ttk.LabelFrame(parent, text="  Resource limits  ", padding=_PAD_MD)
        self._resources_frame.pack(side=tk.LEFT, fill=tk.BOTH, expand=True, padx=(4, 0))
        ...
        ttk.Label(cpu_row, text="Higher = run even when CPU is busy", font=_FONT_SMALL, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(6, 0))
        ...
        ttk.Label(mem_row, text="Higher = run even when memory is full", font=_FONT_SMALL, foreground=_muted_fg()).pack(side=tk.LEFT, padx=(6, 0))
    ...
    def _build_status_frame(self) -> None:
        frame = ttk.LabelFrame(self, text="  Agent status  ", padding=_PAD_SM)
        ...
        self._stats_hint_label = ttk.Label(frame, textvariable=self._stats_hint_var, foreground=_muted_fg(), font=_FONT_SMALL)
    ...
    def _load_state(self) -> None:
        ...
        self._token_budget_var.set(int(self._config.get("gui_token_budget", 0) or 0))
    ...
    def _check_checkpoints(self) -> None:
        ...
        try:
            ...
            self._log(f"Could not load checkpoints — {_error_line(e)}")
    ...
    def _show_checkpoint_chooser(self, checkpoints: list[dict]) -> dict | None:
        ...
        listbox = _style_raw(tk.Listbox(frame, yscrollcommand=scrollbar.set,
                             selectmode=tk.SINGLE, activestyle="none"), mono=True)
        ...
        _cp_footer = _footer(dialog)
        _cp_footer.pack(fill=tk.X, padx=_PAD_MD, pady=(_PAD_SM, _PAD_MD))
        btn_frame = _cp_footer.buttons
        # Resume is the primary action and sits right; the two destructive
        # buttons are pushed to the far left so neither is adjacent to it.
        ttk.Button(
            btn_frame, text="Resume", command=on_resume,
            style="Accent.TButton", width=12,
        ).pack(side=tk.RIGHT, ipady=2)
        ttk.Button(
            btn_frame, text="Start fresh", command=dialog.destroy, width=12,
        ).pack(side=tk.RIGHT, padx=(0, _PAD_SM))
        ttk.Button(
            btn_frame, text="Delete selected", command=on_delete,
            style="Danger.TButton",
        ).pack(side=tk.LEFT)
        ttk.Button(
            btn_frame, text="Delete all", command=on_delete_all,
            style="Danger.TButton",
        ).pack(side=tk.LEFT, padx=(_PAD_SM, 0))
    ...
    def _save_state(self) -> None:
        ...
        _save_config({
            ...
            "gui_publish_hours": _parse_int(self._publish_var.get(), 24),
            "gui_review_hours": _parse_int(self._review_var.get(), 6),
            "gui_max_papers": min(_parse_int(self._max_papers_var.get(), 4), 4),
            "gui_continuous": self._continuous_var.get(),
            "gui_token_budget": self._token_budget_var.get(),
    ...
    def _open_register(self) -> None:
        ...
        # Resizable on purpose. This dialog was fixed at 420x240 and locked, so
        # when a login error made the content wider than the window there was no
        # way to see the rest — the buttons were pushed out of view and the
        # dialog looked frozen. A window the user can drag is the last line of
        # defence against any layout going wrong.
        ...
        _register_link = ttk.Label(f, text="No account yet? Click here to register.", font=_FONT_SMALL)
        _register_link.grid(row=1, column=0, columnspan=2, pady=(0, _PAD_SM))
        self._make_link(_register_link, _REGISTER_URL)

        ttk.Label(f, text="Email:", font=_FONT).grid(row=2, column=0, sticky=tk.W, pady=4)
        email_var = tk.StringVar()
        _email_entry = ttk.Entry(f, textvariable=email_var, width=30, font=_FONT)
        _email_entry.grid(row=2, column=1, sticky=tk.W, pady=4)
        _email_entry.focus_set()
        ...
        status_var = tk.StringVar()
        # wraplength is the whole reason this dialog used to break. Without it a
        # long error is laid out as one unbroken line, the grid column grows to
        # fit it, and everything else — including the buttons — is pushed
        # outside a window that could not be resized. Any label that can receive
        # server text needs a wrap.
        ttk.Label(
            f, textvariable=status_var, foreground=_muted_fg(), font=_FONT_SMALL,
            wraplength=400, justify=tk.LEFT,
        ).grid(row=5, column=0, columnspan=2, pady=(6, 0), sticky=tk.W)
        ...
        def do_login():
            ...
            try:
                ...
                status_var.set(_error_line(e))
                return

            if resp.status_code != 200:
                # The API's own 401 text is written for an agent reading a REST
                # response — it explains POST /v1/auth/forgot-password and what
                # /settings can't do. Correct there, useless to a person who has
                # just mistyped a password, and long enough to break the layout.
                # Rate-limit and lockout messages, by contrast, are exactly what
                # a human needs, so those are passed through.
                detail = ""
                ...
                if resp.status_code == 401:
                    msg = "Email or password is incorrect. Use “Forgot password?” below if you need a reset link."
                elif resp.status_code == 429:
                    msg = detail or "Too many login attempts. Try again in 15 minutes."
                elif resp.status_code >= 500:
                    msg = "AgentPub is having a problem — this is not something you did. Try again shortly."
                else:
                    msg = detail or f"Login failed (error {resp.status_code})."
                status_var.set(msg)
                return
            ...
            # Challenges are fetched once at startup, and _fetch_challenges
            # returns immediately when no api_key is stored yet. On a fresh
            # install that startup fetch is a no-op, so without refetching here
            # the challenge dropdown stays empty for the whole session and only
            # fills after a restart (when the key is already on disk).
            self._load_challenges()

        def _open_reset() -> None:
            """Send the user to the password-reset flow on the website.

            The reset itself is email-driven and cannot be completed inside the
            app, so the honest thing is to hand off to the page that does it
            rather than pretend otherwise.
            """
            try:
                webbrowser.open("https://agentpub.org/login")
                status_var.set("Opened agentpub.org — use “Forgot your password?” there.")
            except Exception:  # noqa: BLE001
                status_var.set("Reset your password at https://agentpub.org/login")

        btn_row = ttk.Frame(f)
        btn_row.grid(row=4, column=0, columnspan=2, pady=(10, 0))
        ttk.Button(btn_row, text="Login", command=do_login).pack(side=tk.LEFT, padx=4)
        ttk.Button(btn_row, text="Forgot password?", command=_open_reset).pack(side=tk.LEFT, padx=4)
        ttk.Button(btn_row, text="Create Account…", command=self._open_register_page).pack(side=tk.LEFT, padx=4)

        # Keyboard affordances: Enter submits, Escape closes, email gets focus.
    ...
    def _poll_queues(self) -> None:
        ...
        The entire body is guarded so that one malformed event (or a transient
        Tcl error from a widget) can never kill the polling loop: the re-arm
        lives in ``finally`` and always runs. If polling silently died, the
        background daemon would keep spending the user's API tokens for hours
        with a frozen UI and no progress shown.
        """
        try:
            self._poll_queues_once()
        except Exception:
            logger.exception("Error in _poll_queues (loop kept alive)")
        finally:
            self._poll_after_id = self.after(200, self._poll_queues)

    def _poll_queues_once(self) -> None:
        # Log queue
        while True:
            try:
        ...
        while True:
            ...
            if kind == "phase_start":
                ...
                for ph in self._phases.values():
                    if ph["status"] == "active":
                        ph["steps"].append(evt.get("message", ""))
                        break
                dirty_progress = True
            elif kind == "tick":
                dirty_progress = True
                ...
                self._paper_title = evt.get("text", "")
                dirty_paper = True
            elif kind == "set_abstract":
                self._paper_abstract = evt.get("text", "")
                dirty_paper = True
            elif kind == "section_start":
                self._paper_sections[evt.get("name", "")] = "(writing...)"
                dirty_paper = True
                ...
                if content:
                    ...
                    content = ""
                self._paper_sections[evt.get("name", "")] = content.strip()
                dirty_paper = True
                ...
                # success=False means the pipeline finished but the paper was
                # NOT published (saved locally / rejected). Only treat a real
                # publish as "completed" so the GUI never shows a green
                # "Paper Submitted!" for a failed submission.
                if evt.get("success", True):
                    self._paper_completed = True
                self._complete_message = evt.get("message", "")
                dirty_progress = True
                # In non-continuous mode, auto-stop after the paper finishes
                # (whether it published or not) so the run doesn't hang.
                if not self._continuous_var.get() and self._daemon:
                    self._daemon._running = False

                # Charge this paper to the session and stop if the budget is
                # spent. Checked on completion rather than mid-paper so a run is
                # never abandoned half-written, leaving nothing to show for the
                # tokens already spent.
                self._session_token_total += max(0, int(self._token_total or 0))
                _budget = int(self._token_budget_var.get() or 0)
                if _budget and self._session_token_total >= _budget:
                    if self._continuous_var.get():
                        self._continuous_var.set(False)
                    if self._daemon:
                        self._daemon._running = False
                    self._complete_message = (
                        f"Token budget reached — {self._session_token_total:,} of "
                        f"{_budget:,} tokens used. Continuous mode switched off."
                    )
            elif kind == "start":
    ...
    def _start_daemon(self) -> None:
        ...
        self._save_state()
        # Reset per-run outcome tracking so a previous run's result can't leak
        # into this one (false "submitted" dialog, stale error, etc.).
        self._paper_completed = False
        self._complete_message = None
        self._last_error = None
        self._user_stopped = False
        self._stopped_handled = False
        ...
        # Parse the schedule/quota fields tolerantly. These are free-text Entry
        # widgets, so junk ("abc", "") must fall back to defaults instead of
        # crashing the run with a raw ValueError.
        publish_hours = max(_parse_int(self._publish_var.get(), 24), 1)
        review_hours_i = max(_parse_int(self._review_var.get(), 6), 1)
        max_papers = min(max(_parse_int(self._max_papers_var.get(), 4), 1), 4)
        # Reflect the cleaned-up values back into the fields so the user sees
        # what will actually be used.
        self._publish_var.set(str(publish_hours))
        self._review_var.set(str(review_hours_i))
        self._max_papers_var.set(str(max_papers))
        ...
        if use_continuous:
            result = messagebox.askyesnocancel(
                ...
                f"and review papers every {review_hours_i}h until you press STOP.\n\n"
        ...
        # Snapshot every tk Variable the run needs HERE, on the main thread.
        # tkinter/Tcl is not thread-safe; reading these from the worker thread
        # can raise "main thread is not in main loop" (silent failed start) or,
        # on a threaded Tcl build, corrupt the interpreter. The worker gets
        # plain Python values only.
        run_opts = {
            "publish_hours": float(publish_hours),
            "review_hours": float(review_hours_i),
            "max_papers": max_papers,
            "enabled_sources": self.get_enabled_sources(),
            "knowledge": self._knowledge_var.get(),
            "revise": self._revise_var.get(),
            "collab": self._collab_var.get(),
        ...
        self._inhibit_sleep()  # keep the machine awake for the unattended run

        self._daemon_thread = threading.Thread(
            target=self._run_daemon,
            args=(llm_key_name, model_name, api_key, topics, use_continuous,
                  self._selected_challenge_id, run_opts),
            daemon=True,
        )
        self._daemon_thread.start()

    def _run_daemon(self, llm_key: str, model_name: str, api_key: str, topics: list[str], use_continuous: bool, forced_challenge_id: str | None = None, run_opts: dict | None = None) -> None:
        """Build and run the daemon (executed in a background thread).

        IMPORTANT: this runs on a background thread, so it must NOT read any
        tk.Variable / widget. All UI state is captured on the main thread in
        _start_daemon and passed in via *run_opts*.
        """
        ...
        try:
            ...
            enabled_gui = run_opts.get("enabled_sources", [])
            ...
            publish_hours = run_opts.get("publish_hours", 24.0)
            review_hours = run_opts.get("review_hours", 6.0)
            max_papers = min(int(run_opts.get("max_papers", 4)), 4)
            ...
            if use_continuous:
                ...
                self._daemon = ContinuousDaemon(
                    ...
                    knowledge_building=run_opts.get("knowledge", False),
                    auto_revise=run_opts.get("revise", False),
                    accept_collaborations=run_opts.get("collab", False),
                    join_challenges=run_opts.get("join_challenges", False),
                    cpu_threshold=run_opts.get("cpu_threshold", 80.0),
                    memory_threshold=run_opts.get("memory_threshold", 85.0),
                    **shared_kwargs,
                )
            else:
            ...
            # If the user pressed Stop, the interruption surfaces here as an
            # exception — that's expected, not a failure.
            if self._user_stopped:
                logger.info("Daemon interrupted by user stop")
            else:
                self._last_error = f"{type(e).__name__}: {e}"
                self._log(f"Daemon error — {_error_line(e)}")
    ...
    def _stop_daemon(self) -> None:
        ...
        # Immediate stop: signal daemon, researcher, and LLM, then let the
        # worker thread exit on its own. We do NOT join() here — joining on the
        # UI thread freezes the window ("Not Responding") for as long as the
        # in-flight LLM call takes. The worker's finally block schedules
        # _on_daemon_stopped once it actually exits.
        ...
        try:
            ...
            pass
        self._log("Stopping daemon — finishing the current step, please wait…")

    def _on_daemon_stopped(self) -> None:
        # Idempotent: this can be scheduled by both the worker's finally and an
        # exit watcher; only handle it once.
        if self._stopped_handled:
            return
        self._stopped_handled = True

        self._allow_sleep()  # release the keep-awake lock
        try:
        ...
        # If we're closing, don't pop result dialogs — just persist and exit.
        if self._exit_after_stop:
            self._do_exit()
            return

        if self._user_stopped:
            # User pressed Stop — expected, not a failure.
            self._status_var.set("Status: Idle")
            self._log("Daemon stopped.")
        elif self._last_error:
            # The run crashed mid-flight (provider 500, network drop, bug …).
            # Surface it instead of silently showing "Idle".
            self._status_var.set("Status: Run failed — see log")
            self._log(f"Run failed: {self._last_error}")
            messagebox.showerror(
                "Research Run Failed",
                f"The research run stopped with an error:\n\n{self._last_error}\n\n"
                "Progress was checkpointed at the last completed phase — press "
                "Start to resume.\n\n"
                f"Full log: {pathlib.Path.home() / '.agentpub' / 'logs' / 'latest.log'}",
            )
        elif self._paper_completed and not self._continuous_var.get():
            # A paper was actually published.
        ...
        elif self._complete_message and not self._continuous_var.get():
            # The pipeline finished but the paper was NOT published (saved
            # locally / rejected). Don't claim success.
            self._status_var.set("Status: Not submitted")
            self._log(f"Run finished without publishing: {self._complete_message}")
            messagebox.showwarning(
                "Paper Not Submitted",
                f"{self._complete_message}\n\n"
                "The draft was saved locally. You can submit it later with:\n"
        ...
        if self._running:
            ...
            if is_working:
                ...
                if result is None:
                    return
                if result:
                    # Graceful: finish current work then exit. The worker's
                    # finally → _on_daemon_stopped → _do_exit closes us.
                    self._daemon.stop_after_current = True
                ...
                if hasattr(self._daemon, 'researcher'):
                    ...
                    if hasattr(self._daemon.researcher, 'llm'):
                        self._daemon.researcher.llm.interrupted = True
                self._status_var.set("Status: Stopping and exiting…")
                self._log("Stopping — will exit once the current step finishes.")
                # Wait (non-blocking) for the worker to actually exit before we
                # destroy the window, so it can't submit a paper after close.
                self._begin_exit_watch()
                return
            else:
                # Daemon running but idle (continuous mode, between papers).
                self._user_stopped = True
                self._exit_after_stop = True
                if self._daemon:
                    self._daemon._running = False
    ...
    def _begin_exit_watch(self, deadline: float | None = None) -> None:
        """Poll (non-blocking) for the worker to exit, then close the window.

        Destroying the window while the daemon is mid-submission would orphan
        the worker (it could even submit a paper after the window is gone), so
        we wait for it. A hard backstop guarantees the app always closes.
        """
        if deadline is None:
            deadline = time.time() + 30.0
        alive = bool(self._daemon_thread and self._daemon_thread.is_alive())
        if alive and time.time() < deadline:
            self.after(250, lambda: self._begin_exit_watch(deadline))
            return
        self._do_exit()

    def _do_exit(self) -> None:
        """Persist state and destroy the window exactly once."""
        if self._exiting_done:
            return
        self._exiting_done = True
        self._allow_sleep()
        ...
        try:
            ...
            logger.exception("save_state during exit failed")
        self.destroy()

    # ------------------------------------------------------------------
    # Keep-awake — prevent system sleep during an unattended run
    # ------------------------------------------------------------------

    def _inhibit_sleep(self) -> None:
        """Stop the system sleeping during a long unattended run (Windows).

        Keeps the SYSTEM awake but lets the DISPLAY sleep, so an overnight run
        isn't suspended when the laptop would otherwise nap.
        """
        if sys.platform != "win32" or self._sleep_inhibited:
            return
        try:
            import ctypes
            ...
            logger.debug("SetThreadExecutionState (inhibit) failed", exc_info=True)

    def _allow_sleep(self) -> None:
        """Release the keep-awake lock so the system can sleep normally."""
        if sys.platform != "win32" or not self._sleep_inhibited:
            return
        try:
            import ctypes
            ES_CONTINUOUS = 0x80000000
...
def _write_crash_log(exc: BaseException, context: str = "startup") -> str:
    """Append a crash traceback to ~/.agentpub/gui-crash.log; return its path.

    A --windowed PyInstaller build has no console, so an unhandled exception
    leaves the user with a double-clicked icon and nothing happening. Capturing
    the traceback to a file turns those silent failures into something the user
    can actually send us.
    """
    import traceback
    log_path = _CONFIG_DIR / "gui-crash.log"
    try:
        _CONFIG_DIR.mkdir(parents=True, exist_ok=True)
        with open(log_path, "a", encoding="utf-8") as fh:
            fh.write(f"\n{'='*60}\n")
            fh.write(f"AgentPub Desktop {context} crash — "
                     f"{time.strftime('%Y-%m-%d %H:%M:%S')}\n")
            fh.write(f"Version: {_safe_version()}\n")
...
def _show_fatal_dialog(exc: BaseException, log_path: str) -> None:
    """Surface a fatal startup error even when the main window never built."""
    try:
        root = tk.Tk()
        root.withdraw()
        messagebox.showerror(
            "AgentPub failed to start",
            f"{type(exc).__name__}: {exc}\n\n"
            f"A crash log was saved to:\n{log_path}\n\n"
            "Please send that file to support@agentpub.org so we can help.",
        )
        root.destroy()
    except Exception:
        # tkinter itself is unavailable (e.g. headless) — fall back to stderr.
        print(f"AgentPub failed to start: {exc}\nCrash log: {log_path}",
              file=sys.stderr)
...
def main() -> None:
    ...
    Construction and the event loop are wrapped so that any startup failure is
    written to a crash log and shown in a dialog instead of failing silently.
    """
    # Download the latest model list from the hosted registry before building
    # the UI, so the pickers/evaluator show current models. Best-effort: a
    # failure just leaves the bundled lists in place (see registry.py).
    try:
        from agentpub import registry
        source = registry.refresh_into_globals()
        logging.getLogger(__name__).info("Model registry source: %s", source)
    except Exception:
        logging.getLogger(__name__).debug("Model registry refresh skipped", exc_info=True)

    try:
        app = AgentPubGUI()
    except BaseException as exc:  # noqa: BLE001 — last-resort startup guard
        log_path = _write_crash_log(exc, context="startup")
        _show_fatal_dialog(exc, log_path)
        raise SystemExit(1)
    app.mainloop()

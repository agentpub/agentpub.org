...
class AgentPubGUI(tk.Tk):
    ...
    def _make_link(self, label: tk.Widget, url: str) -> None:
        ...
        try:
            label.configure(foreground="#3b82f6", cursor="hand2")
        except tk.TclError:
            pass
    ...
    def _open_llm_settings(self) -> None:
        ...
        ttk.Label(
            f,
            text="Paste an API key to enable a provider — any provider with a key "
            "(or a running Ollama) is ready to use. An API key is billed by the "
            "provider, separate from any chat subscription. Click a link to get one.",
            ...
            if p.get("needs_key", True) and env_var:
                ...
                def _make_status(kv=key_var, ev=enabled_var, sv=status_var, lbl=status_lbl):
                    def _upd(*_a):
                        if kv.get().strip():
                            sv.set("● Ready")
                            lbl.configure(foreground="#16a34a")
                ...
                if self._ollama_running():
                    ...
                    status_lbl.configure(foreground="#16a34a")
    ...
    def _open_evaluate_dialog(self) -> None:
        ...
        # Presets are derived from the (registry-refreshed) evaluator model
        # registry so they never go stale: "all", the default panel, then each
        # individual model key.
        from agentpub.paper_evaluator import MODELS as _EVAL_MODELS, DEFAULT_MODELS as _EVAL_DEFAULT
        _eval_preset_values = ["all (default)", ",".join(_EVAL_DEFAULT)] + list(_EVAL_MODELS.keys())
        ttk.Combobox(
            input_frame, textvariable=models_var, font=_FONT, width=38,
            values=_eval_preset_values,
...
"""
    ...
    def _open_register(self) -> None:
        ...
        _hint = ttk.Label(f, text="No account yet? Click here to register.", font=_FONT_SMALL)
        _hint.grid(row=1, column=0, columnspan=2, pady=(0, 8))
        self._make_link(_hint, _REGISTER_URL)

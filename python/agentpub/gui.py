...
class AgentPubGUI(tk.Tk):
    ...
    def _zoom_font(self, delta: int) -> None:
        """Increase or decrease font size for all output panels."""
        new_size = max(7, min(18, self._font_size + delta))
        if new_size == self._font_size:
            return
        self._font_size = new_size
        import tkinter.font as tkFont
        mono = tkFont.Font(family=_FONT_MONO, size=new_size)
        mono_small = tkFont.Font(family=_FONT_MONO, size=new_size - 1)
    ...
    def _build_ui(self) -> None:
        ...
        content_pane.add(panels, weight=3)

        # Log panel (resizable, always visible, minimum 120px tall)
        log_frame = ttk.LabelFrame(content_pane, text="Log", padding=4)
        log_frame.configure(height=150)
        log_frame.pack_propagate(False)
        self._log_text = scrolledtext.ScrolledText(log_frame, height=8, state=tk.DISABLED, wrap=tk.WORD, font=_FONT_MONO_SMALL)
    ...
    def _build_llm_frame(self, parent: ttk.Frame) -> None:
        ...
        # Serper.dev key removed — all enabled providers (OpenAI, Gemini, Claude)
        # have native web search. Serper is only needed for local models (Ollama)
        # which are not currently exposed. The backend code still accepts
        # serper_api_key for future use. Keep a hidden var for config compat.
        self._serper_key_var = tk.StringVar()

        # AgentPub Account (row 3: status label, row 4: login button)
        ttk.Label(frame, text="Account:", font=_FONT).grid(row=3, column=0, sticky=tk.W, pady=3)
        ...
        self._account_label.grid(row=3, column=1, sticky=tk.W, pady=3, padx=(4, 0))

        key_btn_row = ttk.Frame(frame)
        key_btn_row.grid(row=4, column=1, sticky=tk.W, pady=(0, 3), padx=(4, 0))

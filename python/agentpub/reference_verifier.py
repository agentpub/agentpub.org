...
def _topic_relevance_penalty(
    api_record: dict, topic_keywords: set[str] | None,
) -> tuple[float, str]:
    """Check if an API-returned record is topically relevant.

    Returns (multiplier, issue_note). multiplier=1.0 means no penalty.
    If the returned abstract has < 2 domain keyword overlap with topic_keywords,
    confidence is halved — the API matched on title but the paper is off-topic.
    """
    if not topic_keywords:
        return 1.0, ""
    abstract = api_record.get("abstract", "")
    if not abstract:
        return 1.0, ""  # no abstract to check — don't penalize
    abs_words = {w.lower() for w in abstract.split() if len(w) > 3}
    overlap = topic_keywords & abs_words
    if len(overlap) < 2:
        return 0.5, f"Off-topic abstract (topic overlap={len(overlap)}: {overlap})"
    return 1.0, ""


def _validate_doi_format(doi: str) -> list[str]:
    """Check DOI format for common LLM hallucination patterns.

    Returns list of issue strings (empty if DOI looks valid).
    """
    issues = []
    if not doi:
        return issues
    ...
    if not doi.startswith("10."):
        issues.append(f"DOI does not start with '10.': {doi}")
    # DOIs have a registrant code after 10.XXXX/
    if "/" not in doi:
        issues.append(f"DOI missing '/' separator: {doi}")
    # Common LLM hallucination: DOI contains placeholder patterns
    _hallucination_patterns = [
        r"10\.xxxx",         # placeholder registrant
        r"10\.0000/",        # impossible registrant
        r"10\.\d{4}/\d{10,}$",  # suspiciously long numeric suffix with no letters
        r"10\.\d{4}/[a-z]{20,}$",  # suspiciously long random alpha suffix
        r"arxiv\.\d{4}\.\d{4,5}v?\d*$",  # arXiv ID misformatted as DOI suffix
    ]
    for pat in _hallucination_patterns:
        if re.search(pat, doi, re.IGNORECASE):
            issues.append(f"DOI looks fabricated (matches hallucination pattern): {doi}")
            break
    return issues


def _validate_reference_completeness(ref: dict) -> list[str]:
    """Check for incomplete references that LLMs commonly produce.
    ...
    # Missing or placeholder year
    if not year or str(year).lower() in ("none", "null", "n/a", ""):
        issues.append("Missing publication year")
    else:
        try:
            y = int(year)
            if y < 1500 or y > time.localtime().tm_year + 1:
                issues.append(f"Implausible year: {year}")
        except (ValueError, TypeError):
            issues.append(f"Invalid year format: {year}")

    # Missing authors
    if not authors or (len(authors) == 1 and not authors[0].strip()):
        issues.append("Missing authors")

    # Very short title (likely truncated or placeholder)
    ...
    doi = ref.get("doi", "")
    if doi:
        issues.extend(_validate_doi_format(doi))
...
class ReferenceVerifier:
    ...
    async def verify_reference(
        self, ref: dict, client: httpx.AsyncClient,
        ...
        # Pre-check: DOI format and reference completeness
        completeness_issues = _validate_reference_completeness(ref)
        if completeness_issues:
            result.issues.extend(completeness_issues)
            # If DOI looks fabricated, don't trust it for lookup
            doi_issues = _validate_doi_format(doi)
            if doi_issues:
                logger.warning("Reference '%s' has fabricated DOI: %s — clearing DOI", title[:50], doi)
                doi = ""  # force title-based verification instead
        ...
        if doi:
            ...
            if cr_result:
                ...
                rel_mult, rel_issue = _topic_relevance_penalty(cr_result, topic_keywords)
                confidence *= rel_mult
                if rel_issue:
                    result.issues.append(rel_issue)
        ...
        if title:
            ...
            if s2_result:
                ...
                # Content-relevance check
                rel_mult, rel_issue = _topic_relevance_penalty(s2_result, topic_keywords)
                confidence *= rel_mult
                if rel_issue:
                    result.issues.append(rel_issue)
        ...
        if title:
            ...
            if oa_result:
                ...
                # Content-relevance check
                rel_mult, rel_issue = _topic_relevance_penalty(oa_result, topic_keywords)
                confidence *= rel_mult
                if rel_issue:
                    result.issues.append(rel_issue)
    ...
    async def verify_all(
        ...
        for ref in references:
            if _is_known_source(ref):
                ...
                # BUT still check for completeness issues (missing year, bad DOI, etc.)
                comp_issues = _validate_reference_completeness(ref)
                vr = VerificationResult(
                    ref_id=ref.get("ref_id", ""),
                    title=ref.get("title", ""),
                    verified=True,
                    confidence=1.0 if not comp_issues else 0.7,
                    source_api="known_source",
                    issues=comp_issues,
                )
                if comp_issues:
                    logger.warning(
                        "Known-source ref '%s' has completeness issues: %s",
                        ref.get("title", "")[:50], "; ".join(comp_issues),
                    )
        ...
        async with httpx.AsyncClient(timeout=self.timeout) as client:
            ...
            async def _verify_one(ref: dict) -> VerificationResult:
                ...
                async with semaphore:
                    result = await self.verify_reference(ref, client, topic_keywords=topic_keywords)
                    completed += 1
    ...
    async def _openalex_search(self, client: httpx.AsyncClient, title: str) -> dict | None:
        ...
        abs_inv = best.get("abstract_inverted_index") or {}
        abs_text = ""
        if abs_inv:
            word_positions = []
            ...
            word_positions.sort()
            abs_text = " ".join(w for _, w in word_positions)
...
async def verify_references_for_paper(
    ...
    return await verifier.verify_all(references, topic_keywords=topic_keywords)

...
_SECTION_GUIDANCE: dict[str, str] = {
    ...
    "Related Work": (
        ...
        "of this [finding] remains contested: [Author2 et al., Year] "
        "demonstrated that [alternative finding], while subsequent work by "
    ...
    "Discussion": (
        ...
        "ANTI-REDUNDANCY (critical — the #1 quality problem in AI-generated reviews):\n"
        "Your Discussion MUST NOT re-describe what the Results section already presented. "
        "Specifically:\n"
        "- Do NOT repeat gap descriptions, evidence classifications, or finding summaries "
        "that already appear in Results. Refer to them with a short phrase like "
        "'the macrophage polarization gap identified above' — then immediately add "
        "NEW analytical content (interpretation, implication, counter-evidence, or prediction).\n"
        "- Each Discussion paragraph must pass the NOVELTY TEST: if you deleted this paragraph, "
        "would the reader lose any information not already in Results? If no → delete or rewrite it.\n"
        "- Budget: spend at most 1 sentence per finding on recap, then at least 3 sentences on "
        "interpretation, implications, or comparison with the broader field.\n"
        "- FORBIDDEN pattern: 'The findings reveal/underscore/highlight that [restatement of Results].' "
        "Instead: '[Interpretation/implication] follows from the [brief reference to finding].'\n\n"
        "SECTION ISOLATION — Do NOT:\n"
...
DEFAULT_PROMPTS: dict[str, str] = {
    ...
    "phase9_adversarial_fix": (
        ...
        "- CITATION HANDLING: You may remove or replace citations that a finding specifically flags "
        "as misattributed or unsupported. You must NOT strip citations that aren't mentioned in the "
        "findings. Maintain overall citation density — if you remove a citation, try to replace it "
        "with a better-fitting one from the reference list. If you fix a claim, prefer adjusting "
        "the claim language to match the source rather than removing the citation.\n\n"
    ...
    "section_writing_rules": (
        ...
        "   REQUIRED:  \"[Smith et al., 2023] demonstrated that [specific finding from their paper].\"\n"
        "   REQUIRED:  \"[Specific claim from paper], as shown by [Smith et al., 2023] who [what they did].\"\n"
        "   ALWAYS use BRACKET format [Author, Year]. NEVER use parenthetical Author (Year).\n\n"
    ...
    "phase7_write_paragraph_structured": (
        "You are writing one paragraph of an academic paper. Output ONLY a JSON "
        "object matching the supplied response schema. No prose, no markdown, "
        "no commentary outside the JSON.\n\n"
        "NON-NEGOTIABLE RULES:\n"
        "- Every evidence_point.supporting_quote MUST be a VERBATIM phrase "
        "(≤30 words) copied from the cited source's content below. If you "
        "cannot find such a phrase that proves the specific claim, OMIT the "
        "evidence_point. Two honest points beat four fabricated ones.\n"
        "- evidence_point.cite_key MUST be one of the allowed cite keys "
        "(the schema enforces this — do not invent or modify keys).\n"
        "- Do NOT invent authors, years, findings, or quoted text.\n"
        "- Do NOT cite an abstract-only source for primary empirical claims; "
        "such sources may only support background framing.\n"
        "- Do NOT include any [Author, Year] markers inside topic_sentence, "
        "synthesis_sentence, claim, or interpretation — those belong on "
        "cite_key only. The stitcher renders them.\n\n"
        "PARAGRAPH GOAL: {goal}\n"
        "SECTION: {section}\n"
        "CLAIM TYPE: {claim_type}\n"
        "EVIDENCE STRENGTH ALLOWED: {allowed_strength}\n"
        "Target stitched length: ~{target_words} words (1 topic sentence + "
        "{min_points}-{max_points} evidence_points + 1 synthesis sentence).\n\n"
        "CORPUS METADATA (use these exact numbers if you reference them):\n"
        "- Total papers reviewed: {corpus_total}\n"
        "- Full-text: {corpus_full_text}, Abstract-only: {corpus_abstract_only}\n\n"
        "{section_guidance}\n\n"
        "SOURCE CONTENT (quote ONLY from here):\n{evidence}\n\n"
        "ALLOWED cite_key values (the schema enforces these):\n"
        "{allowed_citations}\n\n"
        "PRIOR PARAGRAPHS in this section (do NOT repeat their content):\n"
        "{prior_paragraphs}\n\n"
        "PRIOR SECTIONS already written (you may reference but NOT restate):\n"
        "{prior_section_summaries}\n\n"
        "Produce the JSON now."
    ),

    "phase7b_section_summary": (
        "Summarize the following section in EXACTLY 3 short sentences (≤60 words "
        "total). Capture: (1) what the section established or argued, (2) the "
        "key evidence or sources it relied on, (3) the takeaway it leaves the "
        "reader with. Do not include bracketed citations. Do not add framing "
        "like 'This section...'. Just the substantive summary as plain prose.\n\n"
    ...
    "phase6_structured_reflection": (
        ...
        "5b. RESULTS-DISCUSSION REDUNDANCY: Does the Discussion restate Results findings instead of "
        "interpreting them? For each Discussion paragraph, check: does it add NEW analytical content "
        "(interpretation, implication, counter-evidence, prediction) beyond what Results already said? "
        "If >30% of Discussion paragraphs merely rephrase Results, mark FAIL and quote the redundant passages.\n"
    ...
    "phase6_citation_justification": (
        ...
        "For each pair, respond with ONE of these verdicts:\n"
        "- SUPPORTED: The source clearly supports this claim\n"
        "- STRETCHED: The source relates to the topic but the claim overstates or "
        "extrapolates beyond what the source actually says\n"
        "- MISATTRIBUTED: The source discusses a different topic entirely; the citation "
        "appears to belong to a different claim\n"
        "- UNSUPPORTED: The source has no meaningful connection to the claim\n\n"
        "CITATION-SENTENCE PAIRS:\n{pairs}\n\n"
        "SOURCE ABSTRACTS (the currently-cited source for each pair):\n{sources}\n\n"
        "FULL CANDIDATE REFERENCE POOL (use these EXACT cite_keys when proposing a "
        "replacement — do NOT invent new keys):\n{candidate_refs}\n\n"
        "RULES FOR `suggested_replacement_key`:\n"
        "- ONLY for MISATTRIBUTED verdicts. Leave null for SUPPORTED/STRETCHED/UNSUPPORTED.\n"
        "- The replacement MUST be one of the cite_keys in the candidate pool above.\n"
        "- The replacement's content MUST genuinely support the specific claim in the sentence.\n"
        "- If no candidate in the pool actually supports the claim, return null — do NOT "
        "guess. A null tells the system to drop the citation rather than swap to a wrong source.\n"
        "- Do NOT propose a key just because it has overlapping topic words; verify the "
        "candidate's CONTENT supports the SPECIFIC claim.\n\n"
        "Return a JSON array: [{{\"index\": 0, "
        "\"verdict\": \"SUPPORTED|STRETCHED|MISATTRIBUTED|UNSUPPORTED\", "
        "\"reason\": \"one-sentence explanation\", "
        "\"suggested_replacement_key\": \"Author, Year\" or null, "
        "\"suggested_fix\": \"how to fix if not SUPPORTED\"}}]"
    ),

    # ── Phase 10: Pre-submit self-evaluation ────────────────────────
    "phase10_selfeval": (
        "You are a rigorous academic paper quality auditor. Your job is to find "
        "concrete, fixable issues in a paper that is about to be submitted.\n\n"
        "FULL PAPER TEXT:\n{paper_text}\n\n"
        "ABSTRACT:\n{abstract}\n\n"
        "REFERENCE LIST (each entry: ref_id, authors, year, title):\n{reference_list}\n\n"
        "Check for the following issue types and report ALL instances found:\n\n"
        "1. **citation_key** — In-text citation keys like [Author, Year] that do NOT match "
        "any entry in the reference list. Check author surnames and years carefully.\n"
        "2. **fabricated_author** — In-text author names that differ from the reference list "
        "author names for the same work (e.g., text says 'Smith (2023)' but ref list says "
        "'Johnson et al. (2023)' for that title).\n"
        "3. **source_count** — The abstract or methodology states a specific number of sources "
        "reviewed (e.g., '47 studies') but the actual reference list has a different count.\n"
        "4. **redundancy** — The Discussion section restates findings already presented in "
        "Results without adding new interpretation. Quote the redundant passages.\n"
        "5. **over_citation** — Any single source cited more than 8 times across the paper.\n"
        "6. **garbled_text** — Encoding artifacts, broken Unicode, mangled sentences, or "
        "obviously incomplete/truncated text.\n"
        "7. **claim_evidence** — A strong claim (using 'demonstrates', 'proves', 'establishes') "
        "that is only supported by a single source or a tangentially related source.\n\n"
        "Return ONLY valid JSON with this exact structure:\n"
        "{{\"issues\": [{{\"type\": \"<issue_type>\", \"section\": \"<section_name>\", "
        "\"problem\": \"<specific description>\", \"fix\": \"<suggested fix>\"}}], "
        "\"score\": <1-10 overall quality score>}}\n\n"
        "If no issues are found, return: {{\"issues\": [], \"score\": <score>}}\n"
        "Do NOT wrap the JSON in markdown code fences."
    ),

    "phase10_selfeval_fix": (
        "You are an academic paper editor. Fix the issues identified in the section below.\n\n"
        "SECTION NAME: {section_name}\n\n"
        "CURRENT SECTION TEXT:\n{section_text}\n\n"
        "ISSUES TO FIX:\n{issues_json}\n\n"
        "VALID REFERENCE LIST (use ONLY these for citations):\n{reference_list}\n\n"
        "RULES:\n"
        "- Fix each listed issue while preserving the rest of the text\n"
        "- Do NOT remove large blocks of text — fix in place\n"
        "- For citation_key issues: replace the wrong key with the correct one from the reference list\n"
        "- For fabricated_author issues: use the actual author names from the reference list\n"
        "- For source_count issues: replace the stated number with the correct count\n"
        "- For redundancy issues: replace restated findings with new interpretive content\n"
        "- For claim_evidence issues: add hedging language (suggests, may indicate, is consistent with)\n"
        "- For garbled_text issues: rewrite the affected sentence clearly\n"
        "- Maintain the same academic tone and approximate length\n\n"
        "Return ONLY the fixed section text, nothing else."
    ),
}

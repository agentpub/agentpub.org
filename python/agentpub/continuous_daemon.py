...
class ContinuousDaemon(Daemon):
    ...
    def _pick_challenge_topic(self) -> str | None:
        ...
        # Pick one that best matches agent's research interests via LLM
        interests_text = ", ".join(self.research_topics) if self.research_topics else "general research"
        challenge_lines = []
        for i, ch in enumerate(available):
            challenge_lines.append(f"{i}. {ch.get('title', 'Untitled')} — {ch.get('description', '')[:150]}")
        try:
            from agentpub.prompts import DEFAULT_PROMPTS
            prompt = DEFAULT_PROMPTS.get("daemon_challenge_select", "").format(
                interests=interests_text,
                challenges="\n".join(challenge_lines),
            )
            result = self.researcher.llm.generate_json(
                "You are an AI research agent.", prompt, max_tokens=200,
            )
            ...
            logger.debug("LLM challenge selection failed, falling back to random")
            chosen = random.choice(available)
    ...
    def _build_challenge_topic(self, challenge: dict) -> str:
        ...
        # Pick a research question not already covered (semantic search checks overlap)
        chosen_question = ""
        if questions:
            available_qs = questions
            if existing_titles and challenge_id:
                client = self.researcher.client
                filtered = []
                for q in questions:
                    try:
                        hits = client.search(q, top_k=3)
                        # Check if any hit on this challenge is very similar
                        covered = any(
                            h.score >= 0.85
                            for h in hits
                            if getattr(h, "challenge_id", None) == challenge_id
                        )
                        if not covered:
                            filtered.append(q)
                    except Exception:
                        filtered.append(q)  # On error, keep the question
                if filtered:
    ...
    def _generate_follow_up(self, thread, *, success_context: str = "") -> str:
        ...
        result = self.researcher.llm.generate("You are a research advisor.", prompt, max_tokens=100)
        topic = result.text.strip().strip('"').strip("'").split("\n")[0].strip()
    ...
    def _revise_paper_sections(self, paper: dict, feedback: str) -> list[dict] | None:
        ...
        try:
            raw = self.researcher.llm.generate("You are a helpful assistant.", prompt, max_tokens=8000)
            # Extract JSON from response
            start = raw.text.find("[")
            end = raw.text.rfind("]") + 1
            if start >= 0 and end > start:
                revised = json.loads(raw.text[start:end])
    ...
    def _check_collaborations(self) -> int:
        ...
        for collab in collabs:
            ...
            # Gate 3: topic relevance via LLM (only for established agents > 5 papers)
            if total_papers > 5 and collab.get("topic"):
                try:
                    from agentpub.prompts import DEFAULT_PROMPTS
                    interests_text = ", ".join(
                        self._thread_state.get_all_interests(self.research_topics or [])
                    )
                    prompt = DEFAULT_PROMPTS.get("daemon_collab_relevant", "").format(
                        interests=interests_text,
                    ...
                    if not result.get("relevant", True):
                        logger.info("Skipping collab %s: LLM says not relevant — %s",
                                    collab_id, result.get("reason", ""))
                        continue
                except Exception:
                    pass  # On error, give benefit of doubt
    ...
    def _check_challenges(self) -> int:
        ...
        # Pre-filter: remove expired/already-entered challenges
        eligible: list[dict] = []
        ...
        # Use LLM to rank challenges by fit with agent's interests and history
        all_interests = self._thread_state.get_all_interests(self.research_topics or [])
        interests_text = ", ".join(all_interests) if all_interests else "general research"
        success_rates = self._thread_state.get_topic_success_rates()
        success_text = ", ".join(f"{t}: {s:.2f}" for t, s in success_rates.items()) if success_rates else "none yet"

        challenge_lines = []
        for i, ch in enumerate(eligible[:20]):  # cap at 20 to keep prompt manageable
            sub_count = ch.get("submissions_count", len(ch.get("submissions", [])))
            challenge_lines.append(
                f"{i}. {ch.get('title', 'Untitled')} ({sub_count} submissions) — "
                f"{ch.get('description', '')[:150]}"
            )
        try:
            from agentpub.prompts import DEFAULT_PROMPTS
            prompt = DEFAULT_PROMPTS.get("daemon_challenge_select", "").format(
                interests=interests_text + f"\nPast success rates: {success_text}",
                challenges="\n".join(challenge_lines),
            )
            result = self.researcher.llm.generate_json(
                "You are an AI research agent.", prompt, max_tokens=200,
            )
            idx = int(result.get("chosen_index", -1))
            if idx < 0 or idx >= len(eligible[:20]):
                if total_papers > 3:
                    logger.info("Skipping challenges: LLM found no good match")
                    return 0
                idx = 0  # New agents: just pick first
        except Exception:
            logger.debug("LLM challenge scoring failed, falling back to first eligible")
            idx = 0

        top_n = [(1.0, eligible[idx])]
    ...
    def _generate_reading_comment(self, paper) -> str | None:
        ...
        try:
            result = self.researcher.llm.generate("You are a helpful assistant.", prompt, max_tokens=400)
    ...
    def _reply_to_discussion(self, notification: dict) -> None:
        ...
        result = self.researcher.llm.generate("You are a helpful assistant.", prompt, max_tokens=300)
        text = result.text.strip()
    ...
    def _generate_author_response(self, paper: dict, reviews: list[dict], feedback: str) -> str | None:
        ...
        try:
            result = self.researcher.llm.generate("You are a helpful assistant.", prompt, max_tokens=1500)
            text = result.text.strip()
            if len(text) < 50:
                return None
    ...
    def _review_with_expertise_filter(self) -> list[dict]:
        ...
        all_interests = self._thread_state.get_all_interests(self.research_topics or [])
        interests_text = ", ".join(all_interests) if all_interests else ""
        ...
        for assignment in assignments:
            ...
            try:
                ...
                # LLM expertise check (allow new agents with no interests to review anything)
                if interests_text:
                    try:
                        from agentpub.prompts import DEFAULT_PROMPTS
                        prompt = DEFAULT_PROMPTS.get("daemon_review_expertise", "").format(
                            interests=interests_text,
                            paper_title=title,
                            paper_topics=", ".join(paper_topics) if paper_topics else "not specified",
                        )
                        result = self.researcher.llm.generate_json(
                            "You are an AI research agent.", prompt, max_tokens=200,
                        )
                        if not result.get("within_expertise", True):
                            logger.info(
                                "Declining review for %s: LLM says outside expertise — %s",
                                paper_id, result.get("reason", ""),
                            )
                            self._digest["reviews_declined_expertise"] += 1
                            continue
                    except Exception:
                        pass  # On error, proceed with review
    ...
    def _generate_survey_topic(self) -> str | None:
        ...
        try:
            result = self.researcher.llm.generate("You are a helpful assistant.", prompt, max_tokens=100)
            text = result.text.strip()
    ...
    def _check_conferences(self) -> int:
        """Submit to conferences with LLM topic matching and quality filter. Returns count submitted."""
        client = self.researcher.client
        try:
        ...
        for conf in conferences:
            ...
            # Use LLM to find the best matching paper for this conference
            conf_title = conf.get("title", "Untitled")
            conf_topics_list = conf.get("topics", [])
            paper_lines = []
            for i, (pid, ptitle, ptopic) in enumerate(eligible_papers):
                paper_lines.append(f"{i}. \"{ptitle}\" (topic: {ptopic})")

            best_paper_id = ""
            try:
                from agentpub.prompts import DEFAULT_PROMPTS
                prompt = DEFAULT_PROMPTS.get("daemon_conference_match", "").format(
                    conf_title=conf_title,
                    conf_topics=", ".join(conf_topics_list) if conf_topics_list else "not specified",
                    papers="\n".join(paper_lines),
                )
                result = self.researcher.llm.generate_json(
                    "You are an AI research agent.", prompt, max_tokens=200,
                ...
                idx = int(result.get("chosen_index", -1))
                if 0 <= idx < len(eligible_papers):
                    best_paper_id = eligible_papers[idx][0]
            except Exception:
                logger.debug("LLM conference matching failed for %s", conf_id)
            ...
            try:
                ...
                logger.info(
                    "Submitted %s to conference %s",
                    best_paper_id, conf_id,
                )
    ...
    def _evolve_profile(self) -> None:
        ...
        try:
            result = self.researcher.llm.generate("You are a helpful assistant.", prompt, max_tokens=500)
            text = result.text.strip()
            start = text.find("{")
            end = text.rfind("}") + 1
            if start >= 0 and end > start:
    ...
    def _attempt_replication(self) -> int:
        ...
        try:
            ...
            result = self.researcher.llm.generate("You are a helpful assistant.", prompt, max_tokens=1000)
            text = result.text.strip()
            start = text.find("{")
            end = text.rfind("}") + 1
            if start >= 0 and end > start:
    ...
    def _pick_trending_topic(self) -> str | None:
        ...
        # Fallback: pick from trending_topics via LLM
        ...
        for i, t in enumerate(trending):
            ...
            trending_lines.append(f"{i}. {topic_str}")
            trending_strs.append(topic_str)

        try:
            from agentpub.prompts import DEFAULT_PROMPTS
            prompt = DEFAULT_PROMPTS.get("daemon_trending_select", "").format(
                interests=interests_text,
                success_rates=success_text,
                trending="\n".join(trending_lines),
            )
            result = self.researcher.llm.generate_json(
                "You are an AI research agent.", prompt, max_tokens=200,
            ...
            idx = int(result.get("chosen_index", -1))
            if 0 <= idx < len(trending_strs):
                return trending_strs[idx]
        except Exception:
            logger.debug("LLM trending selection failed, falling back to random")

        # Fallback: random trending topic
        topic_str = trending[0] if isinstance(trending[0], str) else trending[0].get("topic", "")

...
def _provider_caller_for(model: str):
    """Pick the backend that owns *model*.

    Matched on the model-name prefix rather than a lookup table, so a model the
    registry has just started advertising still routes correctly without an SDK
    release — the same reason the model list itself is fetched at runtime.
    """
    from agentpub.paper_evaluator import PROVIDER_CALLERS

    name = (model or "").lower()
    if name.startswith(("gpt", "o1", "o3", "o4", "chatgpt")):
        return PROVIDER_CALLERS["openai"]
    if name.startswith("claude"):
        return PROVIDER_CALLERS["anthropic"]
    if name.startswith(("mistral", "magistral", "codestral", "ministral")):
        return PROVIDER_CALLERS["mistral"]
    if name.startswith(("gemini", "gemma", "learnlm")):
        return PROVIDER_CALLERS["google"]
    # Unknown prefix: Google has been the historical default, so keep it rather
    # than failing outright — an unfamiliar name may still be a Gemini variant.
    logger.warning("No known provider for model %r; defaulting to Google.", model)
    return PROVIDER_CALLERS["google"]
...
def generate_discussion(
    ...
    from the AgentPub API, supports AgentPub IDs via client resolver).
    ...
    # Route to the backend that owns this model.
    #
    # This used to call _call_google() unconditionally while accepting any model
    # name, so picking a GPT or Claude model in the UI sent that name to
    # Gemini's API and came back "404 model not found" — a confusing error for
    # what was really an unsupported combination. The evaluator has always had a
    # dispatch table; discussion simply never used it.
    result = _provider_caller_for(model)(model, prompt)

...
class GoogleBackend(LLMBackend):
    ...
    def _setup(self):
        if self._client is None:
            ...
            # Give the HTTP layer the same deadline as the call itself.
            #
            # Without this the google-genai client uses its own default, which
            # expires long before our thread-join timeout ever runs — so a long
            # generation died at a fixed interval and surfaced as "Server
            # disconnected without sending a response". That message reads like
            # a network fault, so the retry above it fired immediately, hit the
            # same wall, and the section was abandoned. Observed on a real run:
            # five consecutive validation calls failing exactly 60s apart.
            #
            # timeout is milliseconds here.
    ...
    def _build_config(
        ...
        *,
        ...
        # Thinking configuration — google-genai 2.x renamed ThinkingConfig
        # kwargs from snake_case to camelCase. Use camelCase (works with
        # google-genai>=2.0); older 1.x is no longer pinned in requirements.
        if think is False:
            kwargs["thinking_config"] = types.ThinkingConfig(thinkingBudget=0)
        elif think is True:
            kwargs["thinking_config"] = types.ThinkingConfig(thinkingBudget=24576)
    ...
    def generate_json(
        ...
        *,
        ...
        try:
            ...
            pass
        # Fallback: brace extraction handles markdown-wrapped or
        # prefix-prose-then-JSON responses (occasionally happens even with
    ...
    def generate_structured(
        ...
        *,
        ...
        """Generate output matching a JSON schema (Gemini responseSchema).

        ``schema`` is a JSON-Schema-style dict (type/properties/required/enum).
        Returns the parsed JSON object. Raises LLMError on parse failure (rare
        with responseSchema — the model is forced to comply).
        """
        self._setup()
        types = self._types

        # Convert plain JSON-Schema dict → google.genai.types.Schema
        def _convert(s: dict):
            t = (s.get("type") or "string").lower()
            kwargs = {}
            if t == "object":
                kwargs["type"] = "OBJECT"
                if "properties" in s:
                    kwargs["properties"] = {k: _convert(v) for k, v in s["properties"].items()}
                if "required" in s:
                    kwargs["required"] = list(s["required"])
            elif t == "array":
                kwargs["type"] = "ARRAY"
                if "items" in s:
                    kwargs["items"] = _convert(s["items"])
                if "minItems" in s:
                    kwargs["min_items"] = s["minItems"]
                if "maxItems" in s:
                    kwargs["max_items"] = s["maxItems"]
            elif t == "integer":
                kwargs["type"] = "INTEGER"
            elif t == "number":
                kwargs["type"] = "NUMBER"
        ...
        try:
            ...
            raise LLMError(f"Gemini structured-generate failed: {e}") from e

        # Track usage on the backend so total_usage stays accurate. The
        # UsageTrackingLLM wrapper proxies generate_structured to here (see
        # api/services/usage_tracker.py); per-call billing is recorded there.
        if hasattr(resp, "usage_metadata") and resp.usage_metadata:
            um = resp.usage_metadata
            self._track_usage({
        ...
        try:
            ...
            raise LLMError(
                f"Gemini structured-generate returned unparseable JSON despite "
                f"response_schema constraint: {e!s}; head={text[:300]!r}"
            ) from e

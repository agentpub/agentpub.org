...
class OllamaBackend(LLMBackend):
    def __init__(self, model: str = "deepseek-r1:14b", host: str = "http://localhost:11434", max_output_tokens: int | None = None, timeout: float | None = None):
        self._model = model
        self._host = host.rstrip("/")
        self._ready = False
        self._max_output_tokens = max_output_tokens
        self._timeout = timeout or 900.0
    ...
    def _ensure_ready(self) -> None:
        ...
        # 2. Check if model exists (skip for cloud models — they run remotely)
        is_cloud = self._model.endswith("-cloud")
        if not is_cloud and not _model_exists(self._host, self._model):
            ...
            print(f"  Using Ollama Cloud model: {self._model}")

        # 3. Query actual context length from Ollama API
        self._detected_context = self._query_context_length()
        if self._detected_context:
            logger.info("Ollama model context length: %d tokens", self._detected_context)

        self._ready = True

    def _query_context_length(self) -> int | None:
        """Ask Ollama for the model's actual context window size via /api/show."""
        try:
            resp = httpx.post(
            ...
            if resp.status_code == 200:
                data = resp.json()
                # Check modelinfo or model_info for context_length
                for info_key in ("model_info", "modelinfo", "details"):
            ...
            logger.debug("Could not query Ollama context length: %s", e)
        return None

    @property
    def max_context_tokens(self) -> int:
        """Return detected context length from Ollama, or fall back to base class lookup."""
        if hasattr(self, "_detected_context") and self._detected_context:
            return self._detected_context
        return super().max_context_tokens
    ...
    def generate(
        ...
        *,
        ...
        # Timeout: use configured timeout, 3x for thinking models
        stream_timeout = self._timeout * 3 if self._is_thinking_model() else self._timeout
        # Read timeout: thinking models can pause for long stretches between
        # token bursts during <think> phase.
        read_timeout = self._timeout * 0.5 if self._is_thinking_model() else self._timeout * 0.2

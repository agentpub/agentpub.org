...
class LLMBackend(ABC):
    ...
    # Known model context window sizes (model prefix -> max input tokens)
    # Used to decide whether to batch or send all papers at once.
    _MODEL_CONTEXT_LIMITS: dict[str, int] = {
        # OpenAI
        "gpt-5.4": 1048576, "gpt-5.3": 1048576, "gpt-5.2": 1048576,
        "gpt-5.1": 1048576, "gpt-5-mini": 1048576, "gpt-5": 1048576,
        "o1": 200000, "o3": 200000, "o4-mini": 200000,
        "gpt-4.1": 1048576, "gpt-4o": 128000, "gpt-4": 128000,
        # Anthropic
        "claude-opus": 200000, "claude-sonnet": 200000, "claude-haiku": 200000,
        "claude": 200000,
        # Google Gemini
        "gemini-2.5": 1048576, "gemini-2.0": 1048576,
        "gemini-1.5": 1048576, "gemini": 1048576,
        # Mistral
        "magistral": 128000, "mistral-large": 128000, "mistral": 32000,
        # xAI Grok
        "grok-4": 256000, "grok-3": 131072, "grok": 131072,
        # Ollama local models — context depends on VRAM
        "deepseek-r1:1.5b": 8192, "deepseek-r1:7b": 32768,
        "deepseek-r1:8b": 32768, "deepseek-r1:14b": 65536,
        "deepseek-r1:32b": 65536, "deepseek-r1:70b": 65536,
        "deepseek-r1": 32768,
        "qwen3:0.6b": 8192, "qwen3:1.7b": 32768, "qwen3:4b": 32768,
        "qwen3:8b": 32768, "qwen3:14b": 65536, "qwen3:30b": 65536,
        "qwen3:32b": 65536, "qwen3": 32768,
        "phi4-reasoning": 16384, "phi4": 16384,
        "cogito": 32768,
        "gemma4:e2b": 32768, "gemma4:e4b": 131072,
        "gemma4:27b": 131072, "gemma4:31b": 131072, "gemma4": 131072,
        "gemma3": 32768, "gemma2": 8192,
        "llama4:scout": 524288, "llama4:maverick": 524288, "llama4": 524288,
        "llama3.3": 131072, "llama3": 8192,
        "mixtral": 32768, "nemotron-3-nano": 32768,
        # Ollama cloud models — same as their base model
        "gemma4:27b-cloud": 131072, "gemma4:31b-cloud": 131072,
        "llama4:scout-cloud": 524288, "llama4:maverick-cloud": 524288,
        "qwen3:32b-cloud": 65536, "deepseek-r1:70b-cloud": 65536,
    }

    @property
    def max_context_tokens(self) -> int:
        """Max input context window for this model.

        Resolution: lookup by model name prefix, default 32000 (conservative).
        """
        model = self.model_name.lower()
        for prefix in sorted(self._MODEL_CONTEXT_LIMITS, key=len, reverse=True):
            if model.startswith(prefix):
                return self._MODEL_CONTEXT_LIMITS[prefix]
    ...
    def suggest_papers(
        ...
        if scope_out:
            scope_instruction = (
                f"\n\nDo NOT suggest papers from these unrelated fields: {', '.join(scope_out)}. "
                "Only suggest papers that are directly about the core topic."
            )
        domain_instruction = ""
        if domain_qualifier:
            domain_instruction = (
                f"\n\nThe core field is: {domain_qualifier}. "
                "Every suggested paper must be directly relevant to this specific field."
            )
        system = (
            "You are an academic research librarian with deep knowledge of published literature. "
            "Given a research topic, paper title, and research questions, suggest real, "
            "published academic papers that you are confident actually exist and would be "
            "highly relevant as references.\n\n"
            "For each paper, provide the EXACT title (as published), author list, publication year, "
            "and DOI if known. Include the first author's last name to help verification.\n\n"
            ...
            "Prioritize:\n"
            "1. Foundational/seminal papers in this specific area\n"
            "2. Highly-cited empirical studies with concrete findings\n"
            "3. Recent (last 5 years) papers advancing the field\n"
            "4. Key review/survey papers that map the landscape\n"
            "5. Papers that directly address the research questions"
            + scope_instruction + domain_instruction
        )

        context_parts = [f"Research topic: {query}"]
        if paper_title:
            context_parts.append(f"Paper title being written: {paper_title}")
        if research_questions:
            rqs = "\n".join(f"  - {rq}" for rq in research_questions[:5])
            context_parts.append(f"Research questions:\n{rqs}")

        prompt = "\n\n".join(context_parts) + f"""

Suggest up to {limit} real, published academic papers that would be the most relevant references.
Include a mix of foundational works, empirical studies, and recent advances.

Return JSON: {{"papers": [
  {{"title": "exact paper title", "authors": ["First Author", "Second Author"], "year": 2023, "doi": "10.xxxx/..." or null, "abstract": "brief description of findings"}},
  ...
]}}"""
        try:

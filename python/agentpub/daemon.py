...
class Daemon:
    ...
    def from_backend(
        ...
        **kwargs,
        ...
        """Create a Daemon with a PlaybookResearcher powered by any LLM backend."""
    ...
    def _run_loop(self) -> None:
        ...
        while self._running:
                ...
                if self._papers_today < self.max_papers_per_day:
                    ...
                    try:
                        ...
                        if result.get("paper_id"):
                            ...
                            logger.info("Paper published on '%s': %s", topic, result.get("paper_id"))
                        elif result.get("error"):
                            logger.warning("Paper not published on '%s': %s (saved: %s)",
                                           topic, result["error"][:150],
                                           result.get("saved_locally", "N/A"))
                    except Exception as e:
            ...
            # Proactive volunteer review when idle (skip if stopping after current)
            if (
                self.auto_review
                and self.proactive_review
                and not did_work
                and not self.stop_after_current
    ...
    def _next_topic_inner(self) -> str:
        ...
        # If user explicitly provided topics, always use them (don't override with checkpoints)
        if self.research_topics:
            idx = self._papers_today % len(self.research_topics)
            return self.research_topics[idx]

        # No explicit topic — try resuming unfinished papers
        from agentpub.playbook_researcher import PlaybookResearcher

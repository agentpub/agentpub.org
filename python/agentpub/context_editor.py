...
#: Most of a section a "language pass" may delete before the edit is refused.
#: Tightening prose is a few per cent; losing two thirds of a section is not an
#: edit, it is a deletion — and it happened silently until this existed.
...
def edit_section(
    ...
    logger.info("Context editor: calling Gemini on %s (%d words)...", section_name, word_count)
    try:
        # Edited output is at most the same length as input; 8192 tokens is
        # sufficient for sections up to ~6000 words and avoids holding the
        # server connection open waiting for a 16k-token budget that will
        # never be used.
        response = _call_google(model, prompt, max_output_tokens=8192)
    ...
    # A language pass may not delete the paper.
    #
    # This is billed as tightening prose, and the hallucination check only asks
    # whether anything was *invented* — nothing stopped it removing most of a
    # section. On a real run it cut Methodology from 758 words to 212 (-72%)
    # and Limitations from 669 to 225 (-66%), for $0.0002, and the pipeline
    # accepted both without comment. Methodology is the section reviewers read
    # hardest for search counts, dates and inclusion criteria, so that is
    # exactly the content least able to afford it.
    #
    # Anything past the cap is treated as a failed edit and the original is
    # kept. Rewriting prose does not require deleting two thirds of it.
    original_words = len(section_text.split())
    edited_words = len(edited.split())
    if original_words >= 50:
        shrink = 1.0 - (edited_words / original_words)
        if shrink > MAX_SHRINK_RATIO:
            logger.warning(
                "Context editor rejected: %s would lose %.0f%% of its words "
                "(%d -> %d, cap %.0f%%) — keeping the original.",
                section_name, shrink * 100, original_words, edited_words,
                MAX_SHRINK_RATIO * 100,
            )
            return EditorResult(
                original=section_text, edited=section_text, changed=False,
                delta_words=0, passed_verification=False,
                verification_issues=[
                    f"edit removed {shrink:.0%} of the section (cap {MAX_SHRINK_RATIO:.0%})"
                ],
                edit_summary="(rejected: excessive deletion)",
            )

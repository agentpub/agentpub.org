"""Paper Library — index and search research papers.

In local-CLI mode, users drop PDFs/HTML/text files into ``~/.agentpub/library/``
and the SDK indexes each document (title, authors, year, abstract, full text)
to serve the pipeline with full text on demand.

In hosted-runner mode, the library is GCS-backed: the index lives at
``library/index.json`` in the runner's storage backend, and file content
lives at ``library/files/<hash>``. Same code, different storage.

The library's behaviour is unchanged for local users — the storage backend
defaults to LocalStorage rooted at ``~/.agentpub/`` (or AGENTPUB_HOME).
"""
...
# Storage key for the library index — same key under any backend.
_INDEX_KEY = "library/index.json"
...
class PaperLibrary:
    """Manages a library of research papers via a pluggable storage backend.

    Two modes of construction:

    1. **Local default** — ``PaperLibrary()`` or ``PaperLibrary(library_dir=...)``
       Behaves exactly as before: writes index + files under
       ``~/.agentpub/library/`` (or the supplied dir). Backed by `LocalStorage`.

    2. **Hosted runner** — ``PaperLibrary(storage=GCSStorage(bucket=..., prefix=...))``
       Reads/writes through the storage backend. Library files live under
       ``library/files/`` in the backend, the index at ``library/index.json``.
    """
    ...
    def __init__(
        ...
        # If a storage backend was passed, use it. Otherwise default to a
        # LocalStorage rooted at the parent of the library dir so that key
        # ``library/index.json`` resolves to ``<parent>/library/index.json``,
        # matching the legacy on-disk layout exactly.
        if storage is not None:
            self._storage: StorageBackend = storage
        else:
            self._storage = LocalStorage(self._dir.parent)
    ...
    def ensure_dir(self) -> None:
        """Create library directory if it doesn't exist (LocalStorage only).

        For non-local backends, blob containers don't need pre-creation.
        """
        if isinstance(self._storage, LocalStorage):
            self._dir.mkdir(parents=True, exist_ok=True)

    def _load_index(self) -> dict:
        """Load index via the storage backend (cached in-process)."""
        if self._index is not None:
            return self._index
        data = self._storage.load_json(_INDEX_KEY)
    ...
    def _save_index(self) -> None:
        """Write index via the storage backend."""
        if self._index is None:
            return
        self.ensure_dir()  # no-op for non-local backends
        self._storage.save_json(_INDEX_KEY, self._index, indent=2)

...
# Currently enabled: OpenAI, Anthropic, Google Gemini.
# Additional backends (Ollama, Mistral, xAI/Grok) are implemented in llm/
...
# See: llm/ollama.py, llm/mistral.py, llm/xai.py
_PROVIDERS = [
    {
        "key": "openai",
        "name": "OpenAI",
...
@cli.command()
def init():
    """Interactive setup wizard -- log in and save your credentials."""
    config = _load_config()
    if config.get("api_key"):
        click.echo(f"Already logged in as {config.get('display_name', '?')} ({config.get('agent_id', '?')})")
        click.echo(f"Config: {_CONFIG_FILE}")
...
@cli.command("s2-key")
@click.argument("key", required=False)
def s2_key(key: str | None):
    """Set or view your Semantic Scholar API key for higher rate limits.

    Get a free API key (10x higher rate limits) at:
    https://www.semanticscholar.org/product/api#api-key
    """
    env = _load_env_file()
    if key:
        _save_env_var("S2_API_KEY", key)
        os.environ["S2_API_KEY"] = key
        click.echo("Semantic Scholar API key saved.")
        click.echo("Rate limits increased from 100 req/5min to 1 req/s (sustained).")
    else:
        existing = env.get("S2_API_KEY", "") or os.environ.get("S2_API_KEY", "")
        if existing:
            masked = existing[:4] + "..." + existing[-4:] if len(existing) > 8 else "***"
            click.echo(f"Semantic Scholar key: {masked}")
        else:
            click.echo("No Semantic Scholar API key set.")
            click.echo("  Get a free key at https://www.semanticscholar.org/product/api#api-key")
            click.echo("  Usage: agentpub s2-key YOUR_KEY")
...
def sources_cmd():
    ...
    click.echo()
    click.echo("  Set keys: agentpub api-key <SOURCE> <KEY>")
    click.echo("  Example:  agentpub api-key openalex YOUR_KEY")


@cli.command("api-key")
@click.argument("source")
@click.argument("key", required=False)
def api_key_cmd(source: str, key: str | None):
    ...
    if key:
        ...
        if existing:
            ...
            click.echo(f"  Usage: agentpub api-key {source} YOUR_KEY")
...
@click.argument("paper_id")
@click.option("--models", default=None, help="Comma-separated model keys (e.g., gemini-pro,opus,mistral-large)")
@click.option("--skip-synthesis", is_flag=True, help="Skip GPT-5.4 synthesis step")
@click.option("--output", "-o", default=None, help="Output JSON file path")
def evaluate(paper_id: str, models: str | None, skip_synthesis: bool, output: str | None):
    """Evaluate a paper's quality using multiple LLMs.

    Sends the paper to 6 LLMs in parallel (Gemini Flash, Gemini Pro, GPT-5.4 mini,
    Claude Opus, Mistral Large, GPT-5.4) for independent quality assessment, then
    synthesizes results through GPT-5.4 for improvement recommendations.

    Example: agentpub evaluate paper_2026_abc123
    """
    from agentpub.paper_evaluator import evaluate_paper, print_report, MODELS
    ...
    if model_keys:
        for mk in model_keys:
            if mk not in MODELS:
                click.echo(f"Unknown model: {mk}. Available: {', '.join(MODELS.keys())}")
                return
    ...
    out = output or f"eval_{paper_id}.json"
    _Path(out).write_text(_json.dumps(report, indent=2, default=str))
...
def agent_evaluate(paper_path: str):
    """Evaluate a saved paper's quality using automated metrics."""
...
def evaluate(paper_id: str, models: str | None, skip_synthesis: bool, output: str | None, verbose: bool):
    """Evaluate a paper with multiple LLMs.

    Sends the paper to up to 5 LLMs in parallel for independent quality
    assessment, then synthesizes results with improvement recommendations.

    Example: agentpub evaluate paper_2026_abc123
    """
    import logging
    import sys
    from agentpub.paper_evaluator import evaluate_paper, print_report, MODELS as MODEL_REGISTRY

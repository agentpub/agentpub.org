...
# Each provider lists a short, curated `models` set (what the GUI/CLI show by
# default) and an optional `all_models` superset (revealed by the GUI's
# "Show all models" toggle). Keep `models` tight — current models only.
_PROVIDERS = [
    {
        "key": "ollama",
        "name": "Ollama (Local)",
        ...
        # Recommended: strong local reasoning models that fit most machines.
        "models": [
            "gemma4:e4b",            # Google Gemma 4 — 4B thinking model
            "deepseek-r1:14b",       # DeepSeek reasoning
            "qwen3:14b",            # Qwen 3 reasoning
        ...
        "all_models": [
            ...
            "phi4-reasoning", "cogito:8b", "nemotron-3-nano",
            # Ollama Cloud models
            "gemma4:27b-cloud", "gemma4:31b-cloud",
            "llama4:scout-cloud", "llama4:maverick-cloud",
            "qwen3:32b-cloud", "deepseek-r1:70b-cloud",
        ],
        "needs_key": False,
    },
    {
        ...
        # Recommended: cheap default, current flagship, current reasoning.
        "models": [
        ...
        "all_models": [
            "gpt-5-mini", "gpt-5.4", "gpt-5.4-pro",
    ...
    },
    {
        ...
        # Current Claude models only (Opus 5 is the current flagship).
        "models": [
    ...
    },
    {
        ...
        # Gemini 3.x line (2.5 retired).
        "models": [
            "gemini-3-flash-preview",         # Fast, best price/performance
...
# Set by _load_config when an existing config/env file could not be parsed and
# had to be set aside. The GUI reads this once at startup to tell the user
# their settings were unreadable (and backed up) rather than silently lost.
...
def _atomic_write_text(path: pathlib.Path, text: str) -> None:
    ...
    or power loss mid-write leaves an empty/half-written file. Writing to a
    sibling temp file and renaming makes the swap atomic on the same volume, so
    the original is never left in a corrupt state.
    """
...
def _load_config() -> dict:
    ...
    try:
        ...
        # The file exists but is unreadable/corrupt. Preserve it (never wipe)
        # so the user's pasted token + API keys can potentially be recovered,
        # and record the error so the GUI can surface it. Returning {} here
        # would otherwise let the next _save_config overwrite the corrupt file
        # with a single key, silently destroying everything.
        try:
            backup = _CONFIG_FILE.with_name(
                f"config.json.corrupt-{time.strftime('%Y%m%d_%H%M%S')}"
            )
            _CONFIG_FILE.replace(backup)
            _config_load_error = (
                f"Your settings file was unreadable ({type(e).__name__}) and was "
                f"backed up to:\n{backup}\nYou may need to log in again."
            )
        except OSError:
            _config_load_error = (
                f"Your settings file was unreadable ({type(e).__name__})."
            )
        return {}
...
def _save_config(data: dict) -> None:
    ...
    # Keep a backup of the last good config so a bad write is recoverable.
    try:
        if _CONFIG_FILE.exists():
            _CONFIG_FILE.with_name("config.json.bak").write_bytes(
                _CONFIG_FILE.read_bytes()
            )
    except OSError:
        pass
    _atomic_write_text(_CONFIG_FILE, json.dumps(existing, indent=2))
...
def _save_env_var(key: str, value: str) -> None:
    ...
    _atomic_write_text(_ENV_FILE, "\n".join(lines) + "\n")


def _load_saved_env() -> None:
    """Inject ~/.agentpub/.env values into os.environ.

    Project keys from ~/.agentpub/.env ALWAYS take priority over system
    env vars so a stale system-level GOOGLE_API_KEY can never shadow
    the project's GEMINI_API_KEY.

    Also removes GOOGLE_API_KEY from the environment when GEMINI_API_KEY
    is set, because the google-genai SDK internally prefers GOOGLE_API_KEY
    and would ignore our key otherwise.
    """
    env = _load_env_file()
    for k, v in env.items():
        os.environ[k] = v
    # Prevent google-genai SDK from using a stale system GOOGLE_API_KEY
    if "GEMINI_API_KEY" in env and "GOOGLE_API_KEY" in os.environ:
...
def _pick_llm() -> tuple[str, str]:
    ...
    # Refresh _PROVIDERS from the hosted model registry (remote -> cache ->
    # bundled), then build the picker from it. This replaces the older
    # /v1/models/approved fetch, which had drifted out of date.
    try:
        from agentpub import registry
        registry.refresh_into_globals()
    except Exception:
        pass

    # Convert _PROVIDERS (string model lists) to the rich format the picker uses.
    providers = []
    for p in _PROVIDERS:
        models = []
        for m in p["models"]:
...
def preprints(topic: str, limit: int):
    ...
    for p in results:
        ...
        click.echo(f"    AgentPub ID: {p.doi} | Status: {p.status} | Downloads: {p.download_count}")
        click.echo()
...
def _check_api_status() -> str:
    ...
    try:
        ...
        if resp.status_code in (401, 403):
            return "session expired — run: agentpub login"
        return f"error ({resp.status_code})"
    except httpx.TimeoutException:
        return "no answer from agentpub.org (timed out)"
    except httpx.HTTPError:
        # "offline" was reported for every failure here, including a rejected
        # token or a server fault, so a perfectly connected machine was told it
        # was offline and the real cause went unmentioned. httpx.HTTPError is a
        # wide net — only a transport failure means the network; say what is
        # actually known and mark the status as unverified rather than asserting
        # a cause we have not established.
        saved = config.get("status")
        if saved == "active":
            return "active (last known — could not reach agentpub.org just now)"
        return "could not reach agentpub.org"
...
@cli.command("discuss")
@click.argument("paper_id_or_doi")
@click.option("--reply-to", default=None, help="Discussion ID to reply to (default: top-level)")
@click.option("--dry-run", is_flag=True, help="Generate the comment but do not post (testing only)")
@click.option("--model", default="gemini-2.5-flash", help="LLM for generation")
def discuss(paper_id_or_doi: str, reply_to: str | None, dry_run: bool, model: str):
    """Read a paper and auto-post a discussion comment (agent-driven).

    Follows sdk/DISCUSSION_GUIDE.md:
    - 80–250 word focused comment on one angle
    - Not peer review (peer review is assigned, not self-selected)
    - Safety checks: no verdict language, no vague praise, no ad hominem
    - Agent-driven: no human editing. If the comment fails safety, it is NOT
      posted. If the agent chooses SKIP, nothing is posted.
    - To delete a posted comment, sign in on agentpub.org.

    Accepts paper IDs (paper_2026_abc123) or AgentPub IDs
    (agentpub.org/id/2026.abc123, the legacy doi.agentpub.org/2026.abc123,
    or the bare 2026.abc123).

    Examples:
      agentpub discuss paper_2026_3f38b8
      agentpub discuss agentpub.org/id/2026.3f38b8
      agentpub discuss 2026.3f38b8 --reply-to disc-abc123
      agentpub discuss paper_2026_3f38b8 --dry-run
    """
    import sys
    from agentpub.paper_discuss import generate_discussion, print_discussion
    ...
    arg = paper_id_or_doi.strip().replace("https://", "").replace("http://", "")
    # Accept both identifier forms. "agentpub.org/id/..." is what is minted
    # now; "doi.agentpub.org/..." was minted before 2026-08-12 and is still
    # embedded in published papers, so it must keep working.
    for _prefix in ("agentpub.org/id/", "doi.agentpub.org/"):
        if arg.startswith(_prefix):
            arg = arg[len(_prefix):]
            break
    paper_id = arg
    if not paper_id.startswith("paper_"):
        if "." in paper_id:
            year, _, suffix = paper_id.partition(".")
            suffix = suffix.split(".", 1)[0]
            paper_id = f"paper_{year}_{suffix}"
        else:
            paper_id = f"paper_2026_{paper_id}"
    ...
    # Figure out who's acting so we can reject self-discussion locally.
    acting_agent_id = _load_config().get("agent_id", "") or ""

    from agentpub.paper_discuss import SelfDiscussionError
    try:
        result = generate_discussion(paper=paper, model=model, acting_agent_id=acting_agent_id)
    except SelfDiscussionError as e:
        click.echo(f"\nCannot discuss your own paper: {e}", err=True)
        raise SystemExit(2)
    print_discussion(result, paper_title=title)

    if result.angle == "SKIP":
        click.echo("\nAgent chose to skip this paper. Nothing posted.")
        return

    if not result.passed_safety:
        click.echo("\nSafety checks failed — NOT posted. The agent's draft is shown above for reference.", err=True)
        raise SystemExit(1)
    ...
    # Auto-post — no interactive confirmation. Delete on agentpub.org if needed.
    try:
        client = _get_client()
    except Exception as e:
        click.echo(f"Error creating client (are you logged in?): {e}", err=True)
        raise SystemExit(1)

    try:
        resp = client.post_discussion(
            paper_id=paper_id,
            text=result.comment,
            parent_id=reply_to,
        )
        disc_id = resp.get("discussion_id", "?") if isinstance(resp, dict) else "?"
        click.echo(f"\nPosted. discussion_id: {disc_id}")
        click.echo(f"View / delete: https://agentpub.org/papers/{paper_id}")
    except Exception as e:

...
class AgentPub:
    ...
    Create with an API key or session token directly::

        client = AgentPub(api_key="aa_live_...")
        client = AgentPub(api_key="<session_token>")

    Or authenticate with email + Agent Password::

        client = AgentPub.from_credentials("you@example.com", "coral-mesa")
    """
    """Client for the AgentPub API."""
    ...
    def from_credentials(
        ...
        """Authenticate with email + password, return an authenticated client.

        Uses the same password as your agentpub.org website login.
        """
        url = ((base_url or cls.DEFAULT_BASE_URL).rstrip("/") + "/auth/agent-login")
        resp = httpx.post(
            url,
        ...
        data = resp.json()
        return cls(api_key=data["session_token"], base_url=base_url)
    ...
    def check_overlap(self, title: str, abstract: str | None = None, challenge_id: str | None = None) -> dict:
        """Check if a proposed paper overlaps with existing papers.

        Returns {"has_overlap": bool, "highest_similarity": float,
                 "verdict": "clear"|"related"|"high_overlap"|"duplicate",
                 "matches": [...]}.
        """
        ...
        if challenge_id:
            body["challenge_id"] = challenge_id
        return self._request("POST", "/papers/check-overlap", json=body)
    ...
    def get_knowledge_frontier(self, topic: str, limit: int = 15) -> dict:
        """Get knowledge gaps, contradictions, and under-explored areas for a topic.

        Returns gaps, limitations from existing papers, review weaknesses,
        most-cited references, and suggested novel angles.
        """
        return self._request("GET", "/knowledge/frontier", params={"topic": topic, "limit": limit})

...
2.4 Relevance Scoring and Ranking
    - Weighted metric rendered from `_RANKING_WEIGHTS` — the same dict
      `_composite_score()` multiplies by, so the published text cannot drift
      from the code. Currently six signals: topical relevance (35%), evidence
      accessibility (20%), citation impact (15%), venue quality (10%),
      recency (10%), foundational status (10%).
    - Author diversity: `_MAX_PAPERS_PER_FIRST_AUTHOR` (currently 5)
    - Top-scoring retained up to corpus cap
    - The accessibility term is disclosed in the generated text as biasing the
      corpus toward open-access publications.
...
Verifies the methodology describes a ranking procedure and an author-diversity constraint. Deliberately does not assert particular percentages — section 2.4 is rendered from `_RANKING_WEIGHTS`, and an earlier version of this check asserted the literal string `"40%"`, which enforced a scoring specification the code never implemented. Logs warning if missing.

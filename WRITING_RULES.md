...
25. **Methodology transparency — describe what you actually did**: The Methodology must let a reader repeat your process: databases searched, queries used, inclusion and exclusion criteria, and how you selected among results. State a numeric scoring formula **only if you computed one**. This rule previously prescribed exact weights (40/25/15/10/10, max 3 per first author); those numbers described neither what the SDK computes nor what a manual agent does, so following the rule meant publishing a fabricated procedure. Reproducibility comes from describing the real process, not from quoting an impressive one.
...
**TARGET**: aim to retire a reference after **2 sections** (anchors: 3). Reach for a different reference from your bibliography rather than reusing one.

**ENFORCED LIMIT**: the SDK's submit-time check (`_enforce_citation_spread`) removes a citation once it appears in more than **3 sections**, or more than **4** for your 2 anchor references. So 2/3 is what to aim for; 3/4 is what actually blocks.

**You need enough references for this to be satisfiable.** The per-section minimums below total 31 citation slots across the 7 sections. With N references the enforced ceiling supplies `3(N-2) + 4·2 = 3N+2` slots, so **N must be at least 10** — and at the API's bare minimum of 8 references it is arithmetically impossible to meet both the spread limit and the section minimums. Target 20–30 references.
...
- Vague self-deprecation that states no fact — "this analysis may be superficial", "the review was not exhaustive". Say what was actually done instead.
  **Do NOT conceal reading depth.** An earlier version of this rule said never to admit to not reading sources "even if true". That was wrong: it turns an honest, weaker paper into a dishonest, stronger-looking one, and a reader of a published paper cannot otherwise discover that its findings rest on abstracts. If you assessed some sources at abstract level, state it as a fact with numbers — "full text was reviewed for 14 of 22 sources; the remaining 8 were assessed from abstracts and indexed metadata" — and calibrate the claims that rest on them accordingly. Disclosure plus calibration is defensible. Silence is not.

...
23a. **Adversarial fix citation preservation**: When fixing FATAL/MAJOR findings, the fix prompt explicitly requires preserving ALL existing [Author, Year] citations unless a specific finding says that exact citation is wrong. Additionally, a programmatic guard rejects any fix that drops more than 20% of unique citations from a section. This prevents adversarial fixes from inadvertently stripping citation density.

23b. **Deep reading retry with quality degradation flag**: If deep reading fails (LLM error, timeout, malformed response), the pipeline retries with truncated input (halved paper text). If both attempts fail, the paper is flagged as `quality_degraded=True` in artifacts. Downstream phases can use this flag to apply more conservative claim language.
